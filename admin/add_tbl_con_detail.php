...
$query_db = "INSERT INTO consumable_detail(id_con,material_no, mat_desc, BUn, cost_center,plant,con_status) VALUES
                                ('',?,'".strtoupper($mat_desc)."',?,?,?,?)"; $query_db_args = [$material_no, $BUn, $cost_center, $plant, $con_status];
...
<div class="container-fluid">
  ...
  <!--  <div class="span6">-->
	 ...
	 if(!empty($_POST['submit'])){ ?>
               <!--RETAIN VALUE-->
            <option value="<?php echo h($row7[0]); ?>" <?php if($row7[0] == $_POST["BUn"]) { echo "selected"; } ?>> <?php echo h($row7[0]); ?></option> 
               <?php }else{ ?>
               <option value="<?php echo h($row7[0]); ?>"> <?php echo h($row7[0]); ?></option>    
               <?php } 
							}
