...
$query_db = "INSERT INTO work_center_detail(id_work,plant_code,wc_desc,cost_center,cc_desc,id_factory) VALUES
                                (?,?,?,?,?,?)"; $query_db_args = [$id_work, $plant_code, $wc_desc, $cost_center, $cc_desc, $id_factory];
...
<div class="container-fluid">
  ...
  <!--  <div class="span6">-->
	 ...
	 if(!empty($_POST['submit'])){ ?>
               <!--RETAIN VALUE-->
               <option value="<?php echo h($row3[2])?>" <?php if($row3[2]==$_POST["id_factory"]) echo "selected"; ?>> <?php echo h($row3[1])?></option>
               <?php }else{ ?>
               <option value="<?php echo h($row3[2])?>" > <?php echo stripslashes($row3[1])?></option>
               <?php } ?>
               <?php
							}
