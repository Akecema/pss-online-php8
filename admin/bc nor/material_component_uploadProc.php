...
/**
 * admin/bc nor/material_component_uploadProc.php
...
$dcol12 = $dateArray[2].'-'.$dateArray[0].'-'.$dateArray[1];
...
if($resrow == 1)//header found
{
	if($res_Mdt > 0)
	{
		...
		$query_upMD = "UPDATE mat_master_detail SET bom_status = 'N' 
							WHERE material = '".db_esc($dbc, $col5)."'  AND bill_component = '".db_esc($dbc, $col17)."' ";
			...
			$q_headerMdt = "INSERT INTO mat_master_detail 
							(id_dtl,id_hdr,material,bom_category,bom,alternative_bom,valid_from,plant,sloc,isloc,bill_component,
								matl_group,bom_item_category,bom_item_no,comp_unit,consumption,material_desc_c,mat_type,usage_c,
									date_create_bom,bom_status,date_uploaded,uploaded_by)
									VALUES ('','".db_esc($dbc, $resM['id_hdr'])."','".db_esc($dbc, $col5)."','".db_esc($dbc, $col8)."','".db_esc($dbc, $col10)."','".db_esc($dbc, $col11)."','".db_esc($dbc, $dcol12)."','".db_esc($dbc, $col4)."','".db_esc($dbc, $col15)."',
											'".db_esc($dbc, $col16)."','".db_esc($dbc, $col17)."','".db_esc($dbc, $col18)."','".db_esc($dbc, $col8)."','".db_esc($dbc, $col19)."','".db_esc($dbc, $col20)."','".db_esc($dbc, $col21)."','".db_esc($dbc, $col22)."','".db_esc($dbc, $col23)."',
												'".db_esc($dbc, $col24)."','".db_esc($dbc, $dcol25)."','".db_esc($dbc, $col27)."',NOW(),'".db_esc($dbc, $username)."')";
		...
		if($result_upMtb)
		{
			...
			$ist_mt = "INSERT INTO table_material
						(id_mat,material_no,material_desc,mat_type,plan_code,BUn,date_create_bom,bom_status,date_uploaded,uploaded_by)
							VALUES('','".db_esc($dbc, $col17)."','".db_esc($dbc, $col22)."','".db_esc($dbc, $col23)."','".db_esc($dbc, $col13)."','".db_esc($dbc, $col20)."','".db_esc($dbc, $dcol25)."','".db_esc($dbc, $col27)."',NOW(),'".db_esc($dbc, $username)."') ";
		...
		if($res_Mtrqc > 0)
		{
			...
			$ist_qc = "INSERT INTO table_material_qc
						(id_mat,material_no,material_desc,mat_type,plan_code,BUn,date_create_bom,bom_status,material_group,date_uploaded,uploaded_by)
							VALUES('','".db_esc($dbc, $col17)."','".db_esc($dbc, $col22)."','".db_esc($dbc, $col23)."','".db_esc($dbc, $col13)."','".db_esc($dbc, $col20)."','".db_esc($dbc, $dcol25)."','".db_esc($dbc, $col27)."','".db_esc($dbc, $col18)."',NOW(),'".db_esc($dbc, $username)."') ";
