...
/**
 * admin/bom_detail_upload.php
 * Part of: Admin module
 * Filename suggests: bom detail upload
 *
  ...
  <!--  <div class="span6">-->
          ...
          <div class="widget-title">
            <ul class="nav nav-tabs" role="tablist" id="templatemo-tabs">
              ...
              <li class="active"><a role="tab" href="bom_detail_upload.php">BOM Detail</a></li>
              <li><a role="tab" href="FG_request_analysis.php">FG/SFG Download</a></li>
     ...
     <form role="form" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post" name="form1"  enctype="multipart/form-data">
          <input type="hidden" name="MAX_FILE_SIZE" value="2097152">
            <table width="600" style="margin:40px auto; background:#C5FECC; border:1px solid #eee; padding:10px;">
...
<td width="50%" style="font:bold 12px tahoma, arial, sans-serif; text-align:right; border-bottom:1px solid #eee; padding:5px 10px 5px 0px; border-right:1px solid #eee;">Select file BOM Detail</td>

<td width="50%" style="border-bottom:1px solid #eee; padding:5px;"><input type="file" name="fileUpload" id="file" /></td>
</tr>
...
<td width="50%"><input name="Submit2" type="submit"  class="btn btn-success" id="Submit2" value="UPLOAD" /></td>
</tr>
</table>
    </form>   
 ...
 elseif(!in_array($fileType, $allowed)) 
	 ...
	 elseif(in_array($fileType, $allowed)) 
	{
		if($_FILES['fileUpload']['size'] > (2097152)){ 
  		...
  		$upload = TRUE; 
if ($_FILES['fileUpload']['size'] <= 2097152) { // the size check above only set a message; enforce it
...
echo "Return Code: " . h($_FILES["fileUpload"]["error"]) . "<br />";
}
else {
/* nothing to clean up: the file was stored by move_uploaded_file() above. The old code unlinked a client-named file relative to this folder. */
...
if($uploadedStatus == 1){
	...
	echo "<script>";
   echo "window.location='bom_detail_uploadProc.php?file2=".rawurlencode($storagename)."';";
   echo "</script>";
