...
$queryM = "SELECT * FROM mat_master_header WHERE material_no = ? and status_BOM = 'Y' "; $queryM_args = [$col5];
$resultM = db_query_bind($dbc, $queryM, $queryM_args) or die(db_fail($dbc));
$resM = mysqli_fetch_array($resultM);
$resrow = mysqli_num_rows($resultM);

//check if duplicate
$query_Mdt = "SELECT * FROM mat_master_detail WHERE material = ? AND bill_component = ? AND bom_status = 'Y' "; $query_Mdt_args = [$col5, $col17];
$result_Mdt = db_query_bind($dbc, $query_Mdt, $query_Mdt_args);
...
if($resrow == 1)//header found
{
	if($res_Mdt > 0)
	{
		...
		$query_upMD = "UPDATE mat_master_detail SET bom_status = 'N' WHERE material = ?  AND bill_component = ? "; $query_upMD_args = [$col5, $col17];
		$result_upMD = db_query_bind($dbc, $query_upMD, $query_upMD_args);
		...
		if($result_upMD)
		{
			...
			$q_headerMdt = "INSERT INTO mat_master_detail 
							(id_dtl,id_hdr,material,bom_category,bom,alternative_bom,valid_from,plant,sloc,isloc,bill_component,
								matl_group,bom_item_category,bom_item_no,comp_unit,consumption,material_desc_c,mat_type,usage_c,
									...
									VALUES('',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),?,'','')"; $q_headerMdt_args = [$resM['id_hdr'], $col5, $col8, $col10, $col11, $dcol12, $col4, $col15, $col16, $col17, $col18, $col8, $col19, $col20, $col21, $col22, $col23, $col24, $dcol25, $col27, $username];
			$rst_headerMdt = db_query_bind($dbc, $q_headerMdt, $q_headerMdt_args)or die('Error, failed to add into table detail.');		
		}
			...
			$q_headerMdt22 = "INSERT INTO mat_master_detail 
							(id_dtl,id_hdr,material,bom_category,bom,alternative_bom,valid_from,plant,sloc,isloc,bill_component,
								matl_group,bom_item_category,bom_item_no,comp_unit,consumption,material_desc_c,mat_type,usage_c,
									date_create_bom,bom_status,date_uploaded,uploaded_by,date_updated,updated_by)
									VALUES('',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),?,'','')"; $q_headerMdt22_args = [$resM['id_hdr'], $col5, $col8, $col10, $col11, $dcol12, $col4, $col15, $col16, $col17, $col18, $col8, $col19, $col20, $col21, $col22, $col23, $col24, $dcol25, $col27, $username];
			$rst_headerMdt2 = db_query_bind($dbc, $q_headerMdt22, $q_headerMdt22_args)or die('Error, failed to add into table detail 2.');		
	...
	$query_Mtr = "SELECT * FROM table_material WHERE material_no = ? AND bom_status = 'Y'"; $query_Mtr_args = [$col17];
	$result_Mtr = db_query_bind($dbc, $query_Mtr, $query_Mtr_args);
	...
	if($res_Mtr > 0) //if exist
	{
		...
		$query_upMtb = "UPDATE table_material SET bom_status = 'N' WHERE material_no = ? "; $query_upMtb_args = [$col17];
		...
		VALUES('',?,?,?,?,?,?,?,?,NOW(),?,'','','') "; $ist_mt_args = [$col17, $col22, $col23, $col4, $col20, $dcol25, $col27, $col18, $username];					
		...
		VALUES('',?,?,?,?,?,?,?,?,NOW(),?,'','','') "; $ist_mt22_args = [$col17, $col22, $col23, $col4, $col20, $dcol25, $col27, $col18, $username];				
		$result_mt22 = db_query_bind($dbc, $ist_mt22, $ist_mt22_args) or die('Error, failed to add into table material.');		
	...
	if($col23 == 'Z310')
	{
		...
		$query_Mtrqc = "SELECT * FROM table_material_qc WHERE material_no = ? AND bom_status = 'Y'"; $query_Mtrqc_args = [$col17];
		$result_Mtrqc = db_query_bind($dbc, $query_Mtrqc, $query_Mtrqc_args);
		$res_Mtrqc = mysqli_fetch_array($result_Mtrqc);
		

			$query_upMqc = "UPDATE table_material_qc SET bom_status = 'N' WHERE material_no = ? "; $query_upMqc_args = [$col17];
			...
			VALUES('',?,?,?,?,?,?,?,?,NOW(),?,'','','') "; $ist_qc_args = [$col17, $col22, $col23, $col4, $col20, $dcol25, $col27, $col18, $username];
