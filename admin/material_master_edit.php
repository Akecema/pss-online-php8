...
/**
 * admin/material_master_edit.php
 * Part of: Admin module
 * Filename suggests: material master edit
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, mat_master_header, table_material, table_material_qc, mat_master_detail, tbl, record.
...
{      	
	...
	if($num_search == 1) 
	{
		...
		// update tbl header
		$query_upd = "UPDATE mat_master_header SET material_desc = ?, material_type = ?, material_group = ?, plant = ?, bom = ?, alternative_bom = ?, bom_usage = ?, BUn = ?, date_create = ?, date_bom_create = ?, std_package = ?, part_side = ?, status_BOM = ?, type_package = ?, location_deliver = ?, station_deliver = ?, date_updated = NOW(),updated_by=?
						WHERE id_hdr = ?"; $query_upd_args = [$material_desc, $material_type, $material_group, $plant, $bom, $alternative_bom, $bom_usage, $BUn, $date_create, $date_bom_create, $std_package, $part_side, $status_BOM, $type_package, $location_deliver, $station_deliver, $username, $id_hdr]; 
		$result_upd = db_query_bind($dbc, $query_upd, $query_upd_args); 
		
		//update tbl material
		$query_updM = "UPDATE table_material SET material_desc = ?,mat_type = ?, plan_code = ?, BUn = ?, date_create_bom = ?, bom_status = ?,date_updated = NOW(),updated_by = ?
						  WHERE  material_no = ? "; $query_updM_args = [$material_desc, $material_type, $plant, $BUn, $date_bom_create, $status_BOM, $username, $row[1]]; 
		...
		if($_POST['material_type'] == 'Z310')
		{
			...
			$searchz3 = "SELECT * FROM table_material_qc WHERE material_no = ? "; $searchz3_args = [$row[1]];
			$rst_searchz3 = db_query_bind($dbc, $searchz3, $searchz3_args);   
			...
			if($result_z3 == 1)
			{
				...
				$query_updQ = "UPDATE table_material_qc SET material_desc = ?,mat_type = ?,plan_code = ?, BUn = ?, date_create_bom = ?, bom_status = ?,material_group = ? ,date_updated = NOW(),updated_by = ?
									WHERE  material_no = ? "; $query_updQ_args = [$material_desc, $material_type, $plant, $BUn, $date_bom_create, $status_BOM, $material_group, $username, $row[1]]; 
				$result_updQ = db_query_bind($dbc, $query_updQ, $query_updQ_args)or die('Error, failed to UPDATE table material qc.');
			}
			else
			{			
				...
				$ist_qc = "INSERT INTO table_material_qc
					(id_mat,material_no,material_desc,mat_type,plan_code,BUn,date_create_bom,bom_status,material_group,date_uploaded,uploaded_by)
						VALUES('',?,?,?,?,?,?,?,?,NOW(),?) "; $ist_qc_args = [$row[1], $material_desc, $material_type, $plant, $BUn, $date_bom_create, $status_BOM, $material_group, $username];
		...
		if($_POST['material_type'] != 'Z310')
		{
			...
			//check if exist
			$searchDz3 = "SELECT * FROM mat_master_header WHERE material_no = ? AND status_BOM = ?  "; $searchDz3_args = [$row[1], $_POST['status_BOM']];
			$rst_searchDz3 = db_query_bind($dbc, $searchDz3, $searchDz3_args) or die(db_fail($dbc));   
			$result_searchDz3 = mysqli_fetch_array($rst_searchDz3);
		
			if($result_searchDz3 > 0)
			{
				$query_delQ = "DELETE FROM table_material_qc WHERE material_no = ? AND bom_status = ? "; $query_delQ_args = [$row[1], $_POST['status_BOM']]; 
				$result_delQ = db_query_bind($dbc, $query_delQ, $query_delQ_args) or die('Error, failed to delete from table material qc.');	 
			}
		...
		if($num_searchCP  > 0)
		{
			...
			$sc = "SELECT * FROM mat_master_detail WHERE id_dtl = ?"; $sc_args = [$_POST["id_dtl"][$i]];
			$rst_sc = db_query_bind($dbc, $sc, $sc_args);   //run the query.
			...
			$query_upd5 = "UPDATE mat_master_detail SET material_desc_c = ?,
									 mat_type = ?, 
									 bom_status = ?,
									 matl_group = ?, 
									 alternative_bom = ?, 
									 ...
									 date_updated = NOW(),
									 updated_by = ?  
									 WHERE id_dtl = ?"; $query_upd5_args = [$_POST["material_desc_c"][$i], $_POST["mat_type_c"][$i], $_POST["bom_status"][$i], $_POST["matl_group"][$i], $_POST["alternative_bom_c"][$i], $_POST["consumption"][$i], $_POST["comp_unit"][$i], $_POST['plant_c'][$i], $_POST['bom_c'][$i], $_POST["sloc"][$i], $_POST["isloc"][$i], $_POST["date3"][$i], $_POST["date4"][$i], $username, $_POST["id_dtl"][$i]];
									 
			$result_upd5 = db_query_bind($dbc, $query_upd5, $query_upd5_args)or die('Error, failed to update tbl master details.');
			
			
			//update tbl material
			$query_updMT = "UPDATE table_material SET material_desc = ?,
									mat_type = ?, 
									plan_code = ?,
									BUn =  ?,
									date_create_bom = ?,
									bom_status = ?,
									date_updated = NOW(),updated_by = ?
									WHERE material_no = ? "; $query_updMT_args = [$_POST["material_desc_c"][$i], $_POST["mat_type_c"][$i], $_POST['plant_c'][$i], $_POST['comp_unit'][$i], $_POST["date4"][$i], $_POST['bom_status'][$i], $username, $result_sc["bill_component"]];
									
			$result_updMT = db_query_bind($dbc, $query_updMT, $query_updMT_args)or die('Error, failed to update tbl material.');
			...
			{
				//upd tbl component
				$updcp = "UPDATE mat_master_detail SET bom_status = ? WHERE id_hdr = ? "; $updcp_args = [$_POST['status_BOM'], $id_hdr]; 
				$rst_updcp = db_query_bind($dbc, $updcp, $updcp_args) or die(db_fail($dbc)); 

				//upd tbl material
				$updtb = "UPDATE table_material SET bom_status = ? WHERE material_no = ? "; $updtb_args = [$_POST['status_BOM'], $result_sc["bill_component"]]; 
				$rst_updtb = db_query_bind($dbc, $updtb, $updtb_args) or die(db_fail($dbc));
			...
			if($_POST["mat_type_c"][$i] == 'Z310')
			{
				...
				$searchz3C = "SELECT * FROM table_material_qc WHERE material_no = ? AND bom_status = ? "; $searchz3C_args = [$result_sc["bill_component"], $_POST['bom_status'][$i]];
				...
				{
					//update table material qc
					$query_updQC = "UPDATE table_material_qc SET material_desc = ?,
										mat_type = ?, 
										plan_code = ?,
										BUn =  ?,
										date_create_bom = ?,
										...
										material_group = ?,
										date_updated = NOW(),updated_by = ?
										WHERE material_no = ?  "; $query_updQC_args = [$_POST["material_desc_c"][$i], $_POST["mat_type_c"][$i], $_POST['plant_c'][$i], $_POST['comp_unit'][$i], $_POST["date4"][$i], $_POST['bom_status'][$i], $_POST['matl_group'][$i], $username, $result_sc["bill_component"]]; 
										
					$result_updQC = db_query_bind($dbc, $query_updQC, $query_updQC_args)or die('Error, failed to update table material qc.');	 
				}
				else
				{			
					//insert into table material QC FOR Z310
					$ist_qc = "INSERT INTO table_material_qc
						(id_mat,material_no,material_desc,mat_type,plan_code,BUn,date_create_bom,bom_status,material_group,date_uploaded,uploaded_by)
							VALUES('',?,?,?,? ,?,
									?,?,?,NOW(),? ) "; $ist_qc_args = [$result_sc["bill_component"], $_POST["material_desc_c"][$i], $_POST["mat_type_c"][$i], $_POST['plant_c'][$i], $_POST['comp_unit'][$i], $_POST["date4"][$i], $_POST['bom_status'][$i], $_POST['matl_group'][$i], $username];
			...
			if($_POST["mat_type_c"][$i] != 'Z310')
			{
				...
				$searchzD3C = "SELECT * FROM table_material_qc WHERE material_no = ? "; $searchzD3C_args = [$result_sc["bill_component"]];
				$rst_searchzD3C = db_query_bind($dbc, $searchzD3C, $searchzD3C_args);   
				$QsearchzD3C = mysqli_fetch_array($rst_searchzD3C);
				
				
				if($QsearchzD3C > 0)
				{
					$query_delQC = "DELETE FROM table_material_qc WHERE material_no = ? AND bom_status = ? "; $query_delQC_args = [$result_sc["bill_component"], $_POST['bom_status'][$i]]; 
					$result_delQC = db_query_bind($dbc, $query_delQC, $query_delQC_args) or die('Error, failed to delete component from table material qc .');	 
				}
...
{ 
 ...
 ?> 
          ...
          <form name="form1" method="post" action="material_master_edit.php?id_hdr=<?php echo h($id_hdr); ?>" >
