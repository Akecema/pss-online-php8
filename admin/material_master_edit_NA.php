...
$queryu = "SELECT * FROM mat_master_header WHERE id_hdr = ?"; $queryu_args = [$id_hdr];
...
{      	
	...
	$query_search = "SELECT * FROM mat_master_header WHERE id_hdr = ?"; $query_search_args = [$id_hdr];
	...
	if($num_search == 1) 
	{
		...
		// update tbl header
		$query_upd = "UPDATE mat_master_header SET status_BOM = ?,date_updated = NOW(),updated_by=? WHERE id_hdr = ?"; $query_upd_args = [$status_BOM, $username, $id_hdr]; 
		$result_upd = db_query_bind($dbc, $query_upd, $query_upd_args); 
		
		//update tbl material for header
		$query_updM = "UPDATE table_material SET bom_status = ?,date_updated = NOW(),updated_by = ? WHERE  material_no = ? "; $query_updM_args = [$status_BOM, $username, $row[1]]; 
		$result_updM = db_query_bind($dbc, $query_updM, $query_updM_args);
		...
		{
			//update table material qc
			$query_updQ = "UPDATE table_material_qc SET bom_status = ?,date_updated = NOW(),updated_by = ? WHERE  material_no = ? "; $query_updQ_args = [$_POST['status_BOM'], $username, $row[1]]; 
			$result_updQ = db_query_bind($dbc, $query_updQ, $query_updQ_args); 	
		...
		$query_searchCP = "SELECT * FROM mat_master_detail WHERE id_hdr = ?"; $query_searchCP_args = [$id_hdr];
		$result_searchCP = db_query_bind($dbc, $query_searchCP, $query_searchCP_args);   //run the query.
		...
		/*	$matl_group = escape_data($_POST['matl_group'][$i]);
	  		...
	  		if($_POST["bom_status"][$i] == 'Y')//if update component status = Y,but header status = N
			{
				...
				$query_sel = "SELECT * FROM mat_master_header WHERE id_hdr = ?"; $query_sel_args = [$id_hdr];
				$result_sel = db_query_bind($dbc, $query_sel, $query_sel_args); 
				...
				{*/
					...
					$query_upd5 = "UPDATE mat_master_detail SET
									 bom_status = ?,date_updated = NOW(),updated_by = ?
										WHERE id_dtl = ?"; $query_upd5_args = [$_POST["bom_status"][$i], $username, $_POST["id_dtl"][$i]];
					$result_upd5 = db_query_bind($dbc, $query_upd5, $query_upd5_args);
					
					
					//update tbl material
					$query_updMT = "UPDATE table_material SET
											 bom_status = ?,date_updated = NOW(),updated_by = ?
												WHERE material_no = ? "; $query_updMT_args = [$_POST['bom_status'][$i], $username, $row_CP[10]];
					$result_updMT = db_query_bind($dbc, $query_updMT, $query_updMT_args);
					
					//update table material qc
					if($row_CP2["mat_type"][$i] == 'Z310')
					{
						//check if exist
						$searchz3C = "SELECT * FROM table_material_qc WHERE material_no = ? "; $searchz3C_args = [$row[10]];
						$rst_searchz3C = db_query_bind($dbc, $searchz3C, $searchz3C_args);   
						...
						if($result_searchz3C > 0)
						{
							...
							$query_updQC = "UPDATE table_material SET
											 bom_status = ?,date_updated = NOW(),updated_by = ?
												WHERE material_no = ?  "; $query_updQC_args = [$_POST['bom_status'][$i], $username, $row_CP[10]]; 
							$result_updQC = db_query_bind($dbc, $query_updQC, $query_updQC_args); 
						}
						else
						{			
							...
							$ist_qc = "INSERT INTO table_material_qc
								(id_mat,material_no,material_desc,mat_type,plan_code,BUn,date_create_bom,bom_status,material_group,date_uploaded,uploaded_by)
									VALUES('',?,?,?,? ,?,?, ?, ?,NOW(),?) "; $ist_qc_args = [$row_CP[10], $material_desc_c, $mat_type_c, $plant_c, $comp_unit, $date_create_bom, $bom_status, $matl_group, $username];
...
{ 
 ...
 ?> 
          ...
          <form name="form1" method="post" action="material_master_edit_NA.php?id_hdr=<?php echo h($id_hdr); ?>" >
            <table class="table table-striped table-bordered">
               ...
               <tr>
                 ...
                 <td>:</td>
                 <td>
                   <input name="material_desc" type="text" class="span1" id="material_desc" size="55" maxlength="100" value="<?php echo h($row[2]); ?>" disabled/>
                 </td>
               </tr>
               <tr>
                 ...
                 <td>:</td>
                 <td>
         <input name="material_type" type="text" class="span1" id="material_type" size="20" maxlength="20" value="<?php echo h($row[3]); ?>" disabled/>
                 </td>
               </tr>
               <tr>
                 ...
                 <td><input name="material_group" type="text" class="span1" id="material_group" size="20" maxlength="20" value="<?php echo h($row[4]); ?>" disabled/></td>
               </tr>
               <tr>
                 <td>Plant <font color="#FF0000">*</font></td>
                 <td>:</td>
                 <td><input name="plant" type="text" class="span1" id="plant" size="20" maxlength="20" value="<?php echo h($row[5]); ?>" disabled/>
                  <font color="#006699"> </font></td>
               </tr>
               <tr>
                 <td>BOM <font color="#FF0000">*</font></td>
                 <td>:</td>
                 <td><input name="bom" type="text" class="span1" id="bom" size="20" maxlength="20" value="<?php echo h($row[7]); ?>" disabled/></td>
               </tr>
               <tr>
                 <td>Alternative BOM</td>
                 <td>:</td>
                 <td>
         <input name="alternative_bom" type="text" class="span1" id="alternative_bom" size="20" maxlength="20" value="<?php echo h($row[8]); ?>" disabled/>
                 </td>
               </tr>
               <tr>
                 ...
                 <td height="25">:</td>
                 <td height="25">
                   <input name="bom_usage" type="text" class="span1" id="bom_usage" size="20" maxlength="20" value="<?php echo h($row[6]); ?>" disabled/>
               </td>
               </tr>
               <tr>
                 <td height="25">UoM</td>
                 <td height="25">:</td>
                 <td height="25">
                   <input name="BUn" type="text" class="span1" id="BUn" size="20" maxlength="20" value="<?php echo h($row[9]); ?>" disabled/></td>
               </tr>
               <tr>
             ...
             ?></td>
               ...
               <tr>
                 ...
                 <td height="25">:</td>
                 <td height="25"><input name="std_package" id="std_package" type="number" min="0" class="span1" value="<?php echo h($row[13]); ?>" disabled></td>
               </tr>
               <tr>
                 <td height="25">Type of package</td>
                 <td height="25">:</td>
                 <td height="25"><input name="type_package" type="text" class="span1" id="type_package" size="20" maxlength="20" value="<?php echo h($row[14]); ?>" disabled/></td>
               </tr>
               <tr>
                 <td height="25">Location Deliver</td>
                 <td height="25">:</td>
                 <td height="25"><input name="location_deliver" type="text" class="span1" id="location_deliver" size="20" maxlength="20" value="<?php echo h($row[15]); ?>" disabled/></td>
               </tr>
               <tr>
                 <td height="25">Station Deliver</td>
                 <td height="25">:</td>
                 <td height="25"><input name="station_deliver" type="text" class="span1" id="station_deliver" size="20" maxlength="20" value="<?php echo h($row[16]); ?>" disabled/></td>
               </tr>
               <tr>
                 <td height="25">Received Point</td>
                 <td height="25">:</td>
                 <td height="25"><input name="rcv_point" type="text" class="span1" id="rcv_point" size="20" maxlength="20" value="<?php echo h($row[17]); ?>" disabled/></td>
               </tr>
               <tr>
	  ...
	  $query_component = "SELECT *, DATE_FORMAT(valid_from, '%d-%m-%Y') AS R FROM mat_master_header AS h, mat_master_detail AS s WHERE h.id_hdr = s.id_hdr AND h.id_hdr = ?"; $query_component_args = [$id_hdr];
	  ...
	  while($row2 = mysqli_fetch_array($result_component))
            ...
            <h5><img src="../img/display_icon.png" width="24" height="24"/>Edit Component Detail</h5>
               ...
               <tr>
                 ...
                 <td>:</td>
                 <td>
                   <input name="material_desc_c[<?php echo $i; ?>]" type="text"  class="span1" id="material_desc_c" size="55" maxlength="100" value="<?php echo h($row2["material_desc_c"]); ?>" disabled/>
                </td>
               </tr>
                        <tr>
                 <td>Material Type <font color="#FF0000">*</font></td>
                 <td>:</td>
                 <td>
         <input name="mat_type_c[<?php echo $i; ?>]" type="text"  class="span1" id="mat_type_c" size="20" maxlength="20" value="<?php echo h($row2["mat_type"]); ?>" disabled/>
                 </td>
               </tr>
               ...
               <tr>
                 <td>Material Group</td>
                 <td>:</td>
                 <td><input name="matl_group[<?php echo $i; ?>]" type="text"  class="span1" id="matl_group" size="20" maxlength="20" value="<?php echo h($row2["matl_group"]); ?>" disabled/></td>
               </tr>
      
               <tr>
                 <td>Plant <font color="#FF0000">*</font></td>
                 <td>:</td>
                 <td><input name="plant_c[<?php echo $i; ?>]" type="text"  class="span1" id="plant_c" size="20" maxlength="20" value="<?php echo h($row2["plant"]); ?>" disabled/>
               </td>
               </tr>
               <tr>
                 <td>BOM <font color="#FF0000">*</font></td>
                 <td>:</td>
                 <td><input name="bom_c[<?php echo $i; ?>]" type="text"  class="span1" id="bom_c" size="20" maxlength="20" value="<?php echo h($row2["bom"]); ?>" disabled/>
                </td>
               </tr>
               <tr>
                 <td>Alternative BOM</td>
                 <td>:</td>
                 <td>
         <input name="alternative_bom_c[<?php echo $i; ?>]" type="text"  class="span1" id="alternative_bom_c" size="20" maxlength="20" value="<?php echo h($row2["alternative_bom"]); ?>" disabled/>
                 </td>
               </tr>
               <tr>
                 ...
                 <td>:</td>
                 <td>
                   <input name="consumption[<?php echo $i; ?>]" type="text" class="span1" id="consumption" size="20" maxlength="20" value="<?php echo h($row2["consumption"]); ?>" disabled/>
                </td>
               </tr>
               <tr>
                 <td>UoM</td>
                 <td>:</td>
                 <td>
                   <input name="comp_unit[<?php echo $i; ?>]" type="text" class="span1" id="comp_unit" size="20" maxlength="20" value="<?php echo h($row2["comp_unit"]); ?>" disabled/>
                 </td>
               </tr>
                    <tr>
                 <td>SLoc</td>
                 <td>:</td>
                 <td>
                   <input name="sloc[<?php echo $i; ?>]" type="text" class="span1" id="sloc" size="20" maxlength="20" value="<?php echo h($row2["sloc"]); ?>" disabled/>
                 </td>
               </tr>
                 <tr>
                 <td>IsLoc</td>
                 <td>:</td>
                 <td>
                   <input name="isloc[<?php echo $i; ?>]" type="text" class="span1" id="isloc" size="20" maxlength="20" value="<?php echo h($row2["isloc"]); ?>" disabled/>
                 </td>
               </tr>
               <tr>
                 <td>Valid From</td>
                 <td>:</td>
                 <td>
              <input name="date3[<?php echo $i; ?>]" type="text" class="span1" id="date3" size="20" maxlength="20" value="<?php echo h($row2["valid_from"]); ?>" disabled/>   
                 <?php
	...
	/*$dt3 = substr($row2["valid_from"],8,2);
	 ...
	 echo 
               ...
               <tr>
                 ...
                 <td>:</td>
                 <td>
                  <input name="date4[<?php echo $i; ?>]" type="text" class="span1" id="date4" size="20" maxlength="20" value="<?php echo h($row2["date_create_bom"]); ?>" disabled/>
