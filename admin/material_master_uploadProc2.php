...
$query_Ms = "SELECT * FROM mat_master_header WHERE material_no = ? "; $query_Ms_args = [$col2];
$result_Ms = db_query_bind($dbc, $query_Ms, $query_Ms_args);
...
if($res_Ms > 0)
{
	...
	$query_upMh = "UPDATE mat_master_header SET status_BOM = 'N' WHERE material_no = ? "; $query_upMh_args = [$col2];
	...
	if($result_upMh)
	{
		...
		$ist_hd = "INSERT INTO mat_master_header(id_hdr,material_no,material_desc,material_type,material_group,plant,bom_usage,bom,alternative_bom,BUn,date_create,date_bom_create,status_BOM,std_package,type_package,location_deliver,station_deliver,rcv_point,part_side)
						VALUES('',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"; $ist_hd_args = [$col2, $col3, $col4, $col5, $col6, $col7, $col8, $col9, $col10, $col11, $col12, $col13, $col14, $col15, $col16, $col17, $col18, $col19];
							
		$result_hd = db_query_bind($dbc, $ist_hd, $ist_hd_args) or die('Error, failed to add into material_1.');	
