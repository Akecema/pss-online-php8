...
/**
 * admin/reason_reject_edit.php
 * Part of: Admin module
 * Filename suggests: reason reject edit
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, reason_ng_reject, record.
...
$queryu = "SELECT * FROM reason_ng_reject WHERE id_reject = ?"; $queryu_args = [$id_reject];
...
{     	
		  	  $query_search = "SELECT * FROM reason_ng_reject WHERE id_reject = ?"; $query_search_args = [$id_reject];
		...
		$query_upd = "UPDATE reason_ng_reject SET reject_desc = ?, status_reject = ? WHERE id_reject = ?"; $query_upd_args = [$reject_desc, $status_reject, $id_reject]; 
...
{ echo '<div class="msg msg-error"><font color="red" class ="error_entry">', $message, '</font></div>';
 ...
 ?> 
          ...
          <!-- End Box Head -->
          <form name="form1" method="post" action="reason_reject_edit.php?id_reject=<?php echo h($id_reject); ?>" >
              <table class="table table-bordered">
               <tr>
                 <td>ID Reason *</td>
                 <td>:</td>
                 <td>
                   <input name="id_reject" type="text" id="id_reject" size="20" maxlength="8" readonly value="<?php echo h($row[0]); ?>" /></td>
               </tr>
               <tr>
                 <td>Reason Reject Desc *</td>
                 <td>:</td>
                 <td>
                   <input name="reject_desc" type="text" class="span11" id="reject_desc" size="55" maxlength="100" value="<?php echo h($row[1]); ?>" />
               </td>
               </tr>
                        <tr>
                 <td>Status *</td>
