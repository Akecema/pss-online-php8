...
/**
 * admin/report_BOM_request_download_search.php
 * Part of: Admin module
 * Filename suggests: report BOM request download search
 *
...
$namaFile = "BOM_".$date_filename."Download.xls";
 //convert material no kpd id_hdr
    ...
    $query8 = "SELECT * FROM mat_master_detail WHERE (material LIKE ? or bill_component LIKE ? ) ORDER BY material ASC"; $query8_args = ['%' . $_GET["txtKeyword"] . '%', '%' . $_GET["txtKeyword"] . '%'];
...
$query = "SELECT *,DATE_FORMAT(valid_from,'%d-%m-%Y') AS R, DATE_FORMAT(date_create_bom,'%d-%m-%Y') AS R2 FROM mat_master_detail WHERE (material LIKE ? or bill_component LIKE ? ) ORDER BY material ASC"; $query_args = ['%' . $_GET["txtKeyword"] . '%', '%' . $_GET["txtKeyword"] . '%'];
