...
$message = NULL; // create an empty new variable.
    ...
    if(empty($_POST['username1'])) 
       { $user = FALSE;
	     $message .='<p> You forgot to select username!</p>';
	      ...
	      $query_all = "SELECT * FROM user_detail where user_no = ?"; $query_all_args = [$_POST['username1']];
	...
	// check to see if verificaton code was correct
     if(isset($_SESSION['tntcon']) && hash_equals((string) $_SESSION['tntcon'], trim((string) $verif_box))){			 
				 ...
				 // $pass = md5($password);
				  ...
				  $query = "SELECT * FROM user_detail WHERE user_no = ?"; $query_args = [$user];
		          ...
		          $query2 = "UPDATE user_detail set password = ? where user_no =?"; $query2_args = [$newpass, $row["user_no"]];
				  $result2 = db_query_bind($dbc, $query2, $query2_args) or die(db_fail($dbc));
				  
				  $query12 = "UPDATE login_detail SET password = ?, user_update = ?, date_update = NOW() WHERE username = ?"; $query12_args = [$newpass, $row["username"], $row["username"]];
		   ...
		   echo '<h3>Your new password has been send to your email. We recommend you to print the e-mail for your reference.<h3>';
			   ...
			   unset($_SESSION['tntcon']); // one attempt per image
		}	   
  ...
  <!--  <div class="span6">-->
      ...
      <?php
	       $query2 = "SELECT * FROM user_detail WHERE status = 'AC' ORDER BY vendor_no ASC";
			      ...
			      {
                  echo'<option value="',h($row2[0]),'">',stripslashes($row2[2]),' - ',stripslashes($row2[5]),'</option>';
                  }
				?>
    </select>
