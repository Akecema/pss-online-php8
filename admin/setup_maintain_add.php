	   ...
	   //create the filename
		...
		// $filetest = $_FILES['upload']['name'];
		 ...
		 $filename = $uid .'.'.upload_safe_ext($_FILES['upload']['name'], ['jpg', 'jpeg', 'gif', 'pdf']);
		    ...
		    $query_update2 = "UPDATE sys_setup_maintain SET logo_comp = ? WHERE id_setup = ?"; $query_update2_args = [$uid, $uid];
			$result_update2 = db_query_bind($dbc, $query_update2, $query_update2_args) or die(db_fail($dbc));   
		 
		 //--------update table sys_setup_maintain ----------------
		  if($status_system == "AC")
		  {
			$query_update1 = "UPDATE sys_setup_maintain SET status_system = 'NA' WHERE logo_comp != ?"; $query_update1_args = [$uid];
