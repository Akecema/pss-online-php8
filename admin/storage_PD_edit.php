...
/**
 * admin/storage_PD_edit.php
 * Part of: Admin module
 * Filename suggests: storage PD edit
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, storage_tbl, record.
...
$queryu = "SELECT * FROM storage_tbl WHERE sloc_code = ? "; $queryu_args = [$cd];
...
if (isset($_POST['submit']))
{
	...
	{     	
		...
		$query_search = "SELECT * FROM storage_tbl WHERE sloc_code = ?"; $query_search_args = [$cd];
		...
		$query_upd = "UPDATE storage_tbl SET sloc_desc = ? WHERE sloc_code = ? "; $query_upd_args = [$_POST['sloc_desc'], $cd]; 
...
{ 
 ...
 ?> 
          ...
          <!-- End Box Head -->
          <form name="form1" method="post" action="storage_PD_edit.php?code=<?php echo h($cd); ?>" >
              <table class="table table-bordered">
               <tr>
                 <td>Storage Locaton Code <font color="#FF0000"> * </font></td>
                 <td>:</td>
                 <td>
                   <input name="sloc_code" type="text" id="sloc_code" size="20" maxlength="8" readonly value="<?php echo h($row[0]); ?>" /></td>
               </tr>
               <tr>
                 <td>Description <font color="#FF0000"> * </font></td>
                 <td>:</td>
                 <td>
                   <input name="sloc_desc" type="text" class="span11" id="sloc_desc" size="55" maxlength="100" value="<?php echo h($row[1]); ?>" />
               </td>
               </tr>
               <tr>
                 <td>&nbsp;</td>
