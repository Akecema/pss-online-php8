...
/**
 * admin/storage_QC_edit.php
 * Part of: Admin module
 * Filename suggests: storage QC edit
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, storage2_tbl, record.
...
$queryu = "SELECT * FROM storage2_tbl WHERE qc_sloc_code = ? "; $queryu_args = [$cd];
...
if (isset($_POST['submit']))
{
	...
	{ 
		$pd_dc = escape_data($_POST['qc_sloc_desc']);
	}
	...
	{     	
		...
		$query_search = "SELECT * FROM storage2_tbl WHERE qc_sloc_code = ?"; $query_search_args = [$cd];
		...
		$query_upd = "UPDATE storage2_tbl SET qc_sloc_desc = ? WHERE qc_sloc_code = ? "; $query_upd_args = [$_POST['qc_sloc_desc'], $cd]; 
