...
/**
 * admin/storage_QC_list2.php
 * Part of: Admin module
 * Filename suggests: storage QC list2
 *
...
$query8 = "SELECT COUNT(*) FROM storage2_tbl WHERE (qc_sloc_code LIKE ?) or (qc_sloc_desc LIKE ?)"; $query8_args = ['%' . $_GET["txtKeyword"] . '%', '%' . $_GET["txtKeyword"] . '%'];
...
$query = "SELECT * FROM storage2_tbl  WHERE (qc_sloc_code LIKE ?) or (qc_sloc_desc LIKE ?) ORDER BY qc_sloc_code ASC"; $query_args = ['%' . $_GET["txtKeyword"] . '%', '%' . $_GET["txtKeyword"] . '%'];
