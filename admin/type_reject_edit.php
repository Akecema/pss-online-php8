...
$queryu = "SELECT * FROM type_reject_detail WHERE id_type = ?"; $queryu_args = [$id_type];
...
{     	
		  	  $query_search = "SELECT * FROM type_reject_detail WHERE id_type = ?"; $query_search_args = [$id_type];
		...
		$query_upd = "UPDATE type_reject_detail SET type_desc = ?, status_type = ? WHERE id_type = ?"; $query_upd_args = [$type_desc, $status_type, $id_type]; 
...
{ echo '<div class="msg msg-error"><font color="red" class ="error_entry">', $message, '</font></div>';
 ...
 ?> 
          ...
          <!-- End Box Head -->
          <form name="form1" method="post" action="type_reject_edit.php?id_type=<?php echo h($id_type); ?>" >
              <table class="table table-bordered">
               <tr>
                 <td>ID Reason *</td>
                 <td>:</td>
                 <td>
                   <input name="id_type" type="text" id="id_type" size="20" maxlength="8" readonly value="<?php echo h($row[0]); ?>" /></td>
               </tr>
               <tr>
                 <td>Reason Reject Desc *</td>
                 <td>:</td>
                 <td>
                   <input name="type_desc" type="text" class="span11" id="type_desc" size="55" maxlength="100" value="<?php echo h($row[1]); ?>" />
               </td>
               </tr>
                        <tr>
                 <td>Status *</td>
