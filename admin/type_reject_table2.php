...
/**
 * admin/type_reject_table2.php
 * Part of: Admin module
 * Filename suggests: type reject table2
 *
...
<div id="content">
 ...
 $query8 = "SELECT COUNT(*) FROM type_reject_detail WHERE (id_type LIKE ?) or (type_desc LIKE ?)"; $query8_args = ['%' . $_GET["txtKeyword"] . '%', '%' . $_GET["txtKeyword"] . '%'];
...
$query = "SELECT * FROM type_reject_detail WHERE (id_type LIKE ?) or (type_desc LIKE ?) ORDER BY id_type ASC"; $query_args = ['%' . $_GET["txtKeyword"] . '%', '%' . $_GET["txtKeyword"] . '%'];
