...
{  
		  	  ...
		  	  $query_search = "SELECT * FROM user_detail WHERE user_no = ?"; $query_search_args = [$user_no];
			  ...
			  //echo $num_search; 
				...
				$query_upd = "UPDATE user_detail SET status_failed = 'N', date_update= NOW(), user_update =?, date_failed = '' WHERE user_no = ?"; $query_upd_args = [$username, $user_no];
			...
			$query_failed_del = "DELETE FROM failed_login WHERE staff_ID = ?"; $query_failed_del_args = [$row_search["staff_ID"]];
		    $result_failed_del = db_query_bind($dbc, $query_failed_del, $query_failed_del_args); 			
...
{ echo '<div class="msg msg-error"><font color="red" class ="error_entry">', $message, '</font></div>';
 ...
 ?> 
          ...
          <form name="form1" method="post" action="unlock_pass_account.php?user_no=<?php echo h($user_no); ?>" >
