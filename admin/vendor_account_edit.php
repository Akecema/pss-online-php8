...
$queryu = "SELECT * FROM vendor_detail WHERE vendor_code = ?"; $queryu_args = [$vendor_code];
...
{ $status_subcont = FALSE;
 ...
 {  
 	     	  ...
 	     	  $query_search = "SELECT * FROM vendor_detail WHERE vendor_code = ?"; $query_search_args = [$vendor_code];
		...
		$query_upd = "UPDATE vendor_detail SET vendor_name = '".strtoupper($vendor_name)."', add_no1 = ?, add_no2 = ?, search_term = '".strtoupper($search_term)."', post_code = ?, post_city = ?, post_region = ?, post_country = ?, tphone = ?, fax_no = ?, payment_method = ?, term_payment = ?, status_acc = ?, status_subcont = ?, user_update = ?, date_update = NOW() WHERE vendor_code = ?"; $query_upd_args = [$add_no1, $add_no2, $post_code, $post_city, $post_region, $post_country, $tphone, $fax_no, $payment_method, $term_payment, $status_acc, $status_subcont, $username, $vendor_code]; 
...
{ echo '<div class="msg msg-error"><font color="red" class ="error_entry">', $message, '</font></div>';
 ...
 ?> 
          ...
          <!-- End Box Head -->
          <form name="form1" method="post" action="vendor_account_edit.php?vendor_code=<?php echo h($vendor_code); ?>" >
