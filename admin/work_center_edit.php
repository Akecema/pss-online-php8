...
$queryu = "SELECT * FROM work_center_detail WHERE id_work = ?"; $queryu_args = [$id_work];
...
{     	
		  	  $query_search = "SELECT * FROM work_center_detail WHERE id_work = ?"; $query_search_args = [$id_work];
		...
		$query_upd = "UPDATE work_center_detail SET wc_desc = ?, cost_center = ?, cc_desc = ?, plant_code = ?, id_factory = ? WHERE id_work = ?"; $query_upd_args = [$wc_desc, $cost_center, $cc_desc, $plant_code, $id_factory, $id_work]; 
...
{ echo '<div class="msg msg-error"><font color="red" class ="error_entry">', $message, '</font></div>';
 ...
 ?> 
          ...
          <!-- End Box Head -->
          <form name="form1" method="post" action="work_center_edit.php?id_work=<?php echo h($id_work); ?>" >
         ...
         <input name="plant_code" type="text"  class="span11" id="plant_code" size="20" maxlength="20" value="<?php echo h($row[1]); ?>" /></td>
           ...
           <input type="hidden" name="id_work" id="id_work" value="<?php echo h($id_work); ?>">
