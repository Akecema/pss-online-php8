...
while ($r2 = mysql_fetch_array($result))
{
  ...
  $result_delete_request =mysql_query($sql_delete_request);
 
  $sql_delete_scanning = "DELETE FROM scan_detail WHERE id_scan = '".$r2["id_scan"]."'";
   $result_delete_scanning =mysql_query($sql_delete_scanning);
...
$result_c = mysql_query($sql_c) or trigger_error("SQL", E_USER_ERROR);
$r_c = mysql_num_rows($result_c);

//$numrows_c = $r_c;
//echo $numrows_c;
//echo "<br>";
while ($r2_c = mysql_fetch_array($result_c))
{

  $sql_delete_consumable = "DELETE FROM consumable_request WHERE id_req_con = '".$r2_c["id_req_con"]."'";
  $result_delete_consumable =mysql_query($sql_delete_consumable);
...
$result_wip = mysql_query($sql_wip) or trigger_error("SQL", E_USER_ERROR);
$r_wip = mysql_num_rows($result_wip);

$numrows = $r_wip;
//echo $numrows;
//echo "<br>";
while ($r2_wip = mysql_fetch_array($result_wip))
{

  $sql_delete_request_wip = "DELETE FROM wip_request WHERE id_req_wip = '".$r2_wip["id_req_wip"]."'";
  $result_delete_request_wip =mysql_query($sql_delete_request_wip);
 
  $sql_delete_scanning_wip = "DELETE FROM scan_detail_wip WHERE id_scan = '".$r2_wip["id_scan_wip"]."'";
  $result_delete_scanning_wip =mysql_query($sql_delete_scanning);
...
$result_pps = mysql_query($sql_pps) or trigger_error("SQL", E_USER_ERROR);
$r_pps = mysql_num_rows($result_pps);

$numrows2 = $r_pps;
//echo $numrows;
//echo "<br>";
while ($r2_pps = mysql_fetch_array($result_pps))
{
