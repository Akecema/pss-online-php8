 ...
 */
$sql = "SELECT * FROM material_request AS MR, scan_detail AS SD, user_detail AS UD WHERE MR.id_scan = SD.id_scan AND MR.user_create = UD.user_no AND MR.status_request = 'N' AND (MR.status != 'Close' AND MR.status != 'Cancel') AND UD.username = '".db_esc($dbc, $username)."'";
...
while ($r2 = mysqli_fetch_array($result))
{
  ...
  $sql_delete_request = "DELETE FROM material_request WHERE id_req = '".db_esc($dbc, $r2["id_req"])."'";
  $result_delete_request =mysqli_query($dbc, $sql_delete_request);
 
  $sql_delete_scanning = "DELETE FROM scan_detail WHERE id_scan = '".db_esc($dbc, $r2["id_scan"])."'";
...
$sql_c = "SELECT * FROM consumable_request AS CR, user_detail AS UD WHERE CR.user_create = UD.user_no AND CR.status_request = 'N' AND (CR.status != 'Close' AND CR.status != 'Cancel') AND UD.username = '".db_esc($dbc, $username)."'";
...
while ($r2_c = mysqli_fetch_array($result_c))
{
  ...
  $sql_delete_consumable = "DELETE FROM consumable_request WHERE id_req_con = '".db_esc($dbc, $r2_c["id_req_con"])."'";
...
$sql_wip = "SELECT * FROM wip_request AS WR, scan_detail_wip AS WD, user_detail AS UD WHERE WR.id_scan_wip = WD.id_scan AND WR.user_create = UD.user_no AND WR.status_request = 'N' AND (WR.status != 'Close' AND WR.status != 'Cancel') AND UD.username = '".db_esc($dbc, $username)."'";
...
while ($r2_wip = mysqli_fetch_array($result_wip))
{
  ...
  $sql_delete_request_wip = "DELETE FROM wip_request WHERE id_req_wip = '".db_esc($dbc, $r2_wip["id_req_wip"])."'";
...
$sql_pps = "SELECT * FROM pps_detail AS PD, user_detail AS UD2 WHERE PD.user_create = UD2.username AND PD.status_pps = 'New' AND PD.plan_no = '' AND UD2.username = '".db_esc($dbc, $username)."'";
...
while ($r2_pps = mysqli_fetch_array($result_pps))
{
 ...
 /* $sql_delete_request_pps = "UPDATE pps_detail SET status_pps = 'Delete' WHERE id = '".$r2_pps["id"]."'";
  ...
  $sql_delete_request_pps = "DELETE FROM pps_detail WHERE id = '".db_esc($dbc, $r2_pps["id"])."'";
  $result_delete_request_pps =mysqli_query($dbc, $sql_delete_request_pps);
