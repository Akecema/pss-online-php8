			  ...
			  $newpass = escape_data($_POST['newpass']);
				...
				$message .= '<p>Your new password did not match the confirmed new password!</p>';
				 ...
				 $pass = md5($password); // legacy-format fallback for the old-password check below
				  ...
				  $query = "SELECT * FROM user_detail WHERE username =?"; $query_args = [$user];
		          ...
		          $query2 = "UPDATE user_detail set password = ? where username=?"; $query2_args = [$newpass, $row["username"]];
      ...
      <form action="<?php echo h($_SERVER['PHP_SELF']); ?>"  method="post">
        ...
        <table width="99%" border="0" class="table table-striped table-hover table-bordered">
          <tr> 
            ...
            <td><input type="text" name="username1" size="30" class="leave" id="username1"  readonly value="<?php echo h($username); ?>" placeholder="Enter your username"></td>
          </tr>
          <tr> 
