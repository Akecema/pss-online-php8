...
// check for existence of that username
    if($user_name && $email) { 
		   ...
		   $query = "SELECT * FROM user_detail WHERE username = ? and user_email= ?"; $query_args = [$user_name, $email];
		   ...
		   $num = mysqli_num_rows($result);
				  ...
				  if($num == 1) {
				    $row = mysqli_fetch_array($result);
					 ...
					 $p = bin2hex(random_bytes(5)); // 10 hex characters from the OS CSPRNG (was md5(uniqid(rand())), which is guessable)
					 $p2 = password_hash($p, PASSWORD_DEFAULT);
					 	
				  $query2 = "UPDATE user_detail set password = ?, user_update = ?, date_update = NOW() WHERE username = ?"; $query2_args = [$p2, $row["username"], $row["username"]];
				  $result2 = db_query_bind($dbc, $query2, $query2_args) or die(db_fail($dbc));
				  
				$query_login = "UPDATE login_detail SET password = ?, user_update = ?, date_update = NOW() WHERE username = ?"; $query_login_args = [$p2, $row["username"], $row["username"]];
				$result_login = db_query_bind($dbc, $query_login, $query_login_args) or die(db_fail($dbc));
