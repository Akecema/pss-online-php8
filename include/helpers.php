 ...
 *
 * Statements are cached by SQL text and not closed between calls. Legacy pages run
 * the same query once per row of a loop; preparing, executing and closing it every
 * time cost about twice the old mysqli_query() (a 37 MB report went from 72 s to 148 s).
 * Leaving a write statement open also keeps mysqli_affected_rows($dbc) intact, which
 * legacy pages read after the call (closing a statement resets it to -1).
 *
...
{
    static $cache = [];
    $key = spl_object_id($dbc) . ':' . $sql;
    if (!isset($cache[$key])) {
        if (count($cache) >= 200) { // stay far below the server's max_prepared_stmt_count
            foreach ($cache as $old) {
                mysqli_stmt_close($old);
            }
            $cache = [];
        }
        $prepared = mysqli_prepare($dbc, $sql);
        if ($prepared === false) {
            return false;
        }
        $cache[$key] = $prepared;
