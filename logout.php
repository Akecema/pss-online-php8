...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" height="500">
  ...
  <tr> 
    ...
    $query = "SELECT * FROM user_detail WHERE username=?"; $query_args = [$username];
	$result = db_query_bind($dbc, $query, $query_args); 
	$db_rst = mysqli_fetch_array($result);
	
	if (db_query_bind($dbc, $query, $query_args)){
...
<tr> 
  ...
  <tr>
   ...
   //-------------------upldate last login in table user_detail
	$query_upd = "UPDATE user_detail SET last_login = NOW() WHERE user_no = ?"; $query_upd_args = [$db_rst["user_no"]];
	$result_upd = db_query_bind($dbc, $query_upd, $query_upd_args); 
	
	//-------------------upldate last login in table user_detail
	$query_upd2 = "UPDATE login_detail SET last_login = NOW() WHERE staff_ID = ?"; $query_upd2_args = [$db_rst["staff_ID"]];
