...
/**
 * planning/close_plan_order_list.php
 * Part of: Planning module
 * Filename suggests: close plan order list
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, factory_detail, pps_detail, pps_detail_close, level_detail.
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_planning.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a><a href="#" class="current">Closed Planned Order</a> </div>
  <h1>Closed Planned Order</h1>
</div>
...
<?php
        if(isset($_POST["Submit2"]))
        {
            ...
            echo "<script>";
echo "window.location='close_plan_order_list2.php?date1=$dateF&&date2=$dateT&&factory=$factory&&work_center=$work_center&&plan_no=$plan_no&&shift_ops=$shift_ops'";
            echo "</script>";
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail_close WHERE status_pps = ?"; $query8_args = [$rst_sta13["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R FROM pps_detail_close WHERE status_pps = ? order by plan_no ASC"; $query_args = [$rst_sta13["status_desc"]];
