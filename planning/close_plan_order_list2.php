...
/**
 * planning/close_plan_order_list2.php
 * Part of: Planning module
 * Filename suggests: close plan order list2
 *
 * Behavior: requires an active login session ($_SESSION['username']); reads parameters from the query string ($_GET).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, factory_detail, work_center_detail, pps_detail, pps_detail_close, level_detail.
...
<?php
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail_close AS MR WHERE MR.status_pps = ? AND MR.status = 'Y'".$where_sql; $query8_args = [$rst_sta13["status_desc"], ...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM pps_detail_close AS MR WHERE MR.status_pps = ? AND MR.status = 'Y' ".$where_sql." order by MR.plan_no ASC"; $query_args = [$rst_sta13["status_desc"], ...$where_args];
