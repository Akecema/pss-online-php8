...
$query_mat_req = "SELECT COUNT(DISTINCT plan_no) AS cnt FROM pps_detail_transaction WHERE status_pps = ? AND status = 'Y' AND qty_actual != ''"; $query_mat_req_args = [$rst_sta7["status_desc"]];
$rs_mat_req = db_query_bind($dbc, $query_mat_req, $query_mat_req_args);   //run the query.
$num_mat_req = mysqli_fetch_assoc($rs_mat_req)['cnt'];   //how many material are there?

//In Progress Plan Order request
$query_con_req = "SELECT COUNT(*) AS cnt FROM pps_detail WHERE status_pps = ?"; $query_con_req_args = [$rst_sta2["status_desc"]];
...
$query_plan_req = "SELECT COUNT(*) AS cnt FROM pps_detail WHERE status_pps = ?"; $query_plan_req_args = [$rst_sta18["status_desc"]];
  ...
  <div class="container-fluid">
    <div class="quick-actions_homepage">
       <ul class="quick-actions">
        <li class="bg_lb"> <a href="inprogress_plan_order_list.php"> <i class="icon-retweet"></i> <span class="label label-important"><?php echo h($num_mat_req);   ?></span> In Progress Planning No.</a> </li>
        <li class="bg_ls"> <a href="release_plan_order_list.php"> <i class="icon-list-ul"></i><span class="label label-important"><?php echo h($num_con_req);   ?></span> Released PPS</a> </li>
         <li class="bg_lo"> <a href="technical_complete_tran.php"> <i class="icon-lock"></i><span class="label label-important"><?php echo h($num_plan_req);  ?></span> Pending PPS Close</a> </li>
        </ul>
    </div>
<!--End-Action boxes-->    
	...
	$query_mat_prog_close = "SELECT COUNT(*) AS cnt FROM pps_detail_transaction WHERE status_pps = ? AND status = 'Y' AND qty_actual != ''"; $query_mat_prog_close_args = [$rst_sta7["status_desc"]];
	...
	$query_mat_prog_cancel = "SELECT COUNT(*) AS cnt FROM pps_detail WHERE status_pps = ?"; $query_mat_prog_cancel_args = [$rst_sta13["status_desc"]];
	$rs_mat_prog_cancel = db_query_bind($dbc, $query_mat_prog_cancel, $query_mat_prog_cancel_args);
	...
	$percent_cancel = number_format($percent_cancel, 2);
        ...
        <div class="widget-box">
          ...
          <div class="widget-content">
            <ul class="unstyled">
              <li> <span class="icon24 icomoon-icon-arrow-up-2 green"></span>Released <span class="pull-right strong"><?php echo h($num_mat_prog_open);   ?></span>
                <div class="progress progress-striped ">
                  <div style="width: <?php  echo  h($percent_open);  ?>%;" class="bar"></div>
                </div>
              </li>
              <li> <span class="icon24 icomoon-icon-arrow-up-2 green"></span>InProgress<span class="pull-right strong"><?php echo h($num_mat_prog_close);   ?></span>
                <div class="progress progress-success progress-striped ">
                  <div style="width: <?php  echo  h($percent_close);  ?>%;" class="bar"></div>
                </div>
              </li>
              <li> <span class="icon24 icomoon-icon-arrow-up-2 green"></span>Closed <span class="pull-right strong"><?php echo h($num_mat_prog_cancel);   ?></span>
