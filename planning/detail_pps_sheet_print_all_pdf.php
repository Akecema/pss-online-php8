...
$query_convert = "SELECT * FROM `work_center_detail` as SR WHERE SR.id_work = ?"; $query_convert_args = [$work_center];
$result_convert = db_query_bind($dbc, $query_convert, $query_convert_args);
$row_convert = mysqli_fetch_array($result_convert);

$query_convert2 = "SELECT * FROM `ftp_pps` WHERE file_name = ?"; $query_convert2_args = [$name_file];
...
if ($dateT == "0000-00-00") {
   ...
   $wheresql_01 = " AND (MR.date_plan <= '$dateT')"; $wheresql_01_args = [];
}
//2. DateF 
if ($dateF  == "0000-00-00") {
   $wheresql_02 = ""; $wheresql_02_args = [];
} else {
   $wheresql_02 = " AND (MR.date_plan >= '$dateF')"; $wheresql_02_args = [];
}
//3. Factory 
if ($factory == "NULL") {
...
if ($status == "NULL") {
   ...
   $wheresql_07 = " AND MR.status_pps = ?"; $wheresql_07_args = [$status];
}
// 8. File name
if ($name_file == "NULL") {
   $wheresql_08 = ""; $wheresql_08_args = [];
} else {
   $wheresql_08 = " AND MR.upload_id = ?"; $wheresql_08_args = [$row_convert2["upload_id"]];
}
...
$queryu = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') AS T, DATE_FORMAT(MR.date_upload,'%d-%m-%Y %H:%i:%s') AS K FROM pps_detail AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND (MR.status_pps != ? OR MR.status_pps != ?) AND MR.status_pps != ?" . $where_sql . " GROUP BY work_center"; $queryu_args = [$rst_sta4["status_desc"], $rst_sta16["status_desc"], $rst_sta["status_desc"], ...$where_args];
...
while ($db_rs = mysqli_fetch_array($rs)) {
   ...
   $query_by_group = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') AS T, DATE_FORMAT(MR.date_upload,'%d-%m-%Y %H:%i:%s') AS K from pps_detail AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.work_center = ? " . $where_sql . " "; $query_by_group_args = [$db_rs["work_center"], ...$where_args];
