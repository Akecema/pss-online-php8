...
/**
 * planning/detail_pps_sheet_print_all_pdf2.php
 * Part of: Planning module
 * Filename suggests: detail pps sheet print all pdf2
 *
...
$bulan_text = "";
 $upload_id = $_GET["upload_id"];
   ...
   $query_ftp_ppsA = "SELECT * FROM pps_detail WHERE upload_id = ?"; $query_ftp_ppsA_args = [$upload_id];
   $result_ftp_ppsA = db_query_bind($dbc, $query_ftp_ppsA, $query_ftp_ppsA_args);
   $data_ftp_ppsA = mysqli_fetch_array($result_ftp_ppsA);
   
 
$query_convert = "SELECT * FROM `work_center_detail` as SR WHERE SR.id_work = ?"; $query_convert_args = [$data_ftp_ppsA["work_center"]];
$result_convert = db_query_bind($dbc, $query_convert, $query_convert_args);
$row_convert = mysqli_fetch_array($result_convert);

$query_convert2 = "SELECT * FROM `ftp_pps` WHERE upload_id = ?"; $query_convert2_args = [$data_ftp_ppsA["upload_id"]];
...
// TCPDF 6.10+ only runs <tcpdf> tags from HTML when they are enabled and their data is signed (fix for an
// object-injection hole in the old serialised-parameters scheme). Only the barcode call is allowed.
$prevErrorLevel = error_reporting(E_ALL & ~E_WARNING); // TCPDF's config file defines these two again, unguarded
...
$queryu = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') AS T, DATE_FORMAT(MR.date_upload,'%d-%m-%Y %H:%i:%s') AS K FROM pps_detail AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.upload_id = ? GROUP BY work_center"; $queryu_args = [$upload_id];
$rs = db_query_bind($dbc, $queryu, $queryu_args);
...
while ($db_rs = mysqli_fetch_array($rs)) {
   ...
   $query_by_group = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') AS T, DATE_FORMAT(MR.date_upload,'%d-%m-%Y %H:%i:%s') AS K from pps_detail AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.upload_id = ? AND MR.work_center = ? "; $query_by_group_args = [$upload_id, $db_rs["work_center"]];
   ...
   while ($row = mysqli_fetch_array($result_by_group)) {
      ...
      //---------get material header---------
      $query_mat_h = "SELECT * FROM mat_master_header AS HD, mat_master_detail AS AD WHERE HD.material_no = AD.material AND HD.material_no = ?"; $query_mat_h_args = [$row['material_no']];
      $result_mat_h = db_query_bind($dbc, $query_mat_h, $query_mat_h_args);
      $data_mat_h = mysqli_fetch_array($result_mat_h);

      //---------get material detail---------
      $query_mat_d = "SELECT * FROM mat_master_detail WHERE (material = ? OR bill_component = ?)"; $query_mat_d_args = [$row['material_no'], $row['material_no']];
      ...
      $params = $pdf->serializeTCPDFtag('write2DBarcode', 
         array($bar_text, 'PDF417', '', '', 25, '', array(), 'N') // style must be an array
      );
      ...
      '<tr nobr="true">
         ...
         <td width="100" class="text-center"><tcpdf data="'.$params.'" /></td>
