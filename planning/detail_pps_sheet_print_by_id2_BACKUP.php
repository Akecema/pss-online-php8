...
/**
 * planning/detail_pps_sheet_print_by_id2_BACKUP.php
 * Part of: Planning module
 * Filename suggests: detail pps sheet print by id2 BACKUP
 *
...
$queryu = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') AS T, DATE_FORMAT(date_upload,'%d-%m-%Y %H:%i:%s') AS K from pps_detail WHERE upload_id = ? GROUP BY work_center"; $queryu_args = [$id];
...
while ($db_rs = mysqli_fetch_array($rs))
{
 ...
 $query_ftp_pps = "SELECT * FROM ftp_pps WHERE upload_id = ?"; $query_ftp_pps_args = [$id];
...
<div class="page">
<br>
<table width="1053" border="0" cellpadding="2" cellspacing="2">
  <tr>
  ...
  <!--  <div class="page"> -->
	  ...
	  $query_by_group = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') AS T, DATE_FORMAT(MR.date_upload,'%d-%m-%Y %H:%i:%s') AS K from pps_detail AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.upload_id = ? AND MR.work_center = ?"; $query_by_group_args = [$id, $db_rs["work_center"]];
