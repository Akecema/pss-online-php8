...
$queryu = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') AS T, DATE_FORMAT(MR.date_upload,'%d-%m-%Y %H:%i:%s') AS K FROM pps_detail AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND (MR.status_pps != ? AND MR.status_pps != ?) AND MR.status_pps != ?".$where_sql. " GROUP BY work_center"; $queryu_args = [$rst_sta4["status_desc"], $rst_sta16["status_desc"], $rst_sta["status_desc"], ...$where_args];
