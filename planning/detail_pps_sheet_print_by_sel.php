...
/**
 * planning/detail_pps_sheet_print_by_sel.php
 * Part of: Planning module
 * Filename suggests: detail pps sheet print by sel
 *
...
*/
 ...
 //convert 
			...
			$query_convert = "SELECT * FROM work_center_detail WHERE id_work = ?"; $query_convert_args = [$_GET["work_centerA"]];
...
$queryu = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') AS T, DATE_FORMAT(date_upload,'%d-%m-%Y %H:%i:%s') AS K FROM pps_detail WHERE date_plan = ? AND work_center = ? AND (status_pps != ? OR status_pps != ?) AND status_pps != ?  GROUP BY work_center"; $queryu_args = [$plan_date, $work_center, $rst_sta4["status_desc"], $rst_sta16["status_desc"], $rst_sta["status_desc"]];
...
<table width="1050" border="0" cellpadding="2" cellspacing="2">
  ...
  <!--  <div class="page"> -->
	  ...
	  $query_by_group = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') AS T, DATE_FORMAT(date_upload,'%d-%m-%Y %H:%i:%s') AS K from pps_detail WHERE date_plan = ? AND work_center = ? "; $query_by_group_args = [$plan_date, $db_rs["work_center"]];
