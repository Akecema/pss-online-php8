...
/**
 * planning/detail_pps_sheet_print_by_sel_pdf.php
 * Part of: Planning module
 * Filename suggests: detail pps sheet print by sel pdf
 *
 ...
 * Database tables referenced: sys_setup_maintain, request_status, work_center_detail, pps_detail, ftp_pps, mat_master_header, mat_master_detail.
...
$queryu = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') AS T, DATE_FORMAT(MR.date_upload,'%d-%m-%Y %H:%i:%s') AS K FROM pps_detail AS MR WHERE  MR.date_plan = ? AND MR.work_center = ? AND (MR.status_pps != ? OR MR.status_pps != ?) AND MR.status_pps != ? GROUP BY MR.work_center"; $queryu_args = [$plan_date, $work_center, $rst_sta4["status_desc"], $rst_sta16["status_desc"], $rst_sta["status_desc"]];
...
while ($db_rs = mysqli_fetch_array($rs)) {
   ...
   $query_by_group = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') AS T, DATE_FORMAT(MR.date_upload,'%d-%m-%Y %H:%i:%s') AS K from pps_detail AS MR WHERE MR.date_plan = ? AND MR.work_center = ?"; $query_by_group_args = [$plan_date, $db_rs["work_center"]];
