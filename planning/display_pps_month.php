...
$queryu = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as T from pps_detail as MR, ftp_pps as SD WHERE MR.upload_id = SD.upload_id AND MR.upload_id = ?"; $queryu_args = [$upload_id];
...
$message = NULL; // create an empty new variable.	
 ...
 // echo $size2;
	...
	foreach($id as $key => $n ) {
		 ...
		 $query_update = "UPDATE pps_detail SET plan_no = ?, user_update = ?, date_update = NOW(), user_posting = ?, date_posting = NOW() WHERE id = ?"; $query_update_args = [$plan_no[$j], $username, $username, $n];
...
require_once('../include/config.php');   //connect to the db.
         ...
         $query_del_ftp = "DELETE FROM ftp_pps WHERE upload_id = ?"; $query_del_ftp_args = [$upload_id];
	     $result_del_ftp = db_query_bind($dbc, $query_del_ftp, $query_del_ftp_args); 

         $sqlDel = "DELETE FROM pps_detail WHERE upload_id = ?"; $sqlDel_args = [$upload_id];
         $rsDel = db_query_bind($dbc, $sqlDel, $sqlDel_args);
	...
	?>  
      <!-- Content -->             
  <form name="form1" action="display_pps_month.php?upload_id=<?php echo h($upload_id); ?>" method="post" class="form-horizontal">
   ...
   while ($row = mysqli_fetch_array($rs))
   {
	...
	?>
               <tr> 
                 ...
                 <input type="hidden" name="plan_no[]" value="<?php echo h($ref); ?>">
