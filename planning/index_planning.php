...
$query_mat_req = "SELECT * FROM pps_detail_transaction WHERE status_pps = ? AND status = 'Y' AND qty_actual != '' GROUP BY plan_no"; $query_mat_req_args = [$rst_sta7["status_desc"]];
...
$query_con_req = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R FROM pps_detail WHERE status_pps = ? order by plan_no ASC"; $query_con_req_args = [$rst_sta2["status_desc"]];
...
$query_plan_req = "SELECT * FROM pps_detail WHERE status_pps = ? ORDER BY date_plan DESC"; $query_plan_req_args = [$rst_sta18["status_desc"]];
