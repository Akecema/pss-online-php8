...
/**
 * planning/inprogress_plan_order_list.php
 * Part of: Planning module
 * Filename suggests: inprogress plan order list
 *
...
echo "window.location='inprogress_plan_order_list2.php?date1=$dateF&&date2=$dateT&&factory=$factory&&work_center=$work_center&&plan_no=$plan_no&&shift_ops=$shift_ops'";
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail_transaction WHERE status_pps = ? AND status = 'Y' AND qty_actual != ''"; $query8_args = [$rst_sta7["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction WHERE status_pps = ? AND status = 'Y' AND qty_actual != '' order by plan_no ASC"; $query_args = [$rst_sta7["status_desc"]];
