...
<tr>
    ...
    <td width="85%"> <div class="small-nav"></div></td> 
      ...
      <div class="widget-box">
          ...
          <div class="widget-content nopadding">
                ...
                <tr>
                <th>
                <input name="txtKeyword" type="text" id="txtKeyword" value="<?php echo h($strKeyword);?>">
   ...
   $qty_total_NG2 = 0.000;	
	 ...
	 //-----checking QA/QC entering output production
	   ...
	   $query_qqc2 = "SELECT * FROM qqc_transaction WHERE bflush_no = ?"; $query_qqc2_args = [$data_qqc["bflush_no"]];
	...
	}elseif($sta_out == "3")
	{
      ...
      ?>
                ...
                <td width="50"><font color="#0000CC"><?php echo h($row[3]); ?></font></td>
                ...
                <td width="40"><div align="center"><?php echo number_format($qty_total_NG); ?></div></td>
                  ...
                  <input name="id" type="hidden" value="<?php echo h($row["id"]); ?>">
          <?php 
...
<br>
<br>
<?php echo h($num_rows);?> Record : <?php echo h($num_pages);?> Page :
<?php
...
{   
	...
	echo " <a href='$_SERVER[SCRIPT_NAME]?Page=$prev_page&txtKeyword=$strKeyword&&date1=" . urlencode($dateF) . "&&date2=" . urlencode($dateT) . "&&factory=" . urlencode($factory) . "&&work_center=" . urlencode($work_center) . "&&plan_no=" . urlencode($plan_no) . "&&shift_ops=" . urlencode($shift_ops) . "' class='pagin'><< Back</a> ";
	echo "</div>";
}
...
for($i=1; $i<=$num_pages; $i++){
	...
	{   
	    echo "<div class='pagin'>";
		echo "<a href='$_SERVER[SCRIPT_NAME]?Page=$i&txtKeyword=$strKeyword&&date1=" . urlencode($dateF) . "&&date2=" . urlencode($dateT) . "&&factory=" . urlencode($factory) . "&&work_center=" . urlencode($work_center) . "&&plan_no=" . urlencode($plan_no) . "&&shift_ops=" . urlencode($shift_ops) . "' class='pagin'> $i </a> ";
		echo "</div>";
	}
	else
	{
...
{
	echo "<div class='pagin'>"; 
	echo " <a href ='$_SERVER[SCRIPT_NAME]?Page=$next_page&txtKeyword=$strKeyword&&date1=" . urlencode($dateF) . "&&date2=" . urlencode($dateT) . "&&factory=" . urlencode($factory) . "&&work_center=" . urlencode($work_center) . "&&plan_no=" . urlencode($plan_no) . "&&shift_ops=" . urlencode($shift_ops) . "' class='pagin'>Next>></a> ";
	echo "</div>";
}
