...
<div id="content-header">
  ...
  <div class="container-fluid">
        ...
        <!--<div class="widget-box">
		 ...
		 //1. DateF 
                if ($dateF  == "0000-00-00"){
                     ...
                     $wheresql_01 = " AND (date_plan >= ?)"; $wheresql_01_args = [$dateF];} 
					  
		 // 2. DateT
                if($dateT == "0000-00-00") {
                     $wheresql_02 = ""; $wheresql_02_args = []; }
                else {
                      $wheresql_02 = " AND (date_plan <= ?)"; $wheresql_02_args = [$dateT]; }
		 ...
		 //3. Factory 
		  ...
		  //7. plan category
                if ($plan_category == "NULL"){ 
                    ...
                    $wheresql_07 = " AND plan_category = ?"; $wheresql_07_args = [$plan_category]; } 	
			 			
				
				$where_sql =  $wheresql_01. $wheresql_02. $wheresql_03. $wheresql_04. $wheresql_06. $wheresql_07; $where_args = array_merge($wheresql_01_args, $wheresql_02_args, $wheresql_03_args, $wheresql_04_args, $wheresql_06_args, $wheresql_07_args);		
				
	//********** END CONDITION **************
			
			?>     
      
           <form name="form1" method="get" action="<?php echo h($_SERVER['PHP_SELF']); ?>">
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail WHERE status_pps = ? AND status = 'Y' ".$where_sql; $query8_args = [$rst_sta2["status_desc"], ...$where_args];
   $result8 = db_query_bind($dbc, $query8, $query8_args);
...
$query_all = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_create,'%d-%m-%Y') as R3 FROM pps_detail WHERE status_pps = ? AND status = 'Y' ".$where_sql." ORDER BY plan_no ASC"; $query_all_args = [$rst_sta2["status_desc"], ...$where_args];
$rs = db_query_bind($dbc, $query_all, $query_all_args);   //run the query.
...
<tr>
    ...
    <td width="85%"> <div class="small-nav"></div></td> 
      ...
      <div class="widget-box">
         ...
         <form name="myform" method="post" action="list_ftp_to-iposProc2.php?date1=<?php echo h($dateF); ?>&&date2=<?php echo h($dateT); ?>&&factory=<?php echo h($factory); ?>&&work_center=<?php echo h($work_center); ?>&&shift_ops=<?php echo h($shift_ops); ?>&&plan_category=<?php echo h($plan_category); ?>">
   ...
   while ($row = mysqli_fetch_array($rs))
   {
	...
	//---factory detail -------
	$query_fac = "SELECT * FROM factory_detail WHERE factory_desc2 = ?"; $query_fac_args = [$row["id_factory_pps"]];
	$result_fac = db_query_bind($dbc, $query_fac, $query_fac_args);
	$data_fac = mysqli_fetch_array($result_fac);	 
	 
	
	//-----material info -------
	$query_mat = "SELECT * FROM mat_master_header WHERE material_no = ? AND status_BOM = 'Y'"; $query_mat_args = [$row["material_no"]];
...
onClick="Check(document.myform.cancel)">  
	...
	//-----ftp info -------
	$query_ftp_sent = "SELECT * FROM ftp_pss_ipos WHERE plan_no = ? AND material_no = ?"; $query_ftp_sent_args = [$row["plan_no"], $row["material_no"]];
	$result_ftp_sent = db_query_bind($dbc, $query_ftp_sent, $query_ftp_sent_args);
	...
	//-----ftp info -------
	$query_ftp = "SELECT *, DATE_FORMAT(posting_date,'%d-%m-%Y') as TG  FROM ftp_pss_ipos WHERE plan_no = ? AND material_no = ?"; $query_ftp_args = [$row["plan_no"], $row["material_no"]];
	...
	//-----ftp info -------
	$query_ftp2 = "SELECT *, DATE_FORMAT(posting_time,'%H:%i:%s') as TG2  FROM ftp_pss_ipos WHERE plan_no = ? AND material_no = ? ORDER BY posting_time ASC"; $query_ftp2_args = [$row["plan_no"], $row["material_no"]];
	$result_ftp2 = db_query_bind($dbc, $query_ftp2, $query_ftp2_args);
 ...
 $sta_cntr = '';
  ...
  if(isset($_POST["cancel"])) 
  {
	...
	$string = "";
		...
		$query_info = "SELECT * FROM pps_detail WHERE id = ?"; $query_info_args = [$cancel[$i]];
	...
	$query_mat_info2 = "SELECT * FROM mat_master_header WHERE material_no = ? AND status_BOM = 'Y'"; $query_mat_info2_args = [$row_info["material_no"]];
	$result_mat_info2 = db_query_bind($dbc, $query_mat_info2, $query_mat_info2_args);
    ...
    $query_pps_closed = "INSERT INTO ftp_pss_ipos(id,file_chker,file_name,plan_no,id_pss,material_no,material_desc,qty_ftp,uom,work_center,id_factory,plant,date_plan,shift_day,status_ftp,posting_date,posting_time,user_create,date_create) VALUES('',?,?,?,?,?,?,?,?,?,?,?,?,?,'Y',NOW(),NOW(),?,?)"; $query_pps_closed_args = [$file_chker, $filen, $row_info['plan_no'], $row_info["id"], $row_info["material_no"], $data_mat_info2["material_desc"], $row_info["qty_plan"], $data_mat_info2["BUn"], $row_info["work_center"], $row_info["id_factory_pps"], $row_info["comp_code"], $row_info["date_plan"], $shift_dayF, $row_info["user_create"], $row_info["date_create"]];
	$rst_pps_closed = db_query_bind($dbc, $query_pps_closed, $query_pps_closed_args);
...
//-------  checking line x sama --------//
  ...
  $query_chk_line = "SELECT COUNT(DISTINCT work_center) AS GG FROM ftp_pss_ipos WHERE file_chker = ?"; $query_chk_line_args = [$file_chker];
  $rst_chk_line = db_query_bind($dbc, $query_chk_line, $query_chk_line_args);
 ...
 if($data_chk_line["GG"] > 1)
 {
		...
		//-----delete file date x ssama ------
		     ...
		     $query_del_tarikh = "DELETE FROM ftp_pss_ipos WHERE file_chker = ?"; $query_del_tarikh_args = [$file_chker];
			 $result_del_tarikh =  db_query_bind($dbc, $query_del_tarikh, $query_del_tarikh_args); 
...
//	 
$qry = db_query_bind($dbc, "SELECT *, DATE_FORMAT(date_plan,'%Y%m%d') AS R FROM ftp_pss_ipos WHERE file_name = ?", [$filen]);
$data = "";
$bil = 1;
