...
/**
 * planning/printing_pps_sheet_release.php
 * Part of: Planning module
 * Filename suggests: printing pps sheet release
 *
...
$queryu = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') AS T, DATE_FORMAT(date_upload,'%d-%m-%Y %H:%i:%s') AS K from pps_detail WHERE upload_id = ? GROUP BY work_center"; $queryu_args = [$upload_id];
$rs = db_query_bind($dbc, $queryu, $queryu_args);   //run the query.
 ...
 $query_ftp_pps = "SELECT * FROM ftp_pps WHERE upload_id = ?"; $query_ftp_pps_args = [$upload_id];
  ...
  <tr>
    ...
    <td><table class="table table-bordered" >
      ...
      <tr>
        ...
        <th height="28"><span class="style3"><?php echo h($db_rs["K"]); ?></span></th>
      </tr>
      <tr>
...
<table class="table table-bordered" style="page-break-inside:inherit">
	  ...
	  $query_by_group = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') AS T, DATE_FORMAT(date_upload,'%d-%m-%Y %H:%i:%s') AS K from pps_detail WHERE upload_id = ? AND work_center = ?"; $query_by_group_args = [$upload_id, $db_rs["work_center"]];
	...
	}elseif($row["shift_pps2"] != "")
	 {
	  ...
	  //---------get material header---------
        ...
        <tr>
          ...
          <td width="60" height="28"><?php  echo h($row["model_code"]); ?></td>
          <td width="384"><b><h5><?php  echo h($row["material_no"]); ?></h5></b><?php  echo h($data_mat_h["material_desc"]); ?></td>
          <td width="151" height="28"><div align="center"><?php echo h($row["plan_no"]); ?></div></td>
          <td width="93" height="28"><?php  echo h($row["T"]); ?></td>
...
<?php
	//------------update status = "Release"------------------//
	 ...
	 $query_upd = "UPDATE pps_detail SET status = 'Y', status_pps = 'Released' WHERE upload_id = ?"; $query_upd_args = [$upload_id];
	 $result_upd = db_query_bind($dbc, $query_upd, $query_upd_args);   //run the query.	
