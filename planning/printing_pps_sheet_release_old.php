...
/**
 * planning/printing_pps_sheet_release_old.php
 * Part of: Planning module
 * Filename suggests: printing pps sheet release old
 *
...
<table class="table table-bordered" style="page-break-inside:inherit">
	  ...
	  $query_by_group = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') AS T, DATE_FORMAT(date_upload,'%d-%m-%Y %H:%i:%s') AS K from pps_detail WHERE upload_id = '".db_esc($dbc, $upload_id)."' AND work_center = '".db_esc($dbc, $db_rs["work_center"])."'";
...
<?php
	...
	//------------update status = "Release"------------------//
	 ...
	 $query_upd = "UPDATE pps_detail SET status = 'Y', status_pps = 'Released' WHERE upload_id = '".db_esc($dbc, $upload_id)."'";
