...
<?php
		 ...
		 //3. Factory 
		  ...
		  //7. File name
                if ($name_file == "NULL" ){
                    ...
                    $wheresql_07 = " AND MR.upload_id = '".db_esc($dbc, $row_convert2["upload_id"])."'"; }    
	...
	$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R FROM pps_detail AS MR WHERE MR.status_pps = '".db_esc($dbc, $rst_sta2["status_desc"])."'".$where_sql." order by MR.plan_no ASC";
...
<tr>
   ...
   while ($row = mysqli_fetch_array($rs))
   {
	...
	$query_convert2 = "SELECT * FROM `ftp_pps` WHERE id_file = '".db_esc($dbc, $row['upload_id'])."' ";
	$result_convert2 = mysqli_query($dbc, $query_convert2); 
	$row_convert2 = mysqli_fetch_array($result_convert2);
