...
<?php
		 ...
		 //3. Factory 
		  ...
		  //7. File name
                if ($name_file == "NULL" ){
                    ...
                    $wheresql_07 = " AND MR.upload_id = ?"; $wheresql_07_args = [$row_convert2["upload_id"]]; }    
	...
	$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R FROM pps_detail AS MR WHERE MR.status_pps = ?".$where_sql." order by MR.plan_no ASC"; $query_args = [$rst_sta2["status_desc"], ...$where_args];
...
<tr>
   ...
   while ($row = mysqli_fetch_array($rs))
   {
	...
	$query_convert2 = "SELECT * FROM `ftp_pps` WHERE id_file = ? "; $query_convert2_args = [$row['upload_id']];
	$result_convert2 = db_query_bind($dbc, $query_convert2, $query_convert2_args); 
	$row_convert2 = mysqli_fetch_array($result_convert2);
