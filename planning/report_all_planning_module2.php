...
<?php
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail AS MR WHERE (MR.status_pps != '".$rst_sta4["status_desc"]."' AND MR.status_pps != '".$rst_sta16["status_desc"]."')".$where_sql;
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_posting,'%H:%i:%s') as T2 FROM pps_detail AS MR WHERE (MR.status_pps != '".$rst_sta4["status_desc"]."' AND MR.status_pps != '".$rst_sta16["status_desc"]."')".$where_sql. " GROUP BY MR.plan_no ORDER BY MR.plan_no ASC";
