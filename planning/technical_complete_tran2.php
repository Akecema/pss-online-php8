...
<div id="content-header">
		  ...
		  //6. Shift
		   ...
		   //8. status
                if ($status == "NULL"){ 
                    ...
                    $wheresql_08 = " AND status_pps = ?"; $wheresql_08_args = [$status]; } 			 			
 ...
 $query8 = "SELECT COUNT(*) FROM pps_detail WHERE (status_pps = ? OR status_pps = ?) AND status = 'Y' ".$where_sql; $query8_args = [$rst_sta2["status_desc"], $rst_sta18["status_desc"], ...$where_args];
...
$query_all = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_create,'%d-%m-%Y') as R3 FROM pps_detail WHERE (status_pps = ? OR status_pps = ?) AND status = 'Y' ".$where_sql." ORDER BY plan_no ASC"; $query_all_args = [$rst_sta2["status_desc"], $rst_sta18["status_desc"], ...$where_args];
...
// $cancel = $_POST["cancel"]; 
  ...
  if(isset($_POST["cancel"])) 
  {
	...
	$query_pps_closed2 = "UPDATE pps_detail_close SET remark_closed_plan = ?, status_pps = ?, user_closed = ?, date_closed = NOW() WHERE id = ?"; $query_pps_closed2_args = [$string[$i], $rst_sta13["status_desc"], $username, $cancel[$i]];
	$rst_pps_closed2 = db_query_bind($dbc, $query_pps_closed2, $query_pps_closed2_args);
	...
	$rst_pps_closed = db_query_bind($dbc, $query_pps_closed, $query_pps_closed_args);
		...
		$query_close2 = "UPDATE pps_detail_transaction SET status_pps = ? WHERE pps_id = ?"; $query_close2_args = [$rst_sta14["status_desc"], $cancel[$i]];
        $result_close2 = db_query_bind($dbc, $query_close2, $query_close2_args) or die(db_fail($dbc));
