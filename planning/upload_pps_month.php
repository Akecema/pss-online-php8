...
$message = NULL; // create an empty new variable.
 ...
 $plan_category = $_POST['plan_category'];
          ...
          //check file name duplicate-----
            ...
            $query_chk_attach4 = "SELECT * FROM ftp_pps WHERE file_name = ?"; $query_chk_attach4_args = [$_FILES["upload"]["name"]];
            $result_chk_attach4 = db_query_bind($dbc, $query_chk_attach4, $query_chk_attach4_args);   //run the query.
 ...
 {  	
  ...
  $date_plan = $_POST["date1"];
	   ...
	   //Add the record to the database
	    ...
	    $query = "INSERT INTO ftp_pps(upload_id, id_file, file_name, file_size, file_type, date_plan, user_upload, date_upload, user_update, date_update) VALUES('','',?,?,?,?,?,NOW(),'','')"; $query_args = [$_FILES['upload']['name'], $_FILES['upload']['size'], $_FILES['upload']['type'], $date_plan, $username];
	   $result = db_query_bind($dbc, $query, $query_args) or die(db_fail($dbc));   
	   ...
	   //create the filename
		 ...
		 $filename = $uid.'.'.upload_safe_ext($_FILES['upload']['name'], ['xls', 'xlsx']);
		    ...
		    $query_update2 = "UPDATE ftp_pps SET id_file = ? WHERE upload_id = ?"; $query_update2_args = [$uid, $uid];
...
?>
 ...
 <div class="widget-content nopadding">
          <form name="form1" enctype="multipart/form-data" action="<?php echo h($_SERVER['PHP_SELF']); ?>" method="post" class="form-horizontal">
 ...
 <input name="upload" type="file" class="textbox" value="<?php if(isset($_POST['upload'])) echo h($_POST['upload']); ?>"maxlength="200" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel" /> 
             ...
             <select name="plan_category" id="plan_category" class="span5">
			      ...
			      {
				   ?>
                     <option value="<?php echo h($row19["id_plan"]); ?>"> <?php echo h($row19["plan_category_desc"]); ?></option>
