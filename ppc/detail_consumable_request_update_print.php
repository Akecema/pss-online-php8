...
/**
 * ppc/detail_consumable_request_update_print.php
...
<p>
  ...
  <?php
    ...
    $query_update_print = "UPDATE consumable_request SET status_print = 'Y', status = 'Close', user_update = ?, date_update = NOW() WHERE temp_mrin = ?"; $query_update_print_args = [$data_u["user_no"], $temp_mrin];
	$result_update_print = db_query_bind($dbc, $query_update_print, $query_update_print_args);
	
	
 $query_tp = "SELECT *, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') AS R FROM consumable_request AS MR, consumable_detail AS SD WHERE MR.id_con = SD.id_con AND MR.temp_mrin = ? AND MR.status_print = 'Y'"; $query_tp_args = [$temp_mrin];
