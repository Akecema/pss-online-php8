...
/**
 * ppc/history_consumable_request2.php
 * Part of: PPC module (Production Planning & Control)
 * Filename suggests: history consumable request2
 *
 * Behavior: requires an active login session ($_SESSION['username']); reads parameters from the query string ($_GET).
 * Database tables referenced: user_detail, sys_setup_maintain, factory_detail, consumable_request_close, mat_master_header.
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_ppc.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="tip-bottom">Consumable Request</a> <a href="#" class="current">Consumable Request History</a> </div>
...
<?php
	...
	$query8 = "SELECT *, DATE_FORMAT(MR.date_require,'%d-%m-%Y') AS R FROM consumable_request_close AS MR, consumable_detail AS SD WHERE MR.id_con = SD.id_con AND MR.status_request = 'Y' AND MR.status = 'Close'".$where_sql." GROUP BY MR.temp_mrin"; $query8_args = [...$where_args];
   ...
   $pages->paginate();
	...
	$query = "SELECT *, DATE_FORMAT(MR.date_require,'%d-%m-%Y') AS R FROM consumable_request_close AS MR, consumable_detail AS SD WHERE MR.id_con = SD.id_con AND MR.status_request = 'Y' AND MR.status = 'Close'".$where_sql." GROUP BY MR.temp_mrin ORDER BY MR.date_posting ASC, MR.temp_mrin ASC "; $query_args = [...$where_args];
	$rs = db_query_bind($dbc, $query, $query_args);   //run the query.GROUP BY MR.temp_mrin
...
?>
      ...
      <div class="widget-box">
          <div class="widget-title"> <span class="icon"><i class="icon-th"></i></span>
            <h5>Display Consumable Request History</h5>
          </div>
 ...
 //	$row4_p = mysqli_fetch_array($result4_p); 
   ...
   <tr>
    <td width="143">&nbsp;
        <?php if($row2["status_view"] == "N") { echo "<b>";  echo h($row2["temp_mrin"])."</b>"; } else {  echo h($row2["temp_mrin"]);  } ?></td>
    ...
    <td width="60"><a value="Details" href="history_detail_consumable_request.php?mrin_no=<?php echo h($row2["temp_mrin"]); ?>&&TB_iframe=true&&height=400&&width=800" class="thickbox" target="_self"><img src="../img/icon_view.jpg" width="16" height="16" alt="View" />View</a></td>
    <td width="60"><a href="history_detail_consumable_request_printing.php?mrin_no=<?php echo h($row2["temp_mrin"]); ?>&&TB_iframe=true&&height=400&&width=800" class="thickbox" target="_self"><img src="../img/print.jpg" width="16" height="16" alt="Print" />Print</a> </td>
  </tr>
