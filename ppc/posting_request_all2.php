...
/**
 * ppc/posting_request_all2.php
 * Part of: PPC module (Production Planning & Control)
 * Filename suggests: posting request all2
 *
 * Behavior: requires an active login session ($_SESSION['username']); reads parameters from the query string ($_GET).
 * Database tables referenced: user_detail, sys_setup_maintain, factory_detail, work_center_detail, material_request, scan_detail, mat_master_header, post_detail_header, material_request_close.
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_ppc.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="tip-bottom">Material Request</a> <a href="#" class="current">Material Request</a> </div>
...
?></td>
	       ...
	       $query3 = "SELECT * FROM factory_detail GROUP BY factory_desc2 ORDER BY id_fac ASC";
				  ...
				  ?>
                      <option value="<?php echo h($row3[2]); ?>" <?php if($row3[2] == $_GET["factory"]) echo "selected"; ?>> 
					  <?php echo h($row3[1]); ?></option>
...
?>
  ...
  //--------------------------------------------------------------------------------------------------------------------------------------------  
	...
	$query_tp = "SELECT *,SUM(rquantity) as TOT FROM post_detail_header WHERE mrin_no = ? AND prod_order = ? AND mvt_type = 311 AND status_posting = 'New' GROUP BY material_no"; $query_tp_args = [$row2['temp_mrin'], $row2['prod_order']];
 ...
 $result_upd3 = db_query_bind($dbc, $query_upd3, $query_upd3_args); 
	   ...
	   //---------------------------------------------------------------------
        if($result_upd3)
		 ...
		 $query_upd5 = "SELECT * FROM material_request WHERE temp_mrin = ?"; $query_upd5_args = [$row2["temp_mrin"]];
		 $result_upd5 = db_query_bind($dbc, $query_upd5, $query_upd5_args) or trigger_error("SQL", E_USER_ERROR);
         ...
         $r4 = mysqli_num_rows($result_upd4);
		  ...
		  {	
		  	...
		  	$query_mm3 = "INSERT INTO material_request_close SELECT * FROM material_request WHERE temp_mrin = ?"; $query_mm3_args = [$row2["temp_mrin"]]; 
...
<!--      <tbody> -->  
  <tr>
	...
	<?php
     ...
     <td width="80"><?php echo h($row2["time_mrin"]); ?></td>
     <td width="110"><?php echo h($data_u["user_fullname"]); ?></td>
