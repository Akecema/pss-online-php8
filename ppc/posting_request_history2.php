...
$query = "SELECT *, DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R FROM material_request_close AS MR, scan_detail AS SD WHERE MR.id_scan = SD.id_scan AND MR.status_request = 'Y' AND  MR.status = 'Close' ".$where_sql." GROUP BY MR.temp_mrin ORDER BY MR.date_posting DESC, MR.temp_mrin DESC";
