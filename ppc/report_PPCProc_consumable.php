...
$query = "SELECT *,DATE_FORMAT(MR.date_require,'%d-%m-%Y') AS R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') AS R2 FROM consumable_request AS MR WHERE MR.status_request = 'Y' AND MR.status != 'Cancel'".$where_sql." GROUP BY MR.temp_mrin ORDER BY MR.date_posting DESC,MR.temp_mrin ASC "; $query_args = [...$where_args];
