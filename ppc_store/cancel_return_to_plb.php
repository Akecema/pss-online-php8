...
$queryu = "SELECT * FROM ret_plb_detail WHERE doc_tp = ?"; $queryu_args = [$doc_tp];
$rs = db_query_bind($dbc, $queryu, $queryu_args);   //run the query.


$query_2 = "SELECT *, DATE_FORMAT(MR.posting_date,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R2 FROM ret_plb_detail as MR, scan_tp_plb as SD WHERE MR.id_scan_tp = SD.id_scan_tp AND MR.doc_tp = ?"; $query_2_args = [$doc_tp];
...
$message = NULL; // create an empty new variable.
 ...
 //-----select info tp_plb_detail -----------         
  $query_info = "SELECT * FROM ret_plb_detail WHERE doc_tp = ? AND status_tp = ?"; $query_info_args = [$doc_tp, $rst_sta20["status_desc"]];
 ...
 
 {
    ...
    $query_cancellation = "UPDATE ret_plb_detail SET status_tp = ?, ref_doc_tp = ?, user_cancel = ?, date_cancel = NOW() WHERE doc_tp = ? AND status_tp = ? AND id_tp = ?"; $query_cancellation_args = [$rst_sta4["status_desc"], $ref, $username, $doc_tp, $rst_sta20["status_desc"], $row2["id_tp"]];
   ...
   $query_rcv_ftp = "SELECT *, DATE_FORMAT(date_cancel,'%d-%m-%Y') AS J, DATE_FORMAT(date_create,'%Y') AS R2 FROM ret_plb_detail WHERE ref_doc_tp = ? AND id_tp = ?"; $query_rcv_ftp_args = [$ref, $row2["id_tp"]];
...
<br>
          <div class="widget-title"> <span class="icon"><i class="icon-th"></i></span>
            <h5>Cancel Return from PLB  : [<?php echo h($doc_tp);   ?>]</h5>
          </div>
        ...
        <br>  
              <!-- End Box Head -->
            <form method="post" action="cancel_return_to_plb.php?uid=<?php echo h($doc_tp); ?>" >
            <table class="table table-bordered">
               <thead>
               <tr>
                 <th>Item</th>
 ...
 $no = sprintf('%03d', $no);
		 ...
		 ?>
               ...
               <tr>
                ...
                <td width="200"><?php echo h($row["material_desc"]); ?></td>
