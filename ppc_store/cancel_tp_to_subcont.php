...
$queryu = "SELECT * FROM tp_subcont_detail WHERE doc_tp = ?"; $queryu_args = [$doc_tp];
$rs = db_query_bind($dbc, $queryu, $queryu_args);   //run the query.


$query_2 = "SELECT *, DATE_FORMAT(MR.posting_date,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R2 FROM tp_subcont_detail as MR, scan_tp_plb as SD WHERE MR.id_scan_tp = SD.id_scan_tp AND MR.doc_tp = ?"; $query_2_args = [$doc_tp];
...
$message = NULL; // create an empty new variable.
 ...
 $id_tp = $_POST["id_tp"];
   ...
   //-------------------generate tp to store cancellation doc no.---------------
     
	 $query_gen_ID = "SELECT MAX(count_max) FROM run_count_no WHERE uid = '38'";
 ...
 //-----select info tp_subcont_detail -----------         
  $query_info = "SELECT * FROM tp_subcont_detail WHERE doc_tp = ? AND status_tp = ?"; $query_info_args = [$doc_tp, $rst_sta19["status_desc"]];
 ...
 
 {
    ...
    $query_cancellation = "UPDATE tp_subcont_detail SET status_tp = ?, ref_doc_tp = ?, user_cancel = ?, date_cancel = NOW() WHERE doc_tp = ? AND status_tp = ? AND id_tp = ?"; $query_cancellation_args = [$rst_sta4["status_desc"], $ref, $username, $doc_tp, $rst_sta19["status_desc"], $row2["id_tp"]];
	$result_cancellation  = db_query_bind($dbc, $query_cancellation, $query_cancellation_args); 
		  ...
		  //---------------------------------------------------------------------------------------------------------------
		  	...
		  	$query_mm3 = "SELECT * FROM tp_subcont_detail WHERE doc_tp = ? AND id_tp = ?"; $query_mm3_args = [$doc_tp, $row2["id_tp"]]; 
   ...
   $query_rcv_ftp = "SELECT *, DATE_FORMAT(date_cancel,'%d-%m-%Y') AS J, DATE_FORMAT(date_create,'%Y') AS R2 FROM tp_subcont_detail WHERE ref_doc_tp = ? AND id_tp = ?"; $query_rcv_ftp_args = [$ref, $row2["id_tp"]];
...
$data_rcv .= $data_rcv_ftp["J"].";".$data_rcv_ftp["doc_tp"].";".$data_rcv_ftp["R2"]."\r\n";
    ...
    $query_rcv_ftp_info = "INSERT INTO ftp_tp_cancel_subcont(id, file_name, doc_tp, ref_doc_tp, id_tp, material_no, material_desc, qty_ftp, uom, plant, shift_day, mvt_type, status_ftp, posting_date, posting_time, user_create, date_create, vendor_no) VALUES('',?,?,?,".$data_rcv_ftp["id_tp"].",?,?,?,?,?,?,'542','Y',NOW(),NOW(),?,NOW(),?)"; $query_rcv_ftp_info_args = [$filen_rcv, $ref, $data_rcv_ftp["doc_tp"], $data_rcv_ftp["material_no"], $data_rcv_ftp["material_desc"], $data_rcv_ftp["qty_tp"], $data_rcv_ftp["uom"], $data_rcv_ftp["plan_code"], $data_rcv_ftp["shift_day"], $username, $data_rcv_ftp["sloc_to"]]; 
     $rst_rcv_ftp_info = db_query_bind($dbc, $query_rcv_ftp_info, $query_rcv_ftp_info_args);
	  ...
	  //update count_max----------------------------------------
	   ...
	   $query_max_a = "UPDATE run_count_no SET count_max = '".$number2."', date_updated = NOW() WHERE uid = '38'";
...
<br>
          <div class="widget-title"> <span class="icon"><i class="icon-th"></i></span>
            <h5>Cancel TP to Subcont   : [<?php echo h($doc_tp);   ?>]</h5>
          </div>
        ...
        <tr>
          ...
          <th width="4%">:</th>
          <th width="30%"><?php echo h($doc_tp); ?></th>
        ...
        <br>  
              <!-- End Box Head -->
            <form method="post" action="cancel_tp_to_subcont.php?uid=<?php echo h($doc_tp); ?>" >
            <table class="table table-bordered">
               <thead>
               <tr>
                 <th>Item</th>
 ...
 $no = sprintf('%03d', $no);
		 ...
		 ?>
               ...
               <tr>
                ...
                <input name="doc_tp" type="hidden" value="<?php echo h($doc_tp); ?> ">
		  ...
		  $counter++; // menambah counter 
   		  $no ++;   
			   ...
			   ?></tbody> </table> 
         <p>&nbsp;</p>
          <input type="submit" onClick="return confirm('Are you sure you want to cancel document no. : <?php echo h($doc_tp); ?>?');" name="submit" id="button" value="CONFIRM" class="btn btn-success"/> &nbsp;&nbsp; 
