...
$queryu = "SELECT * FROM tp_subcont_detail WHERE doc_tp = '".db_esc($dbc, $doc_tp)."'";
$rs = mysqli_query($dbc, $queryu);   //run the query.


$query_2 = "SELECT *, DATE_FORMAT(MR.posting_date,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R2 FROM tp_subcont_detail as MR, scan_tp_plb as SD WHERE MR.id_scan_tp = SD.id_scan_tp AND MR.doc_tp = '".db_esc($dbc, $doc_tp)."'";
...
$message = NULL; // create an empty new variable.
 ...
 //-----select info tp_subcont_detail -----------         
  $query_info = "SELECT * FROM tp_subcont_detail WHERE doc_tp = '".db_esc($dbc, $doc_tp)."' AND status_tp = '".db_esc($dbc, $rst_sta19["status_desc"])."'";
 ...
 
 {
    ...
    $query_cancellation = "UPDATE tp_subcont_detail SET status_tp = '".db_esc($dbc, $rst_sta4["status_desc"])."', ref_doc_tp = '".db_esc($dbc, $ref)."', user_cancel = '".db_esc($dbc, $username)."', date_cancel = NOW() WHERE doc_tp = '".db_esc($dbc, $doc_tp)."' AND status_tp = '".db_esc($dbc, $rst_sta19["status_desc"])."' AND id_tp = '".db_esc($dbc, $row2["id_tp"])."'";
	$result_cancellation  = mysqli_query($dbc, $query_cancellation); 
		  ...
		  //---------------------------------------------------------------------------------------------------------------
		  	...
		  	$query_mm3 = "SELECT * FROM tp_subcont_detail WHERE doc_tp = '".db_esc($dbc, $doc_tp)."' AND id_tp = '".db_esc($dbc, $row2["id_tp"])."'"; 
   ...
   $query_rcv_ftp = "SELECT *, DATE_FORMAT(date_cancel,'%d-%m-%Y') AS J, DATE_FORMAT(date_create,'%Y') AS R2 FROM tp_subcont_detail WHERE ref_doc_tp = '".db_esc($dbc, $ref)."' AND id_tp = '".db_esc($dbc, $row2["id_tp"])."'";
