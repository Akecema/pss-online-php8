...
<tr>
    ...
    <td width="85%"> <div class="small-nav"></div></td> 
      <td width="7%"><a href="report_document_list_return_to_plb_download.php?doc_tp_from=<?php echo $doc_tp_from; ?>&&doc_tp_to=<?php echo $doc_tp_to; ?>&&shift_day=<?php echo $shift_day; ?>&date1=<?php echo $dateF; ?>&&date2=<?php echo $dateT; ?>&&material_no=<?php echo $material_no; ?>" ><img src="../img/dload_excel.jpg" width="48" height="48" title="Download" /></a></td>
 ...
 <?php
   ...
   while ($data_sql2 = mysqli_fetch_array($result_sql2))
   {
	 ...
	 $query_sql3 = "SELECT *, DATE_FORMAT(posting_date,'%d-%m-%Y') as R3 FROM ret_plb_detail WHERE status_tran = 'Y' AND doc_tp = '".$data_sql2["doc_tp"]."' ORDER BY posting_date DESC, posting_time DESC";
	   ...
	   while ($data_sql3 = mysqli_fetch_array($result_sql3))
   {
	 ...
	 if($data_sql3["shift_day"] == "D/S")
	 {
	  ...
	  //---- record cancellation ------ //
		$query_cancel_plb = "SELECT *, DATE_FORMAT(date_cancel,'%d-%m-%Y') as R4, DATE_FORMAT(date_cancel,'%H:%i:%s') as R5 FROM ret_plb_detail WHERE id_tp = '".$data_sql3["id_tp"]."' AND status_tp = '".$rst_sta4["status_desc"]."' ORDER BY posting_date DESC, posting_time DESC";
