...
/**
 * ppc_store/document_list_RT_to_subcontProc.php
 * Part of: PPC Store module
 * Filename suggests: document list RT to subcontProc
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, vendor_detail, ret_subcont_detail, ret_subcont_cancel.
...
<div id="content-header">
  ...
  <h1>Document List Return from Subcont</h1>
</div>
        ...
        <div class="widget-box">
          <div class="widget-title">
             <ul class="nav nav-tabs" role="tablist" id="templatemo-tabs">
              ...
              <li class="active"><a role="tab" href="document_list_RT_to_subcont.php">Return from Subcont</a></li>
             </ul>
 ...
 ?>        
          <form action="<?php echo h($_SERVER['PHP_SELF']); ?>" method="get" name="frmSearch" id="frmSearch">
            <table class="table table-bordered table-striped">
              <tr>
               ...
               <tr>
              <th>Vendor :</th>
              <td><select name="vendor_no" class="span11">
  ...
  $result2a = mysqli_query($dbc, $query2a);
      ...
      ?>
               <!--RETAIN VALUE-->
               <option value="<?php echo h($row2a["vendor_code"])?>" <?php if($row2a["vendor_code"] == $_GET["vendor_no"]) echo "selected"; ?>> <?php echo h($row2a["vendor_name"]); ?></option>
 ...
 <?php              
		  ...
		  //5. Shift
		   ...
		   //6. Vendor No.
                if ($vendor_no == "NULL"){ 
                    ...
                    $wheresql_06 = " AND vendor_no = ?"; $wheresql_06_args = [$vendor_no]; }  
					
		  //7. Material No.
                if ($material_no == ""){ 
                    $wheresql_07 = ""; $wheresql_07_args = []; }
                else {
                    $wheresql_07 = " AND material_no = ?"; $wheresql_07_args = [$material_no]; }  			 			
   ...
   $query8 = "SELECT * FROM ret_subcont_detail WHERE status_tran = 'Y' ".$where_sql." GROUP BY doc_tp"; $query8_args = [...$where_args];
   ...
   $query8a = "SELECT * FROM ret_subcont_cancel WHERE status_tran = 'Y' ".$where_sql." GROUP BY doc_tp"; $query8a_args = [...$where_args];
   $result8a = db_query_bind($dbc, $query8a, $query8a_args) or die(db_fail($dbc));
...
$query_sql2 = "SELECT *, DATE_FORMAT(posting_date,'%d-%m-%Y') as R2 FROM ret_subcont_detail WHERE status_tran = 'Y' ".$where_sql. " GROUP BY doc_tp ORDER BY posting_date DESC, posting_time DESC"; $query_sql2_args = [...$where_args];
...
//$num = mysqli_num_rows($result_sql2);   //how many material are there?
	...
	// if ($num > 0) {
	 ...
	 echo '<div align="center">There are currently  '. h($count_record).' record(s).</div>';
	// }
...
<tr>
    ...
    <td width="85%"> <div class="small-nav"></div></td> 
      <td width="7%"><a href="report_document_list_return_to_subcont_download.php?doc_tp_from=<?php echo h($doc_tp_from); ?>&&doc_tp_to=<?php echo h($doc_tp_to); ?>&&shift_day=<?php echo h($shift_day); ?>&date1=<?php echo h($dateF); ?>&&date2=<?php echo h($dateT); ?>&&material_no=<?php echo h($material_no); ?>&&vendor_no=<?php echo h($vendor_no); ?>" ><img src="../img/dload_excel.jpg" width="48" height="48" title="Download" /></a></td>
 ...
 <?php
   ...
   while ($data_sql2 = mysqli_fetch_array($result_sql2))
   {
	 ...
	 $query_sql3 = "SELECT *, DATE_FORMAT(posting_date,'%d-%m-%Y') as R3 FROM ret_subcont_detail WHERE status_tran = 'Y' AND doc_tp = ? ORDER BY posting_date DESC, posting_time DESC"; $query_sql3_args = [$data_sql2["doc_tp"]];
	   ...
	   while ($data_sql3 = mysqli_fetch_array($result_sql3))
   {
	 ...
	 if($data_sql3["shift_day"] == "D/S")
	 {
	  ...
	  //---- record cancellation ------ //
		$query_cancel_plb = "SELECT *, DATE_FORMAT(date_cancel,'%d-%m-%Y') as R4, DATE_FORMAT(date_cancel,'%H:%i:%s') as R5 FROM ret_subcont_detail WHERE id_tp = ? AND status_tp = ? ORDER BY posting_date DESC, posting_time DESC"; $query_cancel_plb_args = [$data_sql3["id_tp"], $rst_sta4["status_desc"]];
    ...
    <td width="70"><?php echo h($data_sql3["vendor_no"]);  ?></td>
   ...
   <?php
    ...
    <td width="70"><?php echo h($row_cancel["vendor_no"]);  ?></td>
