...
<div id="content-header">
 ...
 ?>        
  ...
  $result2a = mysqli_query($dbc, $query2a);
      ...
      ?>
               <!--RETAIN VALUE-->
               <option value="<?php echo $row2a["vendor_code"]?>" <?php if($row2a["vendor_code"] == $_GET["vendor_no"]) echo "selected"; ?>> <?php echo $row2a["vendor_name"]; ?></option>
		  ...
		  //5. Shift
		   ...
		   //6. Vendor No.
                if ($vendor_no == "NULL"){ 
                    ...
                    $wheresql_06 = " AND vendor_no = '$vendor_no'"; }  
					
		  //7. Material No.
                if ($material_no == ""){ 
                    $wheresql_07 = ""; }
                else {
                    $wheresql_07 = " AND material_no = '$material_no'"; }  			 			
...
<tr>
    ...
    <td width="85%"> <div class="small-nav"></div></td> 
      <td width="7%"><a href="report_document_list_return_to_subcont_download.php?doc_tp_from=<?php echo $doc_tp_from; ?>&&doc_tp_to=<?php echo $doc_tp_to; ?>&&shift_day=<?php echo $shift_day; ?>&date1=<?php echo $dateF; ?>&&date2=<?php echo $dateT; ?>&&material_no=<?php echo $material_no; ?>&&vendor_no=<?php echo $vendor_no; ?>" ><img src="../img/dload_excel.jpg" width="48" height="48" title="Download" /></a></td>
 ...
 <?php
   ...
   while ($data_sql2 = mysqli_fetch_array($result_sql2))
   {
	 ...
	 $query_sql3 = "SELECT *, DATE_FORMAT(posting_date,'%d-%m-%Y') as R3 FROM ret_subcont_detail WHERE status_tran = 'Y' AND doc_tp = '".db_esc($dbc, $data_sql2["doc_tp"])."' ORDER BY posting_date DESC, posting_time DESC";
	   ...
	   while ($data_sql3 = mysqli_fetch_array($result_sql3))
   {
	 ...
	 if($data_sql3["shift_day"] == "D/S")
	 {
	  ...
	  //---- record cancellation ------ //
		$query_cancel_plb = "SELECT *, DATE_FORMAT(date_cancel,'%d-%m-%Y') as R4, DATE_FORMAT(date_cancel,'%H:%i:%s') as R5 FROM ret_subcont_detail WHERE id_tp = '".db_esc($dbc, $data_sql3["id_tp"])."' AND status_tp = '".db_esc($dbc, $rst_sta4["status_desc"])."' ORDER BY posting_date DESC, posting_time DESC";
    ...
    <td width="70"><?php echo $data_sql3["vendor_no"];  ?></td>
   ...
   <?php
    ...
    <td width="70"><?php echo $row_cancel["vendor_no"];  ?></td>
