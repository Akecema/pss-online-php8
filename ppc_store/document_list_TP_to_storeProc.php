		 ...
		 //3. doc tp from
          ...
          //4. doc tp to
                if ($doc_tp_to  == "" ){
					...
					$wheresql_04 = " AND doc_tp <= '$doc_tp_to'"; }
...
<tr>
 ...
 <?php
   ...
   while ($data_sql2 = mysqli_fetch_array($result_sql2))
   {
	 ...
	 $query_sql3 = "SELECT *, DATE_FORMAT(posting_date,'%d-%m-%Y') as R3 FROM tp_store_detail WHERE status_tran = 'Y' AND doc_tp = '".db_esc($dbc, $data_sql2["doc_tp"])."' ORDER BY posting_date DESC, posting_time DESC";
	   ...
	   while ($data_sql3 = mysqli_fetch_array($result_sql3))
   {
	 ...
	 //---- record cancellation ------ //
		$query_cancel_plb = "SELECT *, DATE_FORMAT(date_cancel,'%d-%m-%Y') as R4, DATE_FORMAT(date_cancel,'%H:%i:%s') as R5 FROM tp_store_detail WHERE id_tp = '".db_esc($dbc, $data_sql3["id_tp"])."' AND status_tp = '".db_esc($dbc, $rst_sta4["status_desc"])."' ORDER BY posting_date DESC, posting_time DESC";
   ...
   <tr class="gradeX">
    ...
    <td width="210"><?php echo $data_sql3["material_desc"];  ?></td>
    ...
    <td width="50"><?php echo $data_sql3["slip_no"];  ?></td>
  ...
  <?php
    ...
    <td width="210"><?php echo $row_cancel["material_desc"];  ?></td>
    ...
    <td width="50"><?php echo $row_cancel["slip_no"];  ?></td>
    <td width="50"><?php echo strtoupper($shift_desc);  ?></td>
