...
/**
 * ppc_store/document_list_TP_to_subcontProc.php
 * Part of: PPC Store module
 * Filename suggests: document list TP to subcontProc
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, vendor_detail, tp_subcont_detail, tp_subcont_cancel.
...
<div id="content-header">
 ...
 <?php              
   ...
   $query8 = "SELECT * FROM tp_subcont_detail WHERE status_tran = 'Y' ".$where_sql." GROUP BY doc_tp"; $query8_args = [...$where_args];
   ...
   $query8a = "SELECT * FROM tp_subcont_cancel WHERE status_tran = 'Y' ".$where_sql." GROUP BY doc_tp"; $query8a_args = [...$where_args];
...
$query_sql2 = "SELECT *, DATE_FORMAT(posting_date,'%d-%m-%Y') as R2 FROM tp_subcont_detail WHERE status_tran = 'Y' ".$where_sql. " GROUP BY doc_tp ORDER BY doc_tp DESC"; $query_sql2_args = [...$where_args];
...
<tr>
    ...
    <td width="85%"> <div class="small-nav"></div></td> 
      <td width="7%"><a href="report_document_list_TP_to_subcont_download.php?doc_tp_from=<?php echo h($doc_tp_from); ?>&&doc_tp_to=<?php echo h($doc_tp_to); ?>&&shift_day=<?php echo h($shift_day); ?>&date1=<?php echo h($dateF); ?>&&date2=<?php echo h($dateT); ?>&&material_no=<?php echo h($material_no); ?>&&vendor_no=<?php echo h($vendor_no); ?>" ><img src="../img/dload_excel.jpg" width="48" height="48" title="Download" /></a></td>
 ...
 <?php
   ...
   while ($data_sql2 = mysqli_fetch_array($result_sql2))
   {
	 ...
	 $query_sql3 = "SELECT *, DATE_FORMAT(posting_date,'%d-%m-%Y') as R3 FROM tp_subcont_detail WHERE status_tran = 'Y' AND doc_tp = ? ORDER BY posting_date DESC, posting_time DESC"; $query_sql3_args = [$data_sql2["doc_tp"]];
	   ...
	   while ($data_sql3 = mysqli_fetch_array($result_sql3))
   {
	 ...
	 if($data_sql3["shift_day"] == "D/S")
	 {
	  ...
	  //---- record cancellation ------ //
		$query_cancel_plb = "SELECT *, DATE_FORMAT(date_cancel,'%d-%m-%Y') as R4, DATE_FORMAT(date_cancel,'%H:%i:%s') as R5 FROM tp_subcont_detail WHERE id_tp = ? AND status_tp = ? ORDER BY posting_date DESC, posting_time DESC"; $query_cancel_plb_args = [$data_sql3["id_tp"], $rst_sta4["status_desc"]];
