...
/**
 * ppc_store/list_ftp_rt_plb.php
 * Part of: PPC Store module
 * Filename suggests: list ftp rt plb
 *
 * Behavior: requires an active login session ($_SESSION['username']).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, ftp_ret_plb, table_material, ftp_ret_cancel_plb.
    ...
    <div class="row-fluid">
         <div class="widget-box">
          <div class="widget-title">
              ...
              <li class="active"><a role="tab" href="list_ftp_rt_plb.php">Return from PLB</a></li>
...
$path = "../../PSS_Online/FromPortal3/RT_PLB/"; 

// Open the folder
 $dir_handle = @opendir($root . $path) or die("Unable to open $path");

$dir = "$root/FromPortal3/RT_PLB/";

$folder = "../../PSS_Online/FromPortal3/RT_PLB/";
$filetype = '*.*';    
...
$dir = "$root/PSS_Online/FromPortal3/RT_PLB/";
...
if(is_dir($dir)){
  ...
  <!--  <td width="10%">Material Type</td>-->
   ...
   <?php
	...
	$query = "SELECT *, DATE_FORMAT(date_create,'%d-%m-%Y %H:%i:%s') AS R2, DATE_FORMAT(posting_date,'%d-%m-%Y') AS R3 FROM ftp_ret_plb WHERE status_ftp = 'Y' AND file_name = ?"; $query_args = [$filename2];
	$rs = db_query_bind($dbc, $query, $query_args);
 ...
 {  ?> 
  <tr>
    ...
    <td width="15%"><?php echo h($row_rs["qty_ftp"]); ?></td>
   <!-- <td width="10%"><?php //echo h($row_mat_type["mat_type"]); ?></td>-->
    <td width="15%"><?php echo h($row_rs["R3"]);  ?></td>
 ...
 <?php  
   ...
   mysqli_free_result($rs); 
	...
	$query_cancel = "SELECT *,DATE_FORMAT(date_create,'%d-%m-%Y %H:%i:%s') AS R8, DATE_FORMAT(posting_date,'%d-%m-%Y') AS R7 FROM ftp_ret_cancel_plb WHERE status_ftp = 'Y' AND file_name = ?"; $query_cancel_args = [$filename2];
 ...
 {  ?>
   <tr>
    ...
    <td width="15%"><?php  echo h($row_rs_cancel["qty_ftp"]); ?></td>
   <!-- <td width="10%"><?php  //echo h($row_mat_type_c["mat_type"]); ?></td>-->
    <td width="15%"><?php  echo h($row_rs_cancel["R7"]);  ?></td>
