...
/**
 * ppc_store/list_ftp_tr_store.php
 * Part of: PPC Store module
 * Filename suggests: list ftp tr store
 *
 * Behavior: requires an active login session ($_SESSION['username']).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, ftp_tp_store, table_material, tp_store_detail, ftp_tp_cancel_store, tp_store_cancel.
    ...
    <div class="row-fluid">
         <div class="widget-box">
          <div class="widget-title">
              ...
              <li class="active"><a  role="tab" href="list_ftp_tr_store.php">TP to Store</a></li>
...
$path = "../../PSS_Online/FromPortal3/TP_STORE/"; 

// Open the folder
 $dir_handle = @opendir($root . $path) or die("Unable to open $path");

$dir = "$root/FromPortal3/TP_STORE/";

$folder = "../../PSS_Online/FromPortal3/TP_STORE/";
$filetype = '*.*';    
...
$dir = "$root/PSS_Online/FromPortal3/TP_STORE/";
...
if(is_dir($dir)){
  ...
  // $total2 = count($dh); removed - count() on a directory resource/handle throws a fatal TypeError under PHP 8 (was PHP7: warning + returned 1). $total2 was assigned but never read anywhere in this file, so the line was just dropped rather than wrapped.
   ...
   <?php
	...
	$query = "SELECT *, DATE_FORMAT(date_create,'%d-%m-%Y %H:%i:%s') AS R2, DATE_FORMAT(posting_date,'%d-%m-%Y') AS R3 FROM ftp_tp_store WHERE status_ftp = 'Y' AND file_name = ?"; $query_args = [$filename2];
	...
	$query_docum = "SELECT * FROM tp_store_detail WHERE id_tp = ?"; $query_docum_args = [$row_rs["id_tp"]];
	$rs_docum = db_query_bind($dbc, $query_docum, $query_docum_args);   //run the query.
	$row_docum = mysqli_fetch_array($rs_docum);   //how many material are there?	
 ...
 {  ?> 
  <tr>
    <td width="15%"><?php echo h($filename2); ?></td>
 ...
 <?php  
   ...
   $query_cancel = "SELECT *,DATE_FORMAT(date_create,'%d-%m-%Y %H:%i:%s') AS R8, DATE_FORMAT(posting_date,'%d-%m-%Y') AS R7 FROM ftp_tp_cancel_store WHERE status_ftp = 'Y' AND file_name = ?"; $query_cancel_args = [$filename2];
	...
	$query_docum_c = "SELECT * FROM tp_store_cancel WHERE id_tp = ?"; $query_docum_c_args = [$row_rs_cancel["id_tp"]];
	$rs_docum_c = db_query_bind($dbc, $query_docum_c, $query_docum_c_args);   //run the query.
	$row_docum_c = mysqli_fetch_array($rs_docum_c);   //how many material are there?	
