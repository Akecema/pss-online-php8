...
if(is_dir($dir)){
  ...
  $total2 = count($dh);  
   ...
   <?php
	...
	$rs_docum = mysql_query($query_docum);   //run the query.
	$row_docum = mysql_fetch_array($rs_docum);   //how many material are there?	
 ...
 <?php  
   ...
   $query_cancel = "SELECT *,DATE_FORMAT(date_create,'%d-%m-%Y %H:%i:%s') AS R8, DATE_FORMAT(posting_date,'%d-%m-%Y') AS R7 FROM ftp_tp_cancel_store WHERE status_ftp = 'Y' AND file_name = '".$filename2."'";
	...
	$rs_docum_c = mysql_query($query_docum_c);   //run the query.
	$row_docum_c = mysql_fetch_array($rs_docum_c);   //how many material are there?	
