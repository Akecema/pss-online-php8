...
/**
 * ppc_store/list_ftp_tr_subcont.php
 * Part of: PPC Store module
 * Filename suggests: list ftp tr subcont
 *
 * Behavior: requires an active login session ($_SESSION['username']).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, ftp_tp_subcont, table_material, vendor_detail, ftp_tp_cancel_subcont.
    ...
    <div class="row-fluid">
         <div class="widget-box">
          <div class="widget-title">
              ...
              <li class="active"><a role="tab" href="list_ftp_tr_subcont.php">TP to Subcont</a></li>
              <li><a role="tab" href="list_ftp_rt_plb.php">Return from PLB</a></li>
              <li><a role="tab" href="list_ftp_rt_subcont.php">Return from Subcont</a></li>
             </ul>
          </div>
          </div>
...
$path = "../../PSS_Online/FromPortal3/TP_SUBCON/"; 

// Open the folder
 $dir_handle = @opendir($root . $path) or die("Unable to open $path");

$dir = "$root/FromPortal3/TP_SUBCON/";

$folder = "../../PSS_Online/FromPortal3/TP_SUBCON/";
$filetype = '*.*';    
...
$dir = "$root/PSS_Online/FromPortal3/TP_SUBCON/";
...
if(is_dir($dir)){
  ...
  // $total2 = count($dh); removed - count() on a directory resource/handle throws a fatal TypeError under PHP 8 (was PHP7: warning + returned 1). $total2 was assigned but never read anywhere in this file, so the line was just dropped rather than wrapped.
   ...
   <?php
	...
	$query = "SELECT *, DATE_FORMAT(date_create,'%d-%m-%Y %H:%i:%s') AS R2, DATE_FORMAT(posting_date,'%d-%m-%Y') AS R3 FROM ftp_tp_subcont WHERE status_ftp = 'Y' AND file_name = ?"; $query_args = [$filename2];
	...
	$query_vendor = "SELECT * FROM vendor_detail WHERE vendor_code = ?"; $query_vendor_args = [$row_rs["vendor_no"]];
	$rs_vendor = db_query_bind($dbc, $query_vendor, $query_vendor_args);   //run the query.
 ...
 {  ?> 
  <tr>
    <td width="20%"><?php echo h($filename2); ?></td>
    <td width="20%"><?php echo h($row_rs["doc_tp"]); ?></td>
    <td width="20%"><?php echo h($row_rs["material_no"]); ?></td>
    <td width="10%"><?php echo h($row_rs["qty_ftp"]); ?></td>
    <td width="10%"><?php echo h($row_vendor["search_term"]); ?></td>
...
//-----------------cancel ----------------
	$query_cancel = "SELECT *,DATE_FORMAT(date_create,'%d-%m-%Y %H:%i:%s') AS R8, DATE_FORMAT(posting_date,'%d-%m-%Y') AS R7 FROM ftp_tp_cancel_subcont WHERE status_ftp = 'Y' AND file_name = ?"; $query_cancel_args = [$filename2];
	$rs_cancel = db_query_bind($dbc, $query_cancel, $query_cancel_args);   //run the query.
	...
	$query_mat_type_c = "SELECT * FROM table_material WHERE material_no = ?"; $query_mat_type_c_args = [$row_rs_cancel["material_no"]];
	$rs_mat_type_c = db_query_bind($dbc, $query_mat_type_c, $query_mat_type_c_args);   //run the query.
	$row_mat_type_c = mysqli_fetch_array($rs_mat_type_c);   //how many material are there?
	
 	//---check vendor in table vendor_detail--------
		
	$query_vendor_cancel = "SELECT * FROM vendor_detail WHERE vendor_code = ?"; $query_vendor_cancel_args = [$row_rs_cancel["vendor_no"]];
	$rs_vendor_cancel = db_query_bind($dbc, $query_vendor_cancel, $query_vendor_cancel_args);   //run the query.
...
?>	  
 ...
 {  ?>
   <tr>
    <td width="20%"><?php  echo h($filename2); ?></td>
    <td width="20%"><?php  echo h($row_rs_cancel["doc_tp"]); ?></td>
    <td width="20%"><?php  echo h($row_rs_cancel["material_no"]); ?></td>
    <td width="10%"><?php  echo h($row_rs_cancel["qty_ftp"]); ?></td>
    <td width="10%"><?php  echo h($row_vendor_cancel["search_term"]); ?></td>
