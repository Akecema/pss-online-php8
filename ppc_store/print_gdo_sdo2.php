...
/**
 * ppc_store/print_gdo_sdo2.php
 * Part of: PPC Store module
 * Filename suggests: print gdo sdo2
 *
...
?></td>
  ...
  $query2 = "SELECT * FROM vendor_detail WHERE status_acc = 'Y' AND status_subcont = 'Y'";
  $result2 = mysqli_query($dbc, $query2);
      ...
      while($row2 =mysqli_fetch_array($result2)) {
          ...
          if($_POST["Submit"] == true){ ?>
               <!--RETAIN VALUE-->
               <option value="<?php echo h($row2[0])?>" <?php if($row2[0] == $_POST["vendor_no"]) echo "selected"; ?>> <?php echo h($row2[1])?></option>
               <?php }else{ ?>
               <option value="<?php echo h($row2[0])?>" > <?php echo stripslashes($row2[1])?></option>
               <?php } ?>
               <?php
							}
	...
	?></td>
              ...
              <td>                 
               ...
               <tr>
                 ...
                 <th>&nbsp;</th>
                 <td><input name="Submit" type="submit" value="Search" class="btn btn btn-info"></td>
               </tr>
              </table>
      ...
      <tr></form>
        ...
        {   
            ...
            echo "<script>";
		   echo "window.location='print_gdo_sdo2Proc.php?doc_tp_from=$doc_tp_from&&doc_tp_to=$doc_tp_to&&shift_day=$shift_day&&date1=$dateF&&date2=$dateT&&material_no=$material_no&&vendor_no=$vendor_no'";
            echo "</script>";
