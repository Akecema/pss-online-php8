...
//-------select data from database --------------------------//
    ...
    $query_detail = "SELECT *, DATE_FORMAT(posting_date,'%d-%m-%Y') AS T5, DATE_FORMAT(date_generate_tp,'%d-%m-%Y') AS T15 FROM tp_subcont_detail WHERE status_tran = 'Y' AND doc_tp = ? GROUP BY doc_tp"; $query_detail_args = [$uid];
    ...
    $query2 = "SELECT * FROM tp_subcont_detail WHERE status_tran = 'Y' AND doc_tp = ? ORDER BY doc_tp"; $query2_args = [$uid];
...
{	
	...
	$query_model = "SELECT * FROM mat_master_header WHERE material_no = ?"; $query_model_args = [$part_no];
	$result_model = db_query_bind($dbc, $query_model, $query_model_args) or die(db_fail($dbc));
