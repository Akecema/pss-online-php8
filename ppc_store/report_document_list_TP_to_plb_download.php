  ...
  while ($data_sql2 = mysqli_fetch_array($result_sql2))
	   ...
	   while ($data_sql3 = mysqli_fetch_array($result_sql3))
   {
	 ...
	 if($data_sql3["shift_day"] == "D/S")
	 {
		...
		//---- record cancellation ------ //
		$query_cancel_plb = "SELECT *, DATE_FORMAT(date_cancel,'%d-%m-%Y') as R4, DATE_FORMAT(date_cancel,'%H:%i:%s') as R5 FROM tp_plb_detail WHERE id_tp = '".db_esc($dbc, $data_sql3["id_tp"])."' AND status_tp = '".db_esc($dbc, $rst_sta4["status_desc"])."'";
