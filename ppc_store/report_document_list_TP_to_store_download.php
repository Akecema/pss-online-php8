...
/**
 * ppc_store/report_document_list_TP_to_store_download.php
 * Part of: PPC Store module
 * Filename suggests: report document list TP to store download
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: sys_setup_maintain, request_status, tp_store_detail, tp_store_cancel.
 * Includes: config.php.
 *
...
$namaFile = "Transfer to STORE_".$date_tdy.".xls";

 $count_record = "";	
		
  $query8 = "SELECT * FROM tp_store_detail WHERE status_tran = 'Y' ".$where_sql." GROUP BY doc_tp"; $query8_args = [...$where_args];
  $result8 = db_query_bind($dbc, $query8, $query8_args) or die(db_fail($dbc));
  $num_rows = mysqli_num_rows($result8);

  $query8a = "SELECT * FROM tp_store_cancel WHERE status_tran = 'Y' ".$where_sql." GROUP BY doc_tp"; $query8a_args = [...$where_args];
...
$content .= "<font size='12px'><strong>DOCUMENT LIST - TRANSFER TO STORE</strong></font> ";
$content .= "<br>";
...
echo '<th width="5" bgcolor="#E9F58D">SLIP NO.</th>';
...
$query_sql2 = "SELECT *, DATE_FORMAT(posting_date,'%d-%m-%Y') as R2 FROM tp_store_detail WHERE status_tran = 'Y' ".$where_sql. " GROUP BY doc_tp ORDER BY posting_date DESC, posting_time DESC"; $query_sql2_args = [...$where_args];
  ...
  while ($data_sql2 = mysqli_fetch_array($result_sql2))
   {
	 ...
	 $query_sql3 = "SELECT *, DATE_FORMAT(posting_date,'%d-%m-%Y') as R3 FROM tp_store_detail WHERE status_tran = 'Y' AND doc_tp = ? ORDER BY doc_tp ASC"; $query_sql3_args = [$data_sql2["doc_tp"]];
	   ...
	   while ($data_sql3 = mysqli_fetch_array($result_sql3))
   {
	 ...
	 if($data_sql3["shift_day"] == "D/S")
	 {
		...
		echo '<td>&nbsp;'. strtoupper($data_sql3["slip_no"]).'</td>';  
	    echo '<td>&nbsp;'. strtoupper($shift_desc).'</td>';  
        echo '<td>&nbsp;'. $data_sql3["doc_tp"].'</td>';
        echo '</tr>'; 
		
		//---- record cancellation ------ //
		$query_cancel_plb = "SELECT *, DATE_FORMAT(date_cancel,'%d-%m-%Y') as R4, DATE_FORMAT(date_cancel,'%H:%i:%s') as R5 FROM tp_store_detail WHERE id_tp = ? AND status_tp = ?"; $query_cancel_plb_args = [$data_sql3["id_tp"], $rst_sta4["status_desc"]];
     ...
     if(($data_sql3["id_tp"]) == ($row_cancel["id_tp"]))  
	  {
		...
		echo '<td>&nbsp;'. strtoupper($row_cancel["slip_no"]).'</td>';  
	    echo '<td>&nbsp;'. strtoupper($shift_desc).'</td>';  
