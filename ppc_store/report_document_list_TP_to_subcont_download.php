...
/**
 * ppc_store/report_document_list_TP_to_subcont_download.php
 * Part of: PPC Store module
 * Filename suggests: report document list TP to subcont download
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: sys_setup_maintain, request_status, tp_subcont_detail, tp_subcont_cancel.
 * Includes: config.php.
 *
...
$namaFile = "Transfer to Subcont_".$date_tdy.".xls";
...
$query_sql2 = "SELECT *, DATE_FORMAT(posting_date,'%d-%m-%Y') as R2 FROM tp_subcont_detail WHERE status_tran = 'Y' ".$where_sql. " GROUP BY doc_tp ORDER BY posting_date DESC, posting_time DESC"; $query_sql2_args = [...$where_args];
  ...
  while ($data_sql2 = mysqli_fetch_array($result_sql2))
	   ...
	   while ($data_sql3 = mysqli_fetch_array($result_sql3))
   {
	 ...
	 if($data_sql3["shift_day"] == "D/S")
	 {
		...
		//---- record cancellation ------ //
		$query_cancel_plb = "SELECT *, DATE_FORMAT(date_cancel,'%d-%m-%Y') as R4, DATE_FORMAT(date_cancel,'%H:%i:%s') as R5 FROM tp_subcont_detail WHERE id_tp = ? AND status_tp = ?"; $query_cancel_plb_args = [$data_sql3["id_tp"], $rst_sta4["status_desc"]];
