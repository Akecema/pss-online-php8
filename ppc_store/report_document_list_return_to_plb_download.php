...
/**
 * ppc_store/report_document_list_return_to_plb_download.php
 * Part of: PPC Store module
 * Filename suggests: report document list return to plb download
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: sys_setup_maintain, request_status, ret_plb_detail, ret_plb_cancel.
 * Includes: config.php.
 *
  ...
  while ($data_sql2 = mysqli_fetch_array($result_sql2))
   {
	 ...
	 $query_sql3 = "SELECT *, DATE_FORMAT(posting_date,'%d-%m-%Y') as R3 FROM ret_plb_detail WHERE status_tran = 'Y' AND doc_tp = ? ORDER BY doc_tp ASC"; $query_sql3_args = [$data_sql2["doc_tp"]];
	   ...
	   while ($data_sql3 = mysqli_fetch_array($result_sql3))
   {
	 ...
	 if($data_sql3["shift_day"] == "D/S")
	 {
		...
		//---- record cancellation ------ //
		$query_cancel_plb = "SELECT *, DATE_FORMAT(date_cancel,'%d-%m-%Y') as R4, DATE_FORMAT(date_cancel,'%H:%i:%s') as R5 FROM ret_plb_detail WHERE id_tp = ? AND status_tp = ?"; $query_cancel_plb_args = [$data_sql3["id_tp"], $rst_sta4["status_desc"]];
