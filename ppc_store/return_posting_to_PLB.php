	...
	$sloc_to2 = "";
	   ...
	   foreach($_POST["cancel"] as $j=>$i) {
		...
		/* echo ($i+1).'-'.$cancel[$i]; echo "&nbsp;&nbsp;";  echo $string[$i]; echo "</br>"; */
		 ...
		 $query_update_scan = "UPDATE scan_ret_plb SET scan_qty = '".db_esc($dbc, $string[$i])."' WHERE id_scan_tp = '".db_esc($dbc, $cancel[$i])."'";
 ...
 if ($result_gen_ID) {
	 ...
	 if(isset($_POST["cancel"])) 
  {
	...
	// $t_time = (($_POST["time1"]).":".($_POST["time2"]));
		...
		$query_update_scan2 = "UPDATE scan_ret_plb SET scan_qty = '".db_esc($dbc, $string[$i])."' WHERE id_scan_tp = '".db_esc($dbc, $cancel[$i])."'";
	    $rst_update_scan2 = mysqli_query($dbc, $query_update_scan2);
		
		 //-----get info scan_ret_plb-------------
		 
		$query_info = "SELECT * FROM scan_ret_plb WHERE id_scan_tp = '".db_esc($dbc, $cancel[$i])."'";
   ...
   $query_rcv_ftp = "SELECT *, DATE_FORMAT(posting_date,'%d-%m-%Y') AS J, DATE_FORMAT(date_create,'%d-%m-%Y') AS R2 FROM ret_plb_detail WHERE doc_tp = '".db_esc($dbc, $ref)."'";
...
$data_rcv .= $data_rcv_ftp["prepared_by"].";".$data_rcv_ftp["J"].";".$data_rcv_ftp["plan_code"].";".$data_rcv_ftp["shift_day"].";".$data_rcv_ftp["sloc_from"].";".$data_rcv_ftp["sloc_to"].";".$data_rcv_ftp["material_no"].";".$data_rcv_ftp["qty_tp"].";".$data_rcv_ftp["uom"].";311;".$ref."\r\n";
    ...
    $query_rcv_ftp_info = "INSERT INTO ftp_ret_plb(id, file_name, doc_tp, id_tp, material_no, material_desc, qty_ftp, uom, plant, shift_day, mvt_type, status_ftp, posting_date, posting_time, user_create, date_create) VALUES('','".db_esc($dbc, $filen_rcv)."','".db_esc($dbc, $ref)."',".$data_rcv_ftp["id_tp"].",'".db_esc($dbc, $data_rcv_ftp["material_no"])."','".db_esc($dbc, $data_rcv_ftp["material_desc"])."','".db_esc($dbc, $data_rcv_ftp["qty_tp"])."','".db_esc($dbc, $data_rcv_ftp["uom"])."','".db_esc($dbc, $data_rcv_ftp["plan_code"])."','".db_esc($dbc, $data_rcv_ftp["shift_day"])."','311','Y','".db_esc($dbc, $data_rcv_ftp["posting_date"])."','".db_esc($dbc, $data_rcv_ftp["posting_time"])."','".db_esc($dbc, $username)."',NOW())"; 
     $rst_rcv_ftp_info = mysqli_query($dbc, $query_rcv_ftp_info);
	   ...
	   // ---update status 
		...
		$query_rcv_ftp2 = "UPDATE ret_plb_detail SET status_ftp = 'Y' WHERE doc_tp = '".db_esc($dbc, $data_rcv_ftp["doc_tp"])."'";
	...
	//update count_max----------------------------------------
       ...
       $query_max_a = "UPDATE run_count_no SET count_max = '".db_esc($dbc, $number)."', date_updated = NOW() WHERE uid = '13'";
...
//-----------delete all data current screen-------------
   ...
   $query_delete_scan = "DELETE FROM scan_ret_plb WHERE scan_doc = '".db_esc($dbc, $number)."'";
...
?></td>
	 ...
	 $w = 1;
             ...
             $query_sql2 = "SELECT * FROM scan_ret_plb WHERE scan_doc = '".db_esc($dbc, $number)."'";
