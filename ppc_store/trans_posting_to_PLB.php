	 ...
	 if(isset($_POST["cancel"])) 
  {
	...
	$sloc_to2 = "";
	   ...
	   foreach($_POST["cancel"] as $j=>$i) {
		...
		//echo ($i+1).'- kkkkk'.$cancel[$i];  echo h($string[$i]); echo "</br>"; 
...
//---------end delete ----------------------------------
 ...
 ?>         
            <form action="trans_posting_to_plb.php?scan_doc=<?php echo h($number); ?>" method="post" name="frmSearch" id="frmSearch">
...
-->          <div class="control-group">
 ...
 <?php
  while($data_sql2 = mysqli_fetch_array($result_sql2))
  {
    ...
    <td width="150" height="30"><?php  if(($data_sql2["scan_qty"] == "") || ($data_sql2["scan_qty"] == "0.000")) { ?><input name="scan_qty[<?php echo h($data_sql2["id_scan_tp"]); ?>]" type="number" min="1" value="<?php if(isset($_POST["scan_qty"])) { echo h($_POST["scan_qty"][($data_sql2["id_scan_tp"])] ?? ''); } ?>" id="scan_qty" ><?php }else{  ?> <input name="scan_qty[<?php echo h($data_sql2["id_scan_tp"]); ?>]" type="number" min="1" value="<?php echo h($data_sql2["scan_qty"]);   ?>" required><?php  } ?>
</td>
