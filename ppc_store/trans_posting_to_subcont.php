...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_ppc_store.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="tip-bottom">Transfer Posting</a> <a href="#" class="current">TP to Subcont</a> </div>
  <h1>Transfer Posting to Subcont</h1>
</div>
...
<div class="container-fluid">
  ...
  <div class="row-fluid">
        ...
        <?php
		  ...
		  //-------------------generate scan TP to subcont doc no.---------------
     $query_id_2 = "SELECT MAX(count_max) FROM run_count_no WHERE uid = '10'";
	...
	$sloc_to2 = "";
	   ...
	   foreach($_POST["cancel"] as $j=>$i) {
		...
		/* echo ($i+1).'-'.$cancel[$i]; echo "&nbsp;&nbsp;";  echo h($string[$i]); echo "</br>"; */
		 ...
		 $query_update_scan = "UPDATE scan_tp_subcont SET scan_qty = ? WHERE id_scan_tp = ?"; $query_update_scan_args = [$string[$i], $cancel[$i]];
...
$message2 = NULL; // create an empty new variable.
	 ...
	 $query_gen_ID = "SELECT MAX(count_max) FROM run_count_no WHERE uid = '37'";
 ...
 if ($result_gen_ID) {
  ...
  if($prepared_by && $driver_by && $plate_no && $plan_code && $shift_day && $date1 && $time1 && $time2 && $vendor_no && $scan_qty)
	 ...
	 if(isset($_POST["cancel"])) 
  {
	...
	/*	echo ($i+1).'-'.$cancel[$i];   echo h($string[$i]); echo "</br>";*/
	 ...
	 $t_time = (($_POST["time1"]).":".($_POST["time2"]));
		...
		$query_update_scan2 = "UPDATE scan_tp_subcont SET scan_qty = ? WHERE id_scan_tp = ?"; $query_update_scan2_args = [$string[$i], $cancel[$i]];
	    $rst_update_scan2 = db_query_bind($dbc, $query_update_scan2, $query_update_scan2_args);
		
		 //-----get info scan_tp_subcont-------------
		 
		$query_info = "SELECT * FROM scan_tp_subcont WHERE id_scan_tp = ?"; $query_info_args = [$cancel[$i]];
...
/* 
				$query_upd_shift = "SELECT * FROM tp_subcont_detail WHERE id_tp = '".mysqli_insert_id($dbc)."'";
				...
				$query_upd_shift2 = "UPDATE tp_subcont_detail SET posting_date = '".$prev_date."' WHERE id_tp = '".$row_upd_shift["id_tp"]."'";
		...
		$query_update_scan3 = "UPDATE scan_tp_subcont SET status = 'Y', sloc_to = ? WHERE id_scan_tp = ?"; $query_update_scan3_args = [$_POST["vendor_no"], $cancel[$i]];
   ...
   $query_rcv_ftp = "SELECT *, DATE_FORMAT(posting_date,'%d-%m-%Y') AS J, DATE_FORMAT(date_create,'%d-%m-%Y') AS R2 FROM tp_subcont_detail WHERE doc_tp = ?"; $query_rcv_ftp_args = [$ref];
...
$data_rcv .= $data_rcv_ftp["prepared_by"].";".$data_rcv_ftp["J"].";".$data_rcv_ftp["plan_code"].";".$data_rcv_ftp["shift_day"].";".$data_rcv_ftp["sloc_from"].";".$data_rcv_ftp["sloc_to"].";".$data_rcv_ftp["material_no"].";".$data_rcv_ftp["qty_tp"].";".$data_rcv_ftp["uom"].";541;".$ref."\r\n";

  
     //----------update table ftp_tp_subcont------------
   
    $query_rcv_ftp_info = "INSERT INTO ftp_tp_subcont(id, file_name, doc_tp, id_tp, material_no, material_desc, qty_ftp, uom, plant, shift_day, mvt_type, status_ftp, posting_date, posting_time, user_create, date_create, vendor_no) VALUES('',?,?,".$data_rcv_ftp["id_tp"].",?,?,?,?,?,?,'541','Y',?,?,?,NOW(),?)"; $query_rcv_ftp_info_args = [$filen_rcv, $ref, $data_rcv_ftp["material_no"], $data_rcv_ftp["material_desc"], $data_rcv_ftp["qty_tp"], $data_rcv_ftp["uom"], $data_rcv_ftp["plan_code"], $data_rcv_ftp["shift_day"], $data_rcv_ftp["posting_date"], $data_rcv_ftp["posting_time"], $username, $data_rcv_ftp["sloc_to"]]; 
     $rst_rcv_ftp_info = db_query_bind($dbc, $query_rcv_ftp_info, $query_rcv_ftp_info_args);
	  
	  
	  }

		$file_rcv = "../FromPortal3/TP_SUBCON/".$filen_rcv.".csv";
		file_put_contents($file_rcv,$data_rcv);

   
	   // ---update status 

		$query_rcv_ftp2 = "UPDATE tp_subcont_detail SET status_ftp = 'Y' WHERE doc_tp = ?"; $query_rcv_ftp2_args = [$data_rcv_ftp["doc_tp"]];
	...
	//update count_max----------------------------------------
       ...
       $query_max_a = "UPDATE run_count_no SET count_max = ?, date_updated = NOW() WHERE uid = '10'"; $query_max_a_args = [$number];
   ...
   //end update count_max ---------------------------------	
		   ...
		   echo "window.location='trans_posting_to_subcont.php'";
	       echo "</script>"; 
...
//-----------delete all data current screen-------------
   ...
   $query_delete_scan = "DELETE FROM scan_tp_subcont WHERE scan_doc = ?"; $query_delete_scan_args = [$number];
...
//---------end delete ----------------------------------
 ...
 ?>         
            <form action="trans_posting_to_subcont.php?scan_doc=<?php echo h($number); ?>" method="post" name="frmSearch" id="frmSearch">
...
?></td>
  ...
  $query_vendor = "SELECT * FROM vendor_detail WHERE vendor_code = ? AND status_acc = 'Y' AND status_subcont = 'Y'"; $query_vendor_args = [$_POST["vendor_no"]]; 	
  $result_vendor = db_query_bind($dbc, $query_vendor, $query_vendor_args);
...
-->                 
   ...
   <?php echo sprintf('%02d', $j); ?>      </option>
      <?php endfor; ?>
            ...
            <tr>  
              <table class="table table-bordered table-striped">
                  ...
                  <th width="80%">
               <input name="barcode_ref" type="text" id="barcode_ref" size="60" maxlength="200" class="span12" autofocus />
	 ...
	 $w = 1;
             ...
             $query_sql2 = "SELECT * FROM scan_tp_subcont WHERE scan_doc = ? AND user_create = ?"; $query_sql2_args = [$number, $username];
...
<!--         <form name="myform" method="post" action="trans_posting_to_store.php?scan_doc=<?php echo h($number); ?>" class="form-horizontal">
-->          <div class="control-group">
 ...
 <?php
  while($data_sql2 = mysqli_fetch_array($result_sql2))
  {
    ...
    <td width="150" height="30"><?php  if(($data_sql2["scan_qty"] == "") || ($data_sql2["scan_qty"] == "0.000")) { ?><input name="scan_qty[<?php echo h($data_sql2["id_scan_tp"]); ?>]" type="number" min="1" value="<?php if(isset($_POST["scan_qty"])) { echo h($_POST["scan_qty"][($data_sql2["id_scan_tp"])] ?? ''); } ?>" id="scan_qty" ><?php }else{  ?> <input name="scan_qty[<?php echo h($data_sql2["id_scan_tp"]); ?>]" type="number" min="1" value="<?php echo h($data_sql2["scan_qty"]);   ?>" required><?php  } ?></td>
