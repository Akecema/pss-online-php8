...
$queryu = "SELECT * from consumable_request WHERE temp_mrin = '".db_esc($dbc, $temp_mrin)."'";
$rs = mysqli_query($dbc, $queryu);   //run the query.


	$query_2 = "SELECT *, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R from consumable_request as MR WHERE MR.temp_mrin = '".db_esc($dbc, $temp_mrin)."'";
	$result_2 = mysqli_query($dbc, $query_2);   //run the query.
	$data_2 = mysqli_fetch_array($result_2);
	...
	$query_tp = "SELECT *, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') AS R FROM consumable_request AS MR WHERE MR.temp_mrin = '".db_esc($dbc, $temp_mrin)."' AND MR.status = 'New'";
	   ...
	   while ($row2 = mysqli_fetch_array($result_tp))
{
 ...
 $query_upd2 = "UPDATE `post_consumable_detail_header` SET status_posting = 'Close', date_close = NOW() WHERE mrin_no = '".db_esc($dbc, $temp_mrin)."'";
 $result_upd2 = mysqli_query($dbc, $query_upd2); 
	      
 $query_upd3 = "UPDATE `consumable_request` SET status = 'Close', user_update = '".db_esc($dbc, $user_no)."', date_update = NOW() WHERE temp_mrin = '".db_esc($dbc, $temp_mrin)."' AND material_no = '".db_esc($dbc, $row2["material_no"])."' ";
