...
/**
 * ppc_super/close_MRIN_consumable_ppc2.php
 * Part of: PPC module (supervisor/admin tier)
 * Filename suggests: close MRIN consumable ppc2
 *
 * Behavior: requires an active login session ($_SESSION['username']); reads parameters from the query string ($_GET).
 * Database tables referenced: user_detail, sys_setup_maintain, factory_detail, consumable_request, material_request.
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_ppc_super.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="current">Consumable Request Maintenance</a> </div>
  <h1>Consumable Request Maintenance</h1>
</div>
...
<div class="container-fluid">
  ...
  <div class="row-fluid">
	       ...
	       $query3 = "SELECT * FROM factory_detail GROUP BY factory_desc2 ORDER BY id_fac ASC";
				  ...
				  ?>
                            <option value="<?php echo h($row3[2]); ?>" <?php if($row3[2] == $_GET["factory"]) echo "selected"; ?>> <?php echo h($row3[1]); ?>&nbsp;&nbsp</option>
                            <?php
                  }
				?>
                        </select></td>
	...
	//********** END CONDITION **************	
     ...
     $query8 = "SELECT *, DATE_FORMAT(MR.date_require,'%d-%m-%Y') AS R FROM consumable_request AS MR WHERE MR.status_request = 'Y' AND (MR.status != 'Close' AND MR.status != 'Cancel') ".$where_sql."  GROUP BY MR.temp_mrin"; $query8_args = [...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_require,'%d-%m-%Y') AS R FROM consumable_request AS MR WHERE MR.status_request = 'Y' AND (MR.status != 'Close' AND MR.status != 'Cancel') ".$where_sql." GROUP BY MR.temp_mrin ORDER BY MR.date_posting ASC, MR.temp_mrin ASC "; $query_args = [...$where_args];
...
?>
   ...
   while ($row2 = mysqli_fetch_array($rs))
   {
    ...
    <td width="50"><?php echo h($row2["factory"]); ?></td>
    <td width="50"><?php echo h($row2["id_work"]); ?></td>
    <td width="120"><div align="center"><?php echo h($row2["R"]); ?></div></td>
    <td><div align="center"><?php echo h($row2["time_require"]); ?></div></td>
    <td width="120"><?php echo h($data_u["user_fullname"]); ?></td>
    <td width="50"><a value="Details" href="close_MRIN_consumable_manual.php?mrin_no=<?php echo h($row2["temp_mrin"]); ?>&&user_no=<?php echo h($data_u["user_no"]); ?>&&TB_iframe=true&&height=400&&width=1000" class="thickbox" target="_self"><img src="../img/exclamation.png" width="16" height="16" alt="View" />Close</a></td>
    <td width="50"><a value="Details" href="detail_consumable_request.php?mrin_no=<?php echo h($row2["temp_mrin"]); ?>&&TB_iframe=true&&height=400&&width=1000" class="thickbox" target="_self"><img src="../img/icon_view.jpg" width="16" height="16" alt="View">View</a></td>
    <td width="50"><a href="detail_consumable_request_printing.php?mrin_no=<?php echo h($row2["temp_mrin"]); ?>&&TB_iframe=true&&height=400&&width=1000" class="thickbox" target="_self"><img src="../img/print.jpg" width="16" height="16" alt="Print">Print</a> </td>
  </tr>
