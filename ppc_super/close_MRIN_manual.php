...
/**
 * ppc_super/close_MRIN_manual.php
...
$query_2 = "SELECT *, DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2 from material_request as MR, scan_detail as SD WHERE MR.id_scan = SD.id_scan AND MR.temp_mrin = ?"; $query_2_args = [$temp_mrin];
	   ...
	   while ($row2 = mysqli_fetch_array($result_tp))
{
  ...
  $query4_p = "SELECT * from mat_master_header as LD, mat_master_detail as SD WHERE SD.id_hdr = LD.id_hdr and SD.id_dtl = ?"; $query4_p_args = [$row2["id_dtl"]];
 ...
 $query_upd3 = "UPDATE material_request SET status = 'Close', user_update = ?, date_update = NOW() WHERE temp_mrin = ?"; $query_upd3_args = [$user_no, $temp_mrin];
...
<br>
 ...
 $no = sprintf('%03d', $no);
   ...
   $query_scan = "SELECT * FROM scan_detail WHERE id_scan = ?"; $query_scan_args = [$row[6]];
   ...
   $query1_p = "SELECT * FROM scan_detail WHERE id_scan = ? GROUP BY id_scan"; $query1_p_args = [$row[6]];
	...
	} elseif($row4_p["mat_type"] == "Z210")	 
	{
	...
	} elseif($row4_p["mat_type"] == "Z310")
	 {
	  ...
	  $sta = "Invalid";
		...
		$query_tp = "SELECT *,SUM(rquantity) as TOT FROM post_detail_header WHERE mrin_no = ? AND prod_order = ? AND mvt_type = 311 AND material_no = ? AND status_posting != 'Cancel'"; $query_tp_args = [$row["temp_mrin"], $row_scan["prod_order"], $row4_p["bill_component"]];
	$result_tp  = db_query_bind($dbc, $query_tp, $query_tp_args); 
	...
	echo "invalid";  }   
     ...
     ?> 
         ...
         <p>&nbsp;</p>
            ...
            <form method="post" action="close_MRIN_manual.php?mrin_no=<?php echo h($temp_mrin); ?>&&prod_order=<?php echo h($prod_order); ?>" >
