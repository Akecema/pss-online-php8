...
/**
 * ppc_super/close_MRIN_ppc2.php
 * Part of: PPC module (supervisor/admin tier)
 * Filename suggests: close MRIN ppc2
 *
 * Behavior: requires an active login session ($_SESSION['username']); reads parameters from the query string ($_GET).
 * Database tables referenced: user_detail, sys_setup_maintain, factory_detail, work_center_detail, material_request, scan_detail, mat_master_header.
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_ppc_super.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="current">Material Request Maintenance</a> </div>
  <h1>Material Request Maintenance</h1>
</div>
...
<div class="container-fluid">
  ...
  <!--  <div class="span6">-->
    ...
    $query8 = "SELECT *, DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R FROM material_request AS MR, scan_detail AS SD WHERE MR.id_scan = SD.id_scan AND MR.status_request = 'Y' AND (MR.status != 'Close' AND MR.status != 'Cancel')".$where_sql." GROUP BY MR.temp_mrin"; $query8_args = [...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R FROM material_request AS MR, scan_detail AS SD WHERE MR.id_scan = SD.id_scan AND MR.status_request = 'Y' AND (MR.status != 'Close' AND MR.status != 'Cancel')".$where_sql." GROUP BY MR.temp_mrin ORDER BY MR.date_posting ASC, MR.temp_mrin ASC "; $query_args = [...$where_args];
  ...
  <tr>
    ...
    <td width="50"><?php echo h($row_scan["factory"]); ?></td>
     <td width="50" height="28"><?php echo h($row_scan["work_center"]); ?></td>
              ...
              <td width="50"><a value="Details" href="close_MRIN_manual.php?mrin_no=<?php echo h($row2["temp_mrin"]); ?>&&prod_order=<?php echo h($row2["prod_order"]); ?>&&TB_iframe=true&&height=400&&width=1000" class="thickbox" target="_self"><img src="../img/exclamation.png" width="16" height="16" alt="View" />Close</a></td>
            ...
            <td width="50"><a value="Details" href="detail_material_request.php?mrin_no=<?php echo h($row2["temp_mrin"]); ?>&&prod_order=<?php echo h($row2["prod_order"]); ?>&&TB_iframe=true&&height=400&&width=1000" class="thickbox" target="_self"><img src="../img/icon_view.jpg" width="16" height="16" alt="View">View</a></td>
                <td width="50"><a href="detail_material_request_printing.php?mrin_no=<?php echo h($row["temp_mrin"]); ?>&&prod_order=<?php echo h($row2["prod_order"]); ?>&&TB_iframe=true&&height=400&&width=1000" class="thickbox" target="_self"><img src="../img/print.jpg" width="16" height="16" alt="Print">Print</a> </td>
  </tr>  
  <?php 
