...
/**
 * ppc_super/list_ftp_pending_sap.php
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_ppc_super.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a><a href="#" class="#">FTP Monitoring</a> <a href="#" class="current">Backflush FTP Monitor</a></div>
...
if(is_dir($dir)){
  ...
  // $total2 = count($dh); removed - count() on a directory resource/handle throws a fatal TypeError under PHP 8 (was PHP7: warning + returned 1). $total2 was assigned but never read anywhere in this file, so the line was just dropped rather than wrapped.
   ...
   <?php
	...
	$query = "SELECT *,DATE_FORMAT(posting_date,'%d-%m-%Y') AS R2,DATE_FORMAT(date_create,'%d-%m-%Y %H:%i:%s') AS R3 FROM ftp_bflush_detail WHERE status_ftp = 'Y' AND file_name = ?"; $query_args = [$filename2];
   ...
   <table class="table">
    ...
    <td width="10%"><?php echo h($row_rs["R2"]);  ?></td>
