...
/**
 * ppc_super/material_request_analysisProc.php
 * Part of: PPC module (supervisor/admin tier)
 * Filename suggests: material request analysisProc
 *
 * Behavior: requires an active login session ($_SESSION['username']); reads parameters from the query string ($_GET).
 * Database tables referenced: user_detail, sys_setup_maintain, factory_detail, work_center_detail, mat_master_detail, mat_master_header, material_request, scan_detail, post_detail_header.
...
<div id="content">
 <div id="content-header" class="noprint">
  <div id="breadcrumb"> <a href="index_ppc_super.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="tip-bottom">Material Request</a> <a href="#" class="current">Material Request Analysis</a> </div>
 ...
 <form action="<?php echo h($_SERVER['PHP_SELF']); ?>" method="get" name="frmSearch" id="frmSearch">
         <table class="table table-bordered table-striped">
            ...
            <tr>
             <th>Material No.  :</th>
             <td><select name="material_no" id="material_no" class="span11">
   ...
   $query8 = "SELECT *,DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') AS R2 FROM material_request AS MR, scan_detail AS SD WHERE MR.id_scan = SD.id_scan AND MR.status_request = 'Y' AND MR.status != 'Cancel'" .$where_sql; $query8_args = [...$where_args];
...
$query = "SELECT *,DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') AS R2 FROM material_request AS MR, scan_detail AS SD WHERE MR.id_scan = SD.id_scan AND MR.status_request = 'Y' AND MR.status != 'Cancel'".$where_sql; $query_args = [...$where_args];
...
<tr>
    ...
    <td width="85%"> <div class="small-nav"></div></td> 
      <td width="7%"><a href="report_material_request_analysisProc.php?factory=<?php echo h($factory); ?>&&work_center=<?php echo h($work_center); ?>&&status=<?php echo h($status); ?>&&date1=<?php echo h($dateF); ?>&&date2=<?php echo h($dateT); ?>&&material_no=<?php echo h($material_no); ?>"><img src="../img/dload_excel.jpg" width="48" height="48" title="Download" /></a></td>
     <td width="7%"><img src="../img/print2.jpg" width="48" height="48" onClick="window.print()" title="Print"/></td>
        ...
        <div class="widget-box">
          <div class="widget-title"> <span class="icon"><i class="icon-th"></i></span>
            <h5>Display Material Request Analysis</h5>
          </div>
     ...
     <th data-field="name" data-sortable="true">Required <i class="icon-sort"></i><br />Date</th>
     <th>Required Time</th>
     <th>Duration</th>
     <th>Status</th>
     </tr>
   ...
   while ($row2 = mysqli_fetch_array($rs))
   {
    ...
    $query5 = "SELECT * FROM post_detail_header WHERE mrin_no = ? AND material_no = ? AND mvt_type = 311 AND prod_order = ?"; $query5_args = [$row2["temp_mrin"], $row2["bom_component"], $row_scan["prod_order"]];
    $result5 = db_query_bind($dbc, $query5, $query5_args);
	$row5 = mysqli_fetch_array($result5);
	
	$query6 = "SELECT * FROM post_detail_header AS PD, material_request AS MR WHERE PD.mrin_no = MR.temp_mrin AND PD.material_no = MR.bom_component AND PD.mrin_no = ? AND PD.material_no = ? AND PD.mvt_type = 311"; $query6_args = [$row2["temp_mrin"], $row2["bom_component"]];
...
//------------------------------------------------------------------------------------------------------------------	
    ...
    $query_tp = "SELECT *,SUM(rquantity) as TOT FROM post_detail_header WHERE mrin_no = ? AND prod_order = ? AND mvt_type = 311 AND material_no = ?"; $query_tp_args = [$row2["temp_mrin"], $row_scan["prod_order"], $row4_p["bill_component"]];
  ...
  <tr>
    ...
    <td width="50"><div align="center"><?php echo h($row2["bom_oum"]); ?></div></td>
    <td width="90"><div align="right"><?php echo h($row2["bom_qty"]); ?></div>
   </td>
    <td width="90"><div align="right"><?php echo h($row5["rquantity"]); ?></div></td>
    <td width="90"><?php if($variance_qty2 < 0 ) { echo "<font color='red'>";  echo h($variance_qty2);  echo "</font>"; }else{ echo h($variance_qty2); } ?></td>
