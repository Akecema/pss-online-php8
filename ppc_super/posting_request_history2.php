...
/**
 * ppc_super/posting_request_history2.php
 * Part of: PPC module (supervisor/admin tier)
 * Filename suggests: posting request history2
 *
 * Behavior: requires an active login session ($_SESSION['username']); reads parameters from the query string ($_GET).
 * Database tables referenced: user_detail, sys_setup_maintain, factory_detail, work_center_detail, material_request_close, scan_detail, mat_master_header.
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_ppc_super.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="tip-bottom">Material Request</a> <a href="#" class="current">Material Request History</a> </div>
  ...
  <!--  <div class="span6">-->
    ...
    //-------Count all results------------------------//
	 ...
	 $where_sql =  $wheresql_01 .$wheresql_02 .$wheresql_03 .$wheresql_04 .$wheresql_05 .$wheresql_06; $where_args = array_merge($wheresql_01_args, $wheresql_02_args, $wheresql_03_args, $wheresql_04_args, $wheresql_05_args, $wheresql_06_args);	
      ...
      $query8 = "SELECT *, DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R FROM material_request_close AS MR, scan_detail AS SD WHERE MR.id_scan = SD.id_scan AND MR.status_request = 'Y' AND MR.status = 'Close' ".$where_sql." GROUP BY MR.temp_mrin"; $query8_args = [...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R FROM material_request_close AS MR, scan_detail AS SD WHERE MR.id_scan = SD.id_scan AND MR.status_request = 'Y' AND  MR.status = 'Close' ".$where_sql." GROUP BY MR.temp_mrin ORDER BY MR.date_posting ASC, MR.temp_mrin ASC "; $query_args = [...$where_args];
...
?>
   ...
   while ($row2 = mysqli_fetch_array($rs))
   {
	...
	$query_again = "SELECT * FROM material_request_close WHERE status_request = 'Y' and id_scan = ? ORDER BY id_req ASC"; $query_again_args = [$row2[6]];
...
$since_start = $start_date->diff(new DateTime($date_transfer));
		  ...
		  ?>        
  <tr>
    <td width="143">&nbsp;<?php echo h($row["temp_mrin"]); ?><?php if($row2["status_urgent"] == "Y") 
	{
	?>
