...
/**
 * ppc_super/posting_request_scan_+_urgent2.php
 * Part of: PPC module (supervisor/admin tier)
 * Filename suggests: posting request scan + urgent2
 *
 * Behavior: requires an active login session ($_SESSION['username']); reads parameters from the query string ($_GET).
 * Database tables referenced: user_detail, sys_setup_maintain, factory_detail, work_center_detail, material_request, scan_detail, material_request_cancel.
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_ppc_super.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="tip-bottom">Material Request</a> <a href="#" class="current">Display Material Request</a> </div>
...
<div class="container-fluid">
  ...
  <!--  <div class="span6">-->
          ...
          <form action="<?php echo h($_SERVER['PHP_SELF']); ?>" method="get" name="frmSearch" id="frmSearch">
            <table class="table table-bordered table-striped">
              <tr>
                ...
                <tr>
                  <th>MRIN No :</th>
                  <td><input name="temp_mrin" type="text" id="temp_mrin" size="25" class="span11" value="<?php echo h($_GET["temp_mrin"]); ?>" />
                  </td>
                  <th>Production Order :</th>
                  <td ><input name="prod_order" type="text" id="prod_order" size="25" class="span11" value="<?php echo h($_GET["prod_order"]); ?>" />
                  </td>
                </tr>
                <tr>
                  <th>Factory :</th>
     ...
     <?php
		 ...
		 // 3. Temp MRIN
                if ($temp_mrin == "" ){
                    ...
                    $wheresql_03 = " AND MR.temp_mrin = ?"; $wheresql_03_args = [$temp_mrin]; }          
   ...
   $query8 = "SELECT *, DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') AS R2 FROM material_request AS MR, scan_detail AS SD WHERE MR.id_scan = SD.id_scan AND MR.status_request = 'Y' ".$where_sql."  GROUP BY MR.temp_mrin ORDER BY MR.id_req ASC"; $query8_args = [...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') AS R2 FROM material_request AS MR, scan_detail AS SD WHERE MR.id_scan = SD.id_scan AND MR.status_request = 'Y' ".$where_sql."  GROUP BY MR.temp_mrin ORDER BY MR.date_posting ASC, MR.temp_mrin ASC"; $query_args = [...$where_args];
...
?>
   ...
   $row_scan = mysqli_fetch_array($result_scan);
	...
	$query_again = "SELECT * FROM material_request WHERE status_request = 'Y' and id_scan = ?"; $query_again_args = [$row2[6]];
	...
	//------------------------------
	    ...
	    $query_check = "SELECT * FROM material_request WHERE temp_mrin = ? AND status != 'New'"; $query_check_args = [$row2["temp_mrin"]];
	   ...
	   $stat = '<font color="#009900"><b>Open</b></font>';
	    ...
	    $query_check2 = "SELECT * FROM material_request_cancel WHERE temp_mrin = ?"; $query_check2_args = [$row2["temp_mrin"]];
  ...
  <tr>
    ...
    <td width="50"><a value="Details" href="detail_material_request.php?mrin_no=<?php echo h($row2["temp_mrin"]); ?>&&prod_order=<?php echo h($row2["prod_order"]); ?>&&TB_iframe=true&&height=400&&width=1000" class="thickbox" target="_self"><img src="../img/icon_view.jpg" width="16" height="16" alt="View">View</a></td>
    <td width="50"><a href="detail_material_request_printing.php?mrin_no=<?php echo h($row2["temp_mrin"]); ?>&&prod_order=<?php echo h($row2["prod_order"]); ?>&&TB_iframe=true&&height=400&&width=1000" class="thickbox" target="_self"><img src="../img/print.jpg" width="16" height="16" alt="Print">Print</a> </td>
  </tr>
  <?php 
