...
// $prod_order = $_GET["prod_order"];
 ...
 $reason_cancel2 = $_POST["reason_cancel2"];
	...
	{
$queryu2 = "SELECT * from consumable_request WHERE temp_mrin = ?"; $queryu2_args = [$temp_mrin];
$rs2 = db_query_bind($dbc, $queryu2, $queryu2_args);   //run the query.


$query_2a = "SELECT *, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R from consumable_request as MR, consumable_detail as SD WHERE MR.id_con = SD.id_con AND MR.temp_mrin = ?"; $query_2a_args = [$temp_mrin];
$result_2a = db_query_bind($dbc, $query_2a, $query_2a_args);   //run the query.
$data_2a = mysqli_fetch_array($result_2a);

    $query3a = "SELECT * FROM factory_detail WHERE id_fac = ?"; $query3a_args = [$data_2a["factory"]];
    $result3a = db_query_bind($dbc, $query3a, $query3a_args);
	$row3a = mysqli_fetch_array($result3a);


$query_tp = "SELECT *, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') AS R FROM consumable_request AS MR, consumable_detail AS SD WHERE MR.id_con = SD.id_con AND MR.temp_mrin = ? AND MR.status = 'New'"; $query_tp_args = [$temp_mrin];
	   ...
	   while ($row2 = mysqli_fetch_array($result_tp))
{
 ...
 $query_upd3 = "UPDATE consumable_request SET status = 'Cancel', user_update = ?, date_update = NOW() WHERE temp_mrin = ? AND id_con = ? "; $query_upd3_args = [$user_no, $temp_mrin, $row2["id_con"]];
...
?>
 ...
 //------------------cost center --------------------//
   ...
   $row_cost_center = mysqli_fetch_array($result_cost_center);
        ...
        <td width="200"><?php  echo h($row["mat_desc"]); ?></td>
        <td width="100"><div align="right"><?php echo h($row["con_qty"]); ?>&nbsp;</div></td>
        <td width="65" ><div align="center"><?php echo h($row["con_uom"]); ?></div></td>
        ...
        <select name="reason_cancel" id="reason_cancel" onchange="ShowReg(this.selectedIndex);">
         <?php if(isset($_POST["reason_cancel"])) { 
				  ...
				  $query_r_1 = "SELECT * FROM reason_req_cancel WHERE id_cancel = ?"; $query_r_1_args = [$_POST["reason_cancel"]];
         ...
         ?>
         <option value="<?php echo h($_POST["reason_cancel"]); ?>"><?php echo h($row_r_1["id_cancel"]).' - '.h($row_r_1["reason_desc_cancel"]); ?></option>
        <?php
        ...
        <?php
           ...
           <br>
            <input type="submit" onClick="return confirm('Are you sure you want to CANCEL MRIN No. : <?php echo h($temp_mrin); ?>?');" name="submit" id="button" value="CANCEL MRIN" class="btn btn-danger"/>
            &nbsp;
