...
function confirmation() {
    var answer = confirm("Are you sure you want to CANCEL MRIN No. : <?php echo h($temp_mrin); ?>?")
    if (answer){
        alert("MRIN Mo. <?php echo h($temp_mrin); ?> is CANCEL !")
		
	top.frames['mainFrame'].location.href = "cancel_material_requestProc.php?mrin_no=<?php echo h($temp_mrin); ?>&&prod_order=<?php echo h($prod_order); ?>&&user_no=<?php echo h($data_u["user_no"]); ?>";
    }
    else{
	   ...
	   while ($row2 = mysqli_fetch_array($result_tp))
{
 ...
 $query_upd3 = "UPDATE `material_request` SET status = 'Cancel', user_update = ?, date_update = NOW() WHERE temp_mrin = ? AND bom_component = ? "; $query_upd3_args = [$user_no, $temp_mrin, $row4_p["bill_component"]];
 $result_upd3 = db_query_bind($dbc, $query_upd3, $query_upd3_args); 
		  ...
		  //-----------------------------------------------------------------------------------------------------------------------
		  	...
		  	$query_mm3 = "SELECT * FROM `material_request` WHERE temp_mrin = ? AND status = 'Cancel' AND bom_component = ?"; $query_mm3_args = [$temp_mrin, $row4_p["bill_component"]]; 
...
<p align="center" class="style4">&nbsp;</p>
       <table class="table table-bordered">
        <tr>
          ...
          <td width="322"><span class="style3"><?php echo h($temp_mrin); ?></span></td>
...
<p align="center" class="style4">&nbsp;</p>
      ...
      <!-- Content -->
              ...
              <!-- End Box Head -->
          <form action="cancel_material_request.php?mrin_no=<?php echo h($temp_mrin); ?>&&prod_order=<?php echo h($prod_order); ?>" method="post" >
            <!-- Form -->
            <div class="form">
...
<table width="1000" border="0" cellpadding="1" cellspacing="2">
         ...
         <input type="submit" onClick="return confirm('Are you sure you want to CANCEL MRIN No. : <?php echo h($temp_mrin); ?>?');" name="submit" id="button" value="CANCEL MRIN" class="button"/>
