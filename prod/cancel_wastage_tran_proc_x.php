...
/**
 * prod/cancel_wastage_tran_proc_x.php
 * Part of: Production module
 * Filename suggests: cancel wastage tran proc x
 *
...
$message = NULL; // create an empty new variable.
   ...
   // echo $uid;
    ...
    $query_upd_detail = "UPDATE reject_detail_disposal SET status_disposal = ? WHERE id_disposal = ?"; $query_upd_detail_args = [$rst_sta16["status_desc"], $uid];
	$result_upd_detail = db_query_bind($dbc, $query_upd_detail, $query_upd_detail_args) or die(db_fail($dbc));
	
	//-----------select statement--------------
	$query_state = "SELECT * FROM reject_detail_disposal WHERE id_disposal = ? ORDER BY id_disposal ASC"; $query_state_args = [$uid];
    $result_state = db_query_bind($dbc, $query_state, $query_state_args);
    $row_state = mysqli_fetch_array($result_state);
	

	//---------------
	  $query_upd_detail2 = "UPDATE wastage_transaction SET status_disposal = ? WHERE id_wastage_tran = ?"; $query_upd_detail2_args = [$rst_sta16["status_desc"], $row_state["uid"]];
...
<p>&nbsp;</p> 
   <form action="cancel_wastage_tran_proc.php?uid=<?php echo h($uid); ?>" method="post" name="frmSearch" id="frmSearch">     
  <table class="table table-bordered data-table">
   ...
   $query_display = "SELECT *, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2 FROM reject_detail_disposal AS MR WHERE MR.id_disposal = ?"; $query_display_args = [$uid];
