...
/**
 * prod/cancellation_list_backflush_tran.php
 * Part of: Production module
 * Filename suggests: cancellation list backflush tran
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, factory_detail, pps_detail, pps_detail_transaction, qqc_detail_transaction.
    ...
    <!--  <div class="span12">-->
      ...
      <form action="<?php echo h($_SERVER['PHP_SELF']); ?>" method="post" name="frmSearch" id="frmSearch">
	       ...
	       $query3 = "SELECT * FROM factory_detail GROUP BY factory_desc2 ORDER BY id_fac ASC";
			      ...
			      {
                  echo'<option value="',h($row3['factory_desc2']),'">',stripslashes($row3['factory_desc']),'</option>';
                  }
				?>
              </select></td>
              <th>Work Center :</th>
...
echo "window.location='cancellation_list_backflush_tran2.php?date1=$dateF&&date2=$dateT&&factory=$factory&&work_center=$work_center&&plan_no=$plan_no&&shift_ops=$shift_ops'";
 ...
 // echo $end_date_check;
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail_transaction WHERE status_pps = ? AND (date_plan >= '$start_date_check' AND date_plan <= '$end_date_check')"; $query8_args = [$rst_sta7["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction WHERE status_pps = ? AND (date_plan >= '$start_date_check' AND date_plan <= '$end_date_check') ORDER BY plan_no ASC"; $query_args = [$rst_sta7["status_desc"]];
...
?>
   ...
   while ($row = mysqli_fetch_array($rs))
   {
	...
	//-----checking QA/QC entering output production ----not allow cancellation-------
	   $query_qqc = "SELECT * FROM qqc_detail_transaction WHERE bflush_no = ? AND status_QC != ?"; $query_qqc_args = [$row["bflush_no"], $rst_sta4["status_desc"]];
	   ...
	   $query_prod_out = "SELECT * FROM pps_detail WHERE plan_no = ? AND status_pps = ?"; $query_prod_out_args = [$row["plan_no"], $rst_sta2["status_desc"]];
	   $result_prod_out = db_query_bind($dbc, $query_prod_out, $query_prod_out_args);
	...
	}elseif($sta_out == "221")
	{
      ...
      ?>
                ...
                <td width="80"><font color="#0000CC"><?php echo h($row["plan_no"]); ?></font></td>
                ...
                <td width="90"><font color="#0000CC"><?php echo h($row["bflush_no"]); ?></font></td>
				...
				?>
                  <a value="Cancellation" href="cancel_backflush_tran_proc.php?buid=<?php echo h($row["id"]); ?>&&TB_iframe=true&height=400&width=1000" class="thickbox" target="_self"><img src="../img/delete.png" width="16" height="16" alt="Cancellation">Cancellation</a><?php
				   }elseif($sta_out == "221")
				   {
					   ?>
				     <a value="Cancellation" href="cancel_backflush_NG_tran_proc.php?buid=<?php echo h($row["id"]); ?>&&TB_iframe=true&height=400&width=1000" class="thickbox" target="_self"><img src="../img/delete.png" width="16" height="16" alt="Cancellation">Cancellation</a>
