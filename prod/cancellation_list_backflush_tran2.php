...
/**
 * prod/cancellation_list_backflush_tran2.php
 * Part of: Production module
 * Filename suggests: cancellation list backflush tran2
 *
 * Behavior: requires an active login session ($_SESSION['username']); reads parameters from the query string ($_GET).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, factory_detail, work_center_detail, pps_detail, pps_detail_transaction, qqc_detail_transaction.
...
<?php
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail_transaction AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND status_pps = ?".$where_sql; $query8_args = [$rst_sta7["status_desc"], ...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status_pps = ?".$where_sql." ORDER BY MR.plan_no ASC"; $query_args = [$rst_sta7["status_desc"], ...$where_args];
