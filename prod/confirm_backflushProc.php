...
<div class="container-fluid">
 ...
 <?php
	   ...
	   $query_scan = "SELECT * FROM scan_prod_planning WHERE id_scan = ?"; $query_scan_args = [$buid];
   ...
   else
   {
	...
	//echo $ref;
	 ...
	 //--------- pps detail ------------
	   ...
	   $query_pps = "SELECT * FROM pps_detail WHERE plan_no = ?"; $query_pps_args = [$data_scan["plan_no"]];
...
$query_upd_detail2 = "UPDATE pps_detail_transaction SET shift_posting = ? WHERE id = ?"; $query_upd_detail2_args = [$shif_p, $data_all_info["id"]];
$result_upd_detail2 = db_query_bind($dbc, $query_upd_detail2, $query_upd_detail2_args);  
  ...
  //-------update status "Released" to "Inprogress" in table pps_detail	
	...
	$query_upd_detail = "UPDATE pps_detail SET status_pps = ? WHERE id = ?"; $query_upd_detail_args = [$rst_sta7["status_desc"], $buid];
  ...
  //----edit by azie 17 nov 2021 night shift ------	
	...
	$query_upd_shift = "SELECT * FROM pps_detail_transaction WHERE id = ?"; $query_upd_shift_args = [$data_all_info["id"]];
  $result_upd_shift = db_query_bind($dbc, $query_upd_shift, $query_upd_shift_args);
	...
	$query_upd_shift2 = "UPDATE pps_detail_transaction SET date_posting = '".$prev_date."' WHERE id = ?"; $query_upd_shift2_args = [$row_upd_shift["id"]];
  ...
  //-------insert table print_tag_backflush [generate print tag] ---------------	
	  ...
	  $query_tag = "SELECT * FROM pps_detail_transaction WHERE id = ?"; $query_tag_args = [$data_all_info["id"]];
    $result_tag = db_query_bind($dbc, $query_tag, $query_tag_args);
		...
		if($row["shift_pps1"] != "")
	{
	...
	}elseif($row["shift_pps2"] != "")
	 {
	  ...
	  //----detail standard packaging [ambil dari table mat_master_header]
	   ...
	   $query_pack = "SELECT std_package, type_package FROM mat_master_header WHERE material_no = ?"; $query_pack_args = [$row["material_no"]];
	   $result_pack = db_query_bind($dbc, $query_pack, $query_pack_args);
...
$query_tag3 = "INSERT INTO print_tag_backflush(id_tag,tag_no,id_tran,bflush_no,plan_no,rev_plan_no,material_no, material_desc,tag_qty, shift_tag,ploc,station_loc,posting_date,posting_time,user_create,date_create,status_tag,status_print,month_plan,year_plan,slip_no,total_slip)  VALUES('','',?,?,?,'',?,?,?,?,?,?,?,?,?,NOW(),'N','N',?,NOW(),?,?)"; $query_tag3_args = [$row["id"], $row["bflush_no"], $row["plan_no"], $row["material_no"], $row["material_desc"], $st_pack, $row["shift_posting"], $row["ploc"], $row["work_center"], $row["date_posting"], $row["time_posting"], $username, $row["month_plan"], $w, $no_tg];
	   $result_tag3 = db_query_bind($dbc, $query_tag3, $query_tag3_args);
	   
	     $tag_no = ($row["bflush_no"].'/'.$w.'/'.$data_pack["std_package"].'/'.$no_tg);
		
		 
		 $query_tag3_t = "UPDATE print_tag_backflush SET tag_no = ? WHERE id_tag = '".mysqli_insert_id($dbc)."' AND bflush_no = ?"; $query_tag3_t_args = [$tag_no, $ref];
	     $result_tag3_t = db_query_bind($dbc, $query_tag3_t, $query_tag3_t_args);
	 ...
	 $query_tag2 = "INSERT INTO print_tag_backflush(id_tag,tag_no,id_tran,bflush_no,plan_no,rev_plan_no,material_no, material_desc,tag_qty, shift_tag,ploc,station_loc,posting_date,posting_time,user_create,date_create,status_tag,status_print,month_plan,year_plan,slip_no,total_slip)  VALUES('','',?,?,?,'',?,?,?,?,?,?,?,?,?,NOW(),'N','N',?,NOW(),?,?)"; $query_tag2_args = [$row["id"], $row["bflush_no"], $row["plan_no"], $row["material_no"], $row["material_desc"], $bil_tag3, $row["shift_posting"], $row["ploc"], $row["work_center"], $row["date_posting"], $row["time_posting"], $username, $row["month_plan"], $w, $no_tg]; 
	   $result_tag2 = db_query_bind($dbc, $query_tag2, $query_tag2_args);
	   

       
	     $tag_no = ($row["bflush_no"].'/'.$w.'/'.$bil_tag3.'/'.($b + 1));
		 
		 $query_tag2_t = "UPDATE print_tag_backflush SET tag_no = ? WHERE id_tag = '".mysqli_insert_id($dbc)."' AND bflush_no = ?"; $query_tag2_t_args = [$tag_no, $ref];
	     $result_tag2_t = db_query_bind($dbc, $query_tag2_t, $query_tag2_t_args);
...
$result_upd_shift2 = db_query_bind($dbc, $query_upd_shift2, $query_upd_shift2_args);  	
  ...
  //-------update status "Released" to "Inprogress" in table pps_detail	
	...
	$query_upd_detail = "UPDATE pps_detail SET status_pps = ? WHERE id = ?"; $query_upd_detail_args = [$rst_sta7["status_desc"], $data_all_info["pps_id"]];
	$result_upd_detail = db_query_bind($dbc, $query_upd_detail, $query_upd_detail_args) or die(db_fail($dbc));
