...
$sql = "SELECT * FROM wip_request AS MR, scan_detail AS SD WHERE MR.id_scan_wip = SD.id_scan AND MR.status_request = 'Y' AND (MR.status != 'Close' AND MR.status != 'Cancel') AND (MR.date_mrin >= '".db_esc($dbc, $start_date_check)."' AND MR.date_mrin <= '".db_esc($dbc, $end_date_check)."') GROUP BY MR.temp_mrin_wip";
