...
/**
 * prod/detail_material_request.php
 ...
 $no = sprintf('%03d', $no);
  ...
  $row4_p = mysqli_fetch_array($result4_p); 
	...
	} elseif($row4_p["mat_type"] == "Z210")	 
	{
	...
	} elseif($row4_p["mat_type"] == "Z310")
	 {
	  ...
	  $sta = "Invalid";
	    ...
	    $query_check = "SELECT * FROM post_detail_header WHERE mrin_no = ? AND mat_doc_year = ? AND status_posting = 'Close'"; $query_check_args = [$row["temp_mrin"], $row["mrin_year"]];
		$result_check = db_query_bind($dbc, $query_check, $query_check_args);		
 	    $rst_check = mysqli_fetch_array($result_check);
		
		//echo h($rst_check["id_post"]);
		//echo h($rst_check["rquantity"]);
	...
	?>
                ...
                <td width="100"><div align="right"><?php echo h($row["bom_qty"]); ?>&nbsp;</div></td>
                ...
                <td width="90">&nbsp;
                <?php
				if($rst_check > 0){ echo h($rst_check["rquantity"]); } else{ }
				
				?>
                </td>
                <td width="65"><div align="center"><font color="#FF0000"><?php echo h($row4_p["isloc"]); ?></font></div></td>
                <td>
                  <div align="left">
                    <?php
