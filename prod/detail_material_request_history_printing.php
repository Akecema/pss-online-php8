	...
	} elseif($row4_p["mat_type"] == "Z210")	 
	{
	...
	} elseif($row4_p["mat_type"] == "Z310")
	 {
	  ...
	  $sta = "Invalid";
		...
		$query_tp = "SELECT *,SUM(rquantity) as TOT FROM post_detail_header WHERE mrin_no = ? AND prod_order = ? AND mvt_type = 311 AND material_no = ?"; $query_tp_args = [$row["temp_mrin"], $row_scan["prod_order"], $row4_p["bill_component"]];
...
?>
    ...
    <th width="3%" >:</th>
    <th width="84%" ><?php echo h($row_reason_tbl["reason_desc_cancel"]).' - ' . h($row_display_reason["reason_cancel2"]); ?>  </th>
    </tr>
    </thead>
    </table>
<?php
   ...
   <th width="13%">Reason</th>
   <th width="3%">:</th>
   <th width="84%"><?php echo h($row_reason_tbl2["reason_desc"]).' - ' . h($row_display_reason2["reason_close2"]); ?></th>
  </tr>
  </thead>
  </table>
<?php
