...
/**
 * prod/dis_request.php
 * Part of: Production module
 * Filename suggests: dis request
 *
...
$query_id_2 = "SELECT MAX(mrin_doc) FROM material_request";
...
echo '<font color="red" class ="error_entry">', $message, '</font>';
   ...
   $query8 = "SELECT COUNT(*) FROM material_request WHERE status_request = 'N' AND user_create = ? AND ((date_mrin >= '$current_date') AND (date_mrin <= '$next_date')) ORDER BY id_req ASC"; $query8_args = [$res["user_no"]];
...
$query = "SELECT *, DATE_FORMAT(date_mrin,'%d-%m-%Y') AS R FROM material_request WHERE status_request = 'N' AND user_create = ? AND ((date_mrin >= '$current_date') AND (date_mrin <= '$next_date')) GROUP BY id_scan ORDER BY id_req ASC"; $query_args = [$res["user_no"]];
...
<br />
   ...
   <td><div align="center">
    ...
    <tbody>
         <?php
	$query_again = "SELECT * FROM material_request WHERE status_request = 'N' and id_scan = ? AND status != 'Cancel' ORDER BY id_req ASC "; $query_again_args = [$row2["id_scan"]];
