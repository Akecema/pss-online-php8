...
echo "window.location='display_cancel_list_backflush_tran2.php?date1=$dateF&&date2=$dateT&&factory=$factory&&work_center=$work_center&&bflush_no=$bflush_no&&shift_ops=$shift_ops'";
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail_transaction WHERE status_pps = ?"; $query8_args = [$rst_sta4["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction WHERE status_pps = ? ORDER BY plan_no ASC"; $query_args = [$rst_sta4["status_desc"]];
