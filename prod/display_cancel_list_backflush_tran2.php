    ...
    <!--  <div class="span12">-->
     ...
     <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get" name="frmSearch" id="frmSearch">
            ...
            $result9 = mysqli_query($dbc, $query9);
			      ...
			      {
				   ?>
                  <option value="<?php echo $row9["bflush_no"]; ?>"<?php if($row9["bflush_no"] == $_GET["bflush_no"]) echo "selected"; ?>> <?php echo $row9["bflush_no"]; ?></option>
                  <?php
                  }
				?>
              </select>
...
<?php
          ...
          //4. Work Center
	       ...
	       //5. Backflush Doc. No.
                if ($bflush_no == "NULL"){ 
                    ...
                    $wheresql_05 = " AND MR.bflush_no = '$bflush_no'"; }  	
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail_transaction AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status_pps = '".$rst_sta4["status_desc"]."'".$where_sql;
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status_pps = '".$rst_sta4["status_desc"]."'".$where_sql."ORDER BY MR.plan_no ASC";
   ...
   while ($row = mysqli_fetch_array($rs))
   {
	...
	}elseif($sta_out == "3")
	{
      ...
      ?>
                ...
                <td width="90"><font color="#FF0000"><?php echo $row[40]; ?></font></td>
                <td width="60"><?php echo $qty_final; ?></td>
                <td width="99"><a value="Cancellation Details" href="detail_cancel_backflush_tran_proc.php?uid=<?php echo $row["id"]; ?>&&TB_iframe=true&height=400&width=1000" class="thickbox" target="_self"><img src="../img/icon_view.jpg" width="16" height="16" alt="Cancellation Details">View</a></td>
                </tr>
