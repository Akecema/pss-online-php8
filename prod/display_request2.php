...
/**
 * prod/display_request2.php
 * Part of: Production module
 * Filename suggests: display request2
 *
...
echo '<font color="red" class ="error_entry">', $message, '</font>';
 ...
 $current_date = (date("Y-m-d"));
   ...
   $query8 = "SELECT COUNT(*) FROM material_request as MR, mat_master_detail as MD, scan_detail as SD WHERE MR.id_scan = SD.id_scan AND MD.id_dtl = MR.id_dtl AND SD.status_urgent = 'N' AND MR.status_request = 'N' AND ((MD.material LIKE ?) OR (MD.bill_component LIKE ?)) AND MR.user_create = ? AND ((MR.date_mrin >= '$current_date') AND (MR.date_mrin <= '$next_date')) GROUP BY MR.id_scan ORDER BY MR.id_req ASC"; $query8_args = ['%' . $_GET["txtKeyword"] . '%', '%' . $_GET["txtKeyword"] . '%', $res["user_no"]];
...
<br />
  ...
  <tr>
    ...
    <td width="156">&nbsp;<?php echo h($row_scan["material_no"]); ?></td>
    <td width="220">&nbsp;<font color="#0000CC"><b><?php echo h($row_scan["prod_order"]); ?></b></font></td>
    <td width="220"><font color="#FF0000"><?php echo h($row2["R"]); ?>&nbsp;<?php echo h($row2["time_mrin"]); ?></font></td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    </tr>
</thead><tbody>
         ...
         <?php
	$query_again = "SELECT * FROM material_request WHERE status_request = 'N' and id_scan = ? ORDER BY id_req ASC "; $query_again_args = [$row2[6]];
  ...
  $row4_p = mysqli_fetch_array($result4_p); 
         ...
         <td width="144" height="28"><div align="right">
	 <input name="bom_qty[<?php echo $i; ?>]" type="text" value="<?php if(isset($_POST["bom_qty"])) { echo h($_POST["bom_qty"][$i]); }else{  echo h($row["bom_qty"]);   } ?>" class="span8"/>	
				
				&nbsp;</div></td>
                <td width="80" height="28"><div align="center"><?php echo h($row["bom_oum"]); ?></div></td>
                <td width="100" height="28"><div align="center"><font color="#FF0000"><?php echo h($row1_p["work_center"]); ?></font></div></td>
