...
/**
 * prod/display_tag_backflush_ts.php
 * Part of: Production module
 * Filename suggests: display tag backflush ts
 *
...
$query_pps = "SELECT *,DATE_FORMAT(posting_date,'%d-%m-%Y') as R FROM print_tag_backflush WHERE id_tran = ?"; $query_pps_args = [$uid];
...
{
  //----------display model 
	$query_info = "SELECT * FROM pps_detail WHERE plan_no = ?"; $query_info_args = [$row["plan_no"]];
	$result_info = db_query_bind($dbc, $query_info, $query_info_args);
	$row_info = mysqli_fetch_array($result_info);
	
	//----------display mat_type 
	$query_info2 = "SELECT * FROM pps_detail_transaction WHERE plan_no = ? AND ref_id = ?"; $query_info2_args = [$row_info["plan_no"], $row_info["ref_id"]];
	...
	$query_info3 = "SELECT * FROM mat_master_header WHERE material_no = ?"; $query_info3_args = [$row["material_no"]];
...
<table width="880" height="458" border="1" cellpadding="0" cellspacing="0">
  <tr>
    <td height="45" colspan="2" valign="top"><p><img src="../set_upload/<?php echo h($filename);  ?>"width="267" height="27" hspace="2" vspace="2"/></p>
   </td>
   ...
   <tr>
	...
	$query = "SELECT *,DATE_FORMAT(date_create,'%d-%m-%Y') as Q, DATE_FORMAT(date_posting,'%d-%m-%Y') as Q2 FROM pps_detail_transaction WHERE id = ?"; $query_args = [$uid];
	$hasil = db_query_bind($dbc, $query, $query_args);
	...
	while ($data = mysqli_fetch_array($hasil))
	{
		...
		$query_sloc = "SELECT * FROM mat_master_detail WHERE material = ? OR bill_component = ?"; $query_sloc_args = [$data["material_no"], $data["material_no"]];
		$result_sloc = db_query_bind($dbc, $query_sloc, $query_sloc_args);
	...
	$barcodeobj = new TCPDF2DBarcode($data['bflush_no'].'|'.$data['comp_code'].'|'.$row['ploc'].'|'.$data['material_no'].'|'.$data['plan_no'].'|'.$row['shift_tag'].'|'.$row['tag_qty'].'|'.$row_info3['BUn'].'|'.$data_sloc['sloc'].'|'.$data['Q2'].'|'.$data['Q'].'|'.$total_slip_no, 'PDF417');
