...
/**
 * prod/disposal_backflush_tran_NG2_approved.php
...
<div id="content-header">
  ...
  <div class="container-fluid">
            ...
            <tr>
              ...
              <td><select name="doc_disposal_no" id="doc_disposal_no" class="span8">
               <option value="NULL" placeholder="Select Disposal Doc. No.">-Disposal Doc. No.-</option>
                  <?php
	       $query9 = "SELECT * FROM reject_detail_disposal WHERE (status_part = 'PR' OR status_part = 'WS') AND doc_disposal_no != '' AND status_disposal != ?  AND status_disposal != ? AND status_disposal != ? GROUP BY doc_disposal_no"; $query9_args = [$rst_sta["status_desc"], $rst_sta16["status_desc"], $rst_sta4["status_desc"]];
   ...
   $query8 = "SELECT * FROM reject_detail_disposal AS MR WHERE (MR.status_part = 'PR' OR MR.status_part = 'WS') AND MR.doc_disposal_no != '' AND status_disposal != ? AND status_disposal != ? AND status_disposal != ?".$where_sql." GROUP BY MR.doc_disposal_no"; $query8_args = [$rst_sta["status_desc"], $rst_sta16["status_desc"], $rst_sta4["status_desc"], ...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2 FROM reject_detail_disposal AS MR WHERE (MR.status_part = 'PR' OR MR.status_part = 'WS') AND MR.doc_disposal_no != '' AND status_disposal != ? AND status_disposal != ? ".$where_sql." AND status_disposal != ? GROUP BY MR.doc_disposal_no ORDER BY MR.plan_no ASC $pages->limit"; $query_args = [$rst_sta["status_desc"], $rst_sta16["status_desc"], ...$where_args, $rst_sta4["status_desc"]];
