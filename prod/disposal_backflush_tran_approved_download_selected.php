...
/**
 * prod/disposal_backflush_tran_approved_download_selected.php
...
//-------Count all results------------------------//
  ...
  $query8 = "SELECT COUNT(*) FROM reject_detail_disposal WHERE (status_part = 'PR' OR status_part = 'WS') AND doc_disposal_no != '' AND status_disposal != ?".$where_sql; $query8_args = [$rst_sta4["status_desc"], ...$where_args];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_reject,'%d-%m-%Y') as R3, DATE_FORMAT(date_wastage,'%d-%m-%Y') as R4, DATE_FORMAT(date_disposal,'%d-%m-%Y') as R5, DATE_FORMAT(date_approve,'%d-%m-%Y') as R6, DATE_FORMAT(date_approve2,'%d-%m-%Y') as R7 FROM reject_detail_disposal WHERE (status_part = 'PR' OR status_part = 'WS') AND doc_disposal_no != '' AND status_disposal != ? AND status_disposal != ? AND status_disposal != ?".$where_sql." ORDER BY doc_disposal_no ASC"; $query_args = [$rst_sta["status_desc"], $rst_sta16["status_desc"], $rst_sta4["status_desc"], ...$where_args];
...
while ($row2 = mysqli_fetch_array($rs))
{
	...
	$query_u2 = "SELECT * FROM user_detail WHERE username = ?"; $query_u2_args = [$row2["user_wastage"]];
	...
	$query_u3 = "SELECT * FROM user_detail WHERE username = ?"; $query_u3_args = [$row2["user_disposal"]];
	$result_u3 = db_query_bind($dbc, $query_u3, $query_u3_args);   //run the query.
	$data_u3 = mysqli_fetch_array($result_u3);   //how many records are there?     
	
	$query_u4 = "SELECT * FROM user_detail WHERE username = ?"; $query_u4_args = [$row2["approve_by"]];
	$result_u4 = db_query_bind($dbc, $query_u4, $query_u4_args);   //run the query.
	$data_u4 = mysqli_fetch_array($result_u4);   //how many records are there?  
	
	$query_u5 = "SELECT * FROM user_detail WHERE username = ?"; $query_u5_args = [$row2["approve_by2"]];
	$result_u5 = db_query_bind($dbc, $query_u5, $query_u5_args);   //run the query.
	...
	$query_type_wastage = "SELECT * FROM type_wastage_detail WHERE id_wastage = ? ORDER BY id_wastage ASC"; $query_type_wastage_args = [$row2['type_wastage']];
    $result_type_wastage = db_query_bind($dbc, $query_type_wastage, $query_type_wastage_args);
    $row_type_wastage = mysqli_fetch_array($result_type_wastage); 
	
	$query_reason_wastage = "SELECT * FROM reason_wastage WHERE id_reason_wastage = ? ORDER BY id_reason_wastage ASC"; $query_reason_wastage_args = [$row2['reason_wastage']];
    $result_reason_wastage = db_query_bind($dbc, $query_reason_wastage, $query_reason_wastage_args);
