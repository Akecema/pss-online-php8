...
$queryu = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction AS MR, work_center_detail AS SR WHERE MR.work_center = SR.id_work AND (MR.status_pps = ? OR MR.status_pps = ? OR MR.status_pps = ?)".$where_sql." ORDER BY MR.plan_no ASC"; $queryu_args = [$rst_sta7["status_desc"], $rst_sta14["status_desc"], $rst_sta4["status_desc"], ...$where_args];
