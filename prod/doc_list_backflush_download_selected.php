...
/**
 * prod/doc_list_backflush_download_selected.php
 * Part of: Production module
 * Filename suggests: doc list backflush download selected
 *
...
set_time_limit(0);
			...
			$query_convert = "SELECT * FROM work_center_detail AS SR WHERE SR.id_factory = ? AND SR.id_work = ?"; $query_convert_args = [$_GET["factory"], $_GET["work_center"]];
...
$namaFile = "Backflush Document List_".$date_tdy2.".xls";
 //convert material no kpd id_hdr
    ...
    $query8 = "SELECT * FROM pps_detail_transaction AS MR WHERE (MR.status_pps = ? OR MR.status_pps = ? OR MR.status_pps = ?)".$where_sql; $query8_args = [$rst_sta7["status_desc"], $rst_sta14["status_desc"], $rst_sta4["status_desc"], ...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction AS MR WHERE (MR.status_pps = ? OR MR.status_pps = ? OR MR.status_pps = ?)".$where_sql." ORDER BY MR.plan_no ASC"; $query_args = [$rst_sta7["status_desc"], $rst_sta14["status_desc"], $rst_sta4["status_desc"], ...$where_args];
