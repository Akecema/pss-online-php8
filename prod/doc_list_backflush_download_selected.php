...
set_time_limit(0);
			...
			$query_convert = "SELECT * FROM work_center_detail AS SR WHERE SR.id_factory = '".db_esc($dbc, $_GET["factory"])."' AND SR.id_work = '".db_esc($dbc, $_GET["work_center"])."'";
...
$namaFile = "Backflush Document List_".$date_tdy2.".xls";
 //convert material no kpd id_hdr
    ...
    $query8 = "SELECT * FROM pps_detail_transaction AS MR WHERE (MR.status_pps = '".db_esc($dbc, $rst_sta7["status_desc"])."' OR MR.status_pps = '".db_esc($dbc, $rst_sta14["status_desc"])."' OR MR.status_pps = '".db_esc($dbc, $rst_sta4["status_desc"])."')".$where_sql;
