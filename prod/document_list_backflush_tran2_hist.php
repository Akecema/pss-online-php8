...
/**
 * prod/document_list_backflush_tran2_hist.php
 * Part of: Production module
 * Filename suggests: document list backflush tran2 hist
 *
 * Behavior: requires an active login session ($_SESSION['username']); reads parameters from the query string ($_GET).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, factory_detail, work_center_detail, pps_detail, pps_detail_transaction.
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_production.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a><a href="#" class="#">Production</a> <a href="#" class="current">Document List</a></div>
...
<?php
			...
			/*$query_convert = "SELECT * FROM pps_detail as SR WHERE SR.id_factory_pps = '".$_GET["factory"]."'";
			$result_convert = mysqli_query($dbc, $query_convert); 
			$row_convert = mysqli_fetch_array($result_convert);
			
		   echo h($row_convert["id_factory_pps"]);*/
		 ...
		 // 2. DateT
                if($dateT == "0000-00-00") {
                     $wheresql_02 = ""; $wheresql_02_args = []; }
                      ...
                      $wheresql_02 = " AND (MR.date_posting < ?)"; $wheresql_02_args = ['' . $dateT . ' 23:59:59']; }		
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail_transaction AS MR, work_center_detail AS SR WHERE MR.work_center = SR.id_work AND (MR.status_pps = ? OR MR.status_pps = ? OR MR.status_pps = ?)".$where_sql; $query8_args = [$rst_sta7["status_desc"], $rst_sta14["status_desc"], $rst_sta4["status_desc"], ...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction AS MR, work_center_detail AS SR WHERE MR.work_center = SR.id_work AND (MR.status_pps = ? OR MR.status_pps = ? OR MR.status_pps = ?)".$where_sql." ORDER BY MR.plan_no ASC"; $query_args = [$rst_sta7["status_desc"], $rst_sta14["status_desc"], $rst_sta4["status_desc"], ...$where_args];
...
<tr>
    ...
    <td width="85%"> <div class="small-nav"></div></td> 
      <td width="7%"><a href="doc_list_backflush_download_selected_hist.php?date1=<?php echo h($dateF); ?>&&date2=<?php echo h($dateT); ?>&&factory=<?php echo h($factory); ?>&&work_center=<?php echo h($work_center); ?>&&plan_no=<?php echo h($plan_no); ?>&&shift_ops=<?php echo h($shift_ops); ?>" ><img src="../img/dload_excel.jpg" width="48" height="48" title="Download" /></a></td>
	...
	if($sta_out2 == "211")
	{
	 ...
	 // $status_output = "";
      ...
      ?>
                ...
                <td width="48"><?php echo h($row["time_posting"]); ?></td>
                ...
                <td width="90"><font color="#0000CC"><?php echo h($row["bflush_no"]); ?></font><br><font color="#FF0000"><?php if ($row["bflush_no_ref"] != "") { echo h($row["bflush_no_ref"]); } ?></font></td>
