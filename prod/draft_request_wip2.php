...
/**
 * prod/draft_request_wip2.php
 * Part of: Production module
 * Filename suggests: draft request wip2
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, scan_detail_wip, wip_request, WIP, mat_master_header.
  ...
  <!--  <div class="span6">-->
          ...
          <div class="widget-title">
            <ul class="nav nav-tabs" role="tablist" id="templatemo-tabs">
              ...
              <li class="active"><a role="tab" href="draft_request_wip.php">Draft Request</a></li>
    ...
    <table width="930">
          <tr>
            ...
            <td width="98%"><form name="frmSearch" method="get" action="<?=h($_SERVER['SCRIPT_NAME']);?>">
              <table width="724">
                <tr>
    ...
    for ($i=0; $i<$how_many; $i++) { 
      //  echo ($i+1) . '- ' . $cancel[$i] . '<br>'; 
		  ...
		  $query_m23 = "DELETE FROM `scan_detail_wip` WHERE id_scan = ?"; $query_m23_args = [$cancel[$i]];
		  $result_m23 = db_query_bind($dbc, $query_m23, $query_m23_args) or die(db_fail($dbc));
		 
		  $query_m24 = "DELETE FROM `wip_request` WHERE id_scan_wip = ?"; $query_m24_args = [$cancel[$i]];
		  $result_m24 = db_query_bind($dbc, $query_m24, $query_m24_args) or die(db_fail($dbc));
	  
	    }
		
		
		   echo "<script>";
		   echo "alert('WIP Request successfully deleted.');";
		   echo "window.location='draft_request_wip.php'";
	       echo "</script>"; 
...
$query_update = "UPDATE wip_request SET bom_qty_wip = ?, user_update = ?, date_update = NOW() WHERE id_req_wip = ? "; $query_update_args = [$_POST["bom_qty"][$i], $res["user_no"], $_POST["id_req"][$i]];

$result_update = db_query_bind($dbc, $query_update, $query_update_args);
     ...
     if($result_update)
		   ...
		   echo "window.location='wip_request_list.php'";
	       echo "</script>"; 
		   exit(); //quit the script
...
$query_update2 = "UPDATE wip_request SET mrin_doc_wip = '$number', mrin_year_wip = ?, temp_mrin_wip = ?, bom_qty_wip = ?, status_request = 'Y', user_update = ?, date_update = NOW(), date_posting = NOW(), time_posting = NOW() WHERE id_req_wip = ? "; $query_update2_args = [$year, $ref, $_POST["bom_qty"][$i], $res["user_no"], $_POST["id_req"][$i]];

$result_update2 = db_query_bind($dbc, $query_update2, $query_update2_args);
	 ...
	 $i++;
      ...
      if($result_update2)
		   ...
		   echo "window.location='display_request_wip.php'";
	       echo "</script>"; 
			 exit(); //quit the script
...
echo '<font color="red" class ="error_entry">', $message, '</font>';
   ...
   $query8 = "SELECT COUNT(*) FROM wip_request AS MR, mat_master_detail as MD, scan_detail_wip AS SD WHERE MR.id_scan_wip = SD.id_scan AND MD.id_dtl = MR.id_dtl_wip AND SD.status_urgent = 'N' AND MR.status_request = 'N' AND ((MD.material LIKE ?) OR (MD.bill_component LIKE ?)) AND MR.user_create = ? AND ((MR.date_mrin >= '$current_date') AND (MR.date_mrin <= '$next_date')) ORDER BY MR.id_req_wip ASC"; $query8_args = ['%' . $_GET["txtKeyword"] . '%', '%' . $_GET["txtKeyword"] . '%', $res["user_no"]];
...
$query = "SELECT *, DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R FROM wip_request AS MR, mat_master_detail as MD, scan_detail_wip AS SD WHERE MR.id_scan_wip = SD.id_scan AND MD.id_dtl = MR.id_dtl_wip AND SD.status_urgent = 'N' AND ((MD.material LIKE ?) OR (MD.bill_component LIKE ?)) AND MR.status_request = 'N' AND MR.user_create = ? AND ((MR.date_mrin >= '$current_date') AND (MR.date_mrin <= '$next_date')) GROUP BY MR.id_scan_wip ORDER BY MR.id_req_wip ASC"; $query_args = ['%' . $_GET["txtKeyword"] . '%', '%' . $_GET["txtKeyword"] . '%', $res["user_no"]];
...
?>
    ...
    <td width="65">&nbsp;<a value="Details" href="detail_prod_order_wip.php?&&TB_iframe=true&height=400&width=650" class="thickbox" target="_self"><img src="../img/icon_view.jpg" width="16" height="16" alt="View">View</a></td>
  </tr>
</table>
<br />
   ...
   $query_scan = "SELECT * FROM scan_detail_wip WHERE id_scan = ? GROUP BY id_scan"; $query_scan_args = [$row2[6]];
   ...
   <td><div align="center">
    ...
    <tbody>
         <?php
	$query_again = "SELECT * FROM wip_request WHERE status_request = 'N' and id_scan_wip = ? AND status != 'Cancel' ORDER BY id_req_wip ASC "; $query_again_args = [$row2[6]];
    $rs_again = db_query_bind($dbc, $query_again, $query_again_args);   //run the query.
	 while ($row = mysqli_fetch_array($rs_again))
   {
		 
   $query1_p = "SELECT * FROM scan_detail_wip WHERE id_scan = ? GROUP BY id_scan"; $query1_p_args = [$row2[6]];
...
<br>
           ...
           <input name="save_btn" type="submit" id="submit" value="POST" class="btn btn-success" onClick="return confirm('Confirm to post request?');">
