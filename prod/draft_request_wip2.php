...
echo '<font color="red" class ="error_entry">', $message, '</font>';
   ...
   $query8 = "SELECT COUNT(*) FROM wip_request AS MR, mat_master_detail as MD, scan_detail_wip AS SD WHERE MR.id_scan_wip = SD.id_scan AND MD.id_dtl = MR.id_dtl_wip AND SD.status_urgent = 'N' AND MR.status_request = 'N' AND ((MD.material LIKE '%".db_esc($dbc, $_GET["txtKeyword"])."%') OR (MD.bill_component LIKE '%".db_esc($dbc, $_GET["txtKeyword"])."%')) AND MR.user_create = '".db_esc($dbc, $res["user_no"])."' AND ((MR.date_mrin >= '$current_date') AND (MR.date_mrin <= '$next_date')) ORDER BY MR.id_req_wip ASC";
...
$query = "SELECT *, DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R FROM wip_request AS MR, mat_master_detail as MD, scan_detail_wip AS SD WHERE MR.id_scan_wip = SD.id_scan AND MD.id_dtl = MR.id_dtl_wip AND SD.status_urgent = 'N' AND ((MD.material LIKE '%".db_esc($dbc, $_GET["txtKeyword"])."%') OR (MD.bill_component LIKE '%".db_esc($dbc, $_GET["txtKeyword"])."%')) AND MR.status_request = 'N' AND MR.user_create = '".db_esc($dbc, $res["user_no"])."' AND ((MR.date_mrin >= '$current_date') AND (MR.date_mrin <= '$next_date')) GROUP BY MR.id_scan_wip ORDER BY MR.id_req_wip ASC";
...
<br />
  ...
  $row4_p = mysqli_fetch_array($result4_p); 
	 ...
	 <input name="bom_qty[<?php echo $i; ?>]" type="text" value="<?php if(isset($_POST["bom_qty"])) { echo $_POST["bom_qty"][$i]; }else{  echo $row["bom_qty_wip"];   } ?>" class="span8" />	
