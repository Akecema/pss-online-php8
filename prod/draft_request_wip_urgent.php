...
echo '<font color="red" class ="error_entry">', $message, '</font>';
  ...
  $query8 = "SELECT COUNT(*) FROM wip_request AS MR, scan_detail_wip AS SD WHERE MR.id_scan_wip = SD.id_scan AND SD.status_urgent = 'Y' AND MR.status_request = 'N' AND MR.user_create = '".$res["user_no"]."' AND ((MR.date_mrin >= '$current_date') AND (MR.date_mrin <= '$next_date')) ORDER BY MR.id_req_wip ASC";
