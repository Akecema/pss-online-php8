...
$qry = mysql_query("SELECT *, DATE_FORMAT(date_posting,'%Y-%m-%d') AS R, DATE_FORMAT(date_create,'%Y-%m-%d') AS R2, DATE_FORMAT(date_create,'%H:%i:%s') AS R3 FROM pps_detail_transaction WHERE bflush_no = '".$buid."' ");
$data = "";
while($row = mysql_fetch_array($qry)) {
	
	$query_iscan = "SELECT * FROM scan_prod_planning WHERE id_scan = '".$row["id_scan"]."'";
	$result_iscan = mysql_query($query_iscan);
	$row_iscan = mysql_fetch_array($result_iscan);
...
//chmod($file, 0777);
	...
	$result_cftp = mysql_query($query_cftp);
	$row_cftp = mysql_num_rows($result_cftp);
  ...
  $data_collect = mysql_fetch_array($rst_collect);
	...
	$result_iscan2 = mysql_query($query_iscan2);
	$row_iscan2 = mysql_fetch_array($result_iscan2);
