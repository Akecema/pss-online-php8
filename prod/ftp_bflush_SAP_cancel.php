...
/**
 * prod/ftp_bflush_SAP_cancel.php
 * Part of: Production module
 * Filename suggests: ftp bflush SAP cancel
 *
...
$uid2 = $_GET["buid"];
$buid = preg_replace('~[^A-Za-z0-9_-]~', '', (string) ($_GET["buid2A"] ?? '')); // goes into a file name: no path characters

$qry = db_query_bind($dbc, "SELECT *, DATE_FORMAT(A1.date_posting,'%Y-%m-%d') AS R, DATE_FORMAT(A1.date_create,'%Y-%m-%d') AS R2, DATE_FORMAT(A1.date_create,'%H:%i:%s') AS R3 FROM pps_detail_transaction AS A1 WHERE A1.id = ?", [$uid2]);
$data = "";
while($row = mysqli_fetch_array($qry)) {
	...
	$filen="BF2".$buid;
	  ...
	  $query_uom = "SELECT * FROM scan_prod_planning WHERE id_scan = ?"; $query_uom_args = [$row["id_scan"]];
	  $rst_uom = db_query_bind($dbc, $query_uom, $query_uom_args);
   ...
   //----colect data --------------
  $query_collect = "SELECT *, DATE_FORMAT(A1.date_posting,'%Y-%m-%d') AS R FROM pps_detail_transaction AS A1 WHERE A1.id = ?"; $query_collect_args = [$uid2];
  $rst_collect = db_query_bind($dbc, $query_collect, $query_collect_args);
  $data_collect = mysqli_fetch_array($rst_collect);
  
  //------get data UOM ----------
	 
  $query_collectA = "SELECT * FROM scan_prod_planning WHERE id_scan = ?"; $query_collectA_args = [$data_collect["id_scan"]];
  $rst_collectA = db_query_bind($dbc, $query_collectA, $query_collectA_args);
  $data_collectA = mysqli_fetch_array($rst_collectA);

 
     //--------insert into table ftp_bflush_detail
  
     $query_ftp_info = "INSERT INTO ftp_bflush_detail(id,file_name,bflush_no,ref_id,plan_no,material_no,material_desc,qty_ftp,uom,status_ftp,posting_date,posting_time,user_create,date_create) VALUES('',?,?,?,?,?,?,?,?,'Y',?,?,?,NOW())"; $query_ftp_info_args = [$filen, $data_collect["bflush_no"], $data_collect["ref_id"], $data_collect["plan_no"], $data_collect["material_no"], $data_collect["material_desc"], $data_collect["qty_NG"], $data_collectA["scan_uom"], $data_collect["R"], $data_collect["time_posting"], $username]; 
     $rst_ftp_info = db_query_bind($dbc, $query_ftp_info, $query_ftp_info_args);
	 
	}elseif($sta_out == "211")
	{
	
	 $data .= $row['bflush_no_ref'].";".$row['bflush_no'].";".$row['comp_code'].";".$row['work_center'].";".$row['material_no'].";132;".$row['plan_no'].";".$row['date_plan'].";".$row['shift_posting'].";".$row['qty_actual'].";".$data_uom['scan_uom'].";".$row['ploc'].";".$row['R'].";".$row['time_posting'].";".$row['R2'].";".$row['R3'].";".$row['user_cancel'].";".$row['date_cancel']."\r\n";	
		
	
	
	  //----colect data --------------
  $query_collect = "SELECT *, DATE_FORMAT(date_posting,'%Y-%m-%d') AS R FROM pps_detail_transaction WHERE id = ?"; $query_collect_args = [$uid2];
  $rst_collect = db_query_bind($dbc, $query_collect, $query_collect_args);
  $data_collect = mysqli_fetch_array($rst_collect);
...
file_put_contents($file,$data);
	 ...
	 // ---update status 
		...
		$query_ftp = "UPDATE pps_detail_transaction SET status_ftp_bflush = 'Y' WHERE id = ?"; $query_ftp_args = [$uid2];
