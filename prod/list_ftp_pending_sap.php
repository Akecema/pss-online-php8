         ...
         <div class="widget-box">
          <div class="widget-title">
             <ul class="nav nav-tabs" role="tablist" id="templatemo-tabs">
              <li class="active"><a  role="tab" href="list_ftp_pending_sap.php">Backflush</a></li>
              <li><a role="tab" href="list_ftp_pending_GIProd.php">Goods Issue</a></li>
             </ul>
          </div>
          </div>
...
if(is_dir($dir)){
  ...
  // $total2 = count($dh); removed - count() on a directory resource/handle throws a fatal TypeError under PHP 8 (was PHP7: warning + returned 1). $total2 was assigned but never read anywhere in this file, so the line was just dropped rather than wrapped.
   ...
   <?php
	...
	$query = "SELECT *,DATE_FORMAT(date_create,'%d-%m-%Y %H:%i:%s') AS R2, DATE_FORMAT(posting_date,'%d-%m-%Y') AS R3 FROM ftp_bflush_detail WHERE status_ftp = 'Y' AND file_name = ?"; $query_args = [$filename2];
