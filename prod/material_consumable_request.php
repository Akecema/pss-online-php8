...
/**
 * prod/material_consumable_request.php
 * Part of: Production module
 * Filename suggests: material consumable request
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, consumable_detail, consumable_request, factory_detail.
...
//insert to scan_detail
$query_db = "INSERT INTO consumable_request(id_req_con, mrin_doc, mrin_year, temp_mrin, id_con, id_scan, material_no, con_qty, con_uom, status_request, status_print, status_view, factory, user_create, date_create, user_update, date_update, date_posting, time_posting, status, date_require, time_require, id_work) VALUES('".mysqli_insert_id($dbc)."', '$number', ?,?, ?, ?, ?,  ?, ?, 'Y', 'N','N',?, ?, NOW(), '', '', NOW() ,NOW(), 'New', ?, ?, ?)"; $query_db_args = [$year, $ref, $material_no, $lastID_2, $row_data["material_no"], $con_qty, $row_data["BUn"], $factory, $user_no, $date1, $t_time, $work_center];
$result = db_query_bind($dbc, $query_db, $query_db_args) or die(db_fail($dbc));
