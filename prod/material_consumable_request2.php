...
$query_data = "SELECT * FROM consumable_detail WHERE id_con = ?"; $query_data_args = [$material_no];
$result_data = db_query_bind($dbc, $query_data, $query_data_args) or die(db_fail($dbc));
...
$query_update = "UPDATE consumable_request SET status_request = 'Y', mrin_doc = '$number', mrin_year = ?, temp_mrin = ?, con_qty = ?, user_update = ?, date_update = NOW(), date_posting = NOW(), time_posting = NOW() WHERE id_req_con = ? AND user_create = ? "; $query_update_args = [$year, $ref, $_POST["con_qty"][$i], $res["user_no"], $_POST["id_req_con"][$i], $res["user_no"]];
     ...
     <!-- Box -->
              ...
              <tr>
                 ...
                 <td width="63%" height="35"><font color="#006699">
                  <select name="work_center" id="work_center">
                    ...
                    <?php
	       $query4 = "SELECT * FROM work_center_detail WHERE id_factory = ? ORDER BY id_work ASC"; $query4_args = [$factory];
                   $result4 = db_query_bind($dbc, $query4, $query4_args);
  
                   while($row4=mysqli_fetch_array($result4)) 
			      {
                  echo'<option value="',h($row4["id_work"]),'">',stripslashes($row4["id_work"]),' - ',stripslashes($row4["wc_desc"]),'</option>';
                  }
				?>
                  </select>
            *</font></td>
             ...
             <tr>
                 ...
                 <td width="63%" height="35"><select name="material_no" id="material_no">
			      ...
			      {
                  echo'<option value="',h($row2["id_con"]),'">',stripslashes($row2["material_no"]),' - ',stripslashes($row2["mat_desc"]),'</option>';
                  }
				?>
                   </select>
                   *</td>
               ...
               <tr>
                 ...
                 <td height="10" colspan="2"><div align="right">
                     ...
                     <input name="lastID" type="hidden" value="<?php echo h($lastID); ?>">
        ...
        <?php
           ...
           <!--     <form name="form2" method="post" action="material_consumable_request2.php">
              -->
      <form name="form2" method="post" action="material_consumable_request2.php?lastID=<?php echo h($lastID); ?>&&date1=<?php echo h($date1); ?>&&t_time=<?php echo h($t_time); ?>&&factory=<?php echo h($factory); ?>" >
             <?php
			
			  $i = 1;
			  
			$query_data2 = "SELECT * FROM consumable_request WHERE id_scan = ? AND status_request = 'N' AND user_create = ?"; $query_data2_args = [$lastID, $res["user_no"]];
		   ...
		   while ($row_data2 = mysqli_fetch_array($result_data2))
		   {
			 ...
			 $query_con_detail = "SELECT * FROM consumable_detail WHERE id_con = ?"; $query_con_detail_args = [$row_data2["id_con"]];
			 $result_con_detail = db_query_bind($dbc, $query_con_detail, $query_con_detail_args) or die(db_fail($dbc));
             ...
             <table width="98%" height="26" border="0" cellspacing="2" bgcolor="#eee">
               <tr> 
                 ...
                 <td width="6%">
                 <div align="center"><a href="delete_consumable_request.php?id_req_con=<?php echo h($row_data2["id_req_con"]); ?>&&lastID=<?php echo h($lastID); ?>&&date1=<?php echo h($date1); ?>&&t_time=<?php echo h($t_time); ?>&&factory=<?php echo h($factory); ?>" onclick="return confirm('Are you sure you want to delete?')"><img src="../img/delete.png" width="16" height="16"  /></a></div></td>
               </tr>
             </table>
