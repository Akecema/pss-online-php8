...
/**
 * prod/material_request_list.php
 * Part of: Production module
 * Filename suggests: material request list
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, work_center_detail, mat_master_detail, scan_detail.
...
//print_r(explode('|', $str, -1));
  ...
  $query3 = "SELECT * FROM work_center_detail WHERE id_work = ? ORDER BY id_work ASC"; $query3_args = [$part4];
  $result3 = db_query_bind($dbc, $query3, $query3_args);
  $row3 = mysqli_fetch_array($result3); 
				   
  $query_q2 = "SELECT * FROM mat_master_detail WHERE (material = ? OR bill_component = ?)"; $query_q2_args = [$part1, $part1];
...
$query_db = "INSERT INTO scan_detail(id_scan, pps_ref, factory, work_center, prod_order, material_no, scan_oum, scan_plant, scan_sloc, scan_qty, user_create, date_create, user_update, date_update, status_urgent) VALUES ('".mysqli_insert_id($dbc)."', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(),'','','N')"; $query_db_args = [$pps_ref2, $row3["id_factory"], $part4, $part2, $part1, $part7, $ans3["plant"], $ans3["sloc"], $part6, $user_no];
...
echo "window.location='material_request_listProc.php?uid=$data_sql[id_scan]'";
echo "</script>";
...
?>
 ...
 <div class="widget-content nopadding">
         <form name="form1" method="post" action="material_request_list.php" class="form-horizontal">
