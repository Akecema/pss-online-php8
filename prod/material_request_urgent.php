...
$query_db = "INSERT INTO `scan_detail` (id_scan, pps_ref, factory, work_center, prod_order, material_no, scan_oum, scan_plant, scan_sloc, scan_qty, user_create, date_create, user_update, date_update, status_urgent) VALUES ('".mysqli_insert_id($dbc)."', '', ?, ?, ?, ?, '', '', ?, '', ?, NOW(),'','','Y')"; $query_db_args = [$factory, $work_center, $prod_order, $material_no, $factory, $user_no];
$result = db_query_bind($dbc, $query_db, $query_db_args) or die(db_fail($dbc));
			 ...
			 $query_sql2 = "SELECT * FROM `mat_master_header` WHERE material_no = ?"; $query_sql2_args = [$data_sql["material_no"]];
			 $result_sql2 = db_query_bind($dbc, $query_sql2, $query_sql2_args);
			 $data_sql2 = mysqli_fetch_array($result_sql2);
			 
			 
	$query_update = "UPDATE `scan_detail` SET scan_oum = ?, scan_plant = ? WHERE id_scan = ?"; $query_update_args = [$data_sql2["BUn"], $data_sql2["plant"], $data_sql["id_scan"]];
...
<div class="widget-content nopadding">
         <form name="form1" method="post" action="material_request_urgent.php" class="form-horizontal">
	       ...
	       $query2 = "SELECT * FROM mat_master_header WHERE status_BOM = 'Y' ORDER BY material_no ASC";
			      ...
			      {
                  echo'<option value="',h($row2["material_no"]),'">',stripslashes($row2["material_no"]),' - ',stripslashes($row2["material_desc"]),'</option>';
                  }
				?>
                   </select>
              </div>
            ...
            <label class="control-label">Production Order : </label>
              <div class="controls">
            <input name="prod_order" type="text" id="prod_order" size="20" maxlength="15" placeholder="Enter Production Order" value="<?php if(isset($_POST['prod_order'])) echo h($_POST['prod_order']); ?>" />
              </div>
            </div>
             <div class="control-group">
              <div class="controls">
