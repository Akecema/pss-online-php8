...
/**
 * prod/material_request_urgentProc.php
 * Part of: Production module
 * Filename suggests: material request urgentProc
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, scan_detail, material_request, mat_master_detail, mat_master_header.
...
<div class="container-fluid">
 ...
 <?php
	   ...
	   $query_scan = "SELECT * FROM scan_detail WHERE id_scan = ?"; $query_scan_args = [$uid];
...
{  	
  ...
  $query_q2 = "SELECT * FROM mat_master_detail WHERE id_dtl = ?"; $query_q2_args = [$cancel2[$i]];
  $result_q2 = db_query_bind($dbc, $query_q2, $query_q2_args) or die(db_fail($dbc));
  $ans3 = mysqli_fetch_array($result_q2);
  
//register the user in the db.
$query_db2 = "INSERT INTO material_request (id_req, mrin_doc, mrin_year, temp_mrin, id_hdr, id_dtl, id_scan, bom_id, bom_qty, bom_oum, status_request, status_print, user_create, date_create, user_update, date_update, date_posting, time_posting, status, bom_component, date_mrin, time_mrin) VALUES('','','','',?,?,?,?,?, ?,'N','N',?,NOW(),'','','','','New',?,?,?)"; $query_db2_args = [$ans3['id_hdr'], $ans3['id_dtl'], $uid, $cancel2[$i], $string[$i], $ans3['comp_unit'], $res['user_no'], $ans3['bill_component'], $_POST["date1"], $t_time];
$result_db2 = db_query_bind($dbc, $query_db2, $query_db2_args) or die(db_fail($dbc));
...
{ echo '<div class="msg msg-error"><font color="red" class ="error_entry">', $message, '</font></div>';
    ...
    $no = 1; 
		  ...
		  ?>
         <form name="myform" method="post" action="material_request_urgentProc.php?uid=<?php echo h($uid); ?>">
 ...
 ?></td>
  ...
  <tr>
    ...
    <td colspan="5">&nbsp;</td>
	   ...
	   $query_component = "SELECT * FROM mat_master_header AS h, mat_master_detail AS s WHERE h.id_hdr = s.id_hdr AND s.material = ? AND s.bom_status = 'Y'"; $query_component_args = [$data_scan["material_no"]];
	   $result_component = db_query_bind($dbc, $query_component, $query_component_args);
		...
		$num_convert = number_format($formula_qty, 3, '.', '');
               ...
               <tr>
               <td><div align="center"><input type="checkbox" name="cancel[]" value="<?php echo h($row["id_dtl"]); ?>" <?=was_checked($row["id_dtl"],$a) ?> />
             <input type="hidden" name="Check_ctr" value="yes" 
onClick="Check(document.myform.cancel)">  </div></td>
               <td><?php echo $no2; ?></td>
               <td><?php echo h($row["bill_component"]);   ?></td>
               <td><input name="comp_quantity[<?php echo h($row["id_dtl"]); ?>]" type="text" value="<?php if(isset($_POST["comp_quantity"])) { echo h($_POST["comp_quantity"][($row["id_dtl"])] ?? ''); }else{  echo h($num_convert);   } ?>" /></td>
               <td><div align="center"><?php echo h($row["comp_unit"]);   ?></div></td>
               <td><div align="center"><?php echo h($data_scan["work_center"]); ?></div>
