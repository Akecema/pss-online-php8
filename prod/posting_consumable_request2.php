...
/**
 * prod/posting_consumable_request2.php
 * Part of: Production module
 * Filename suggests: posting consumable request2
 *
...
<form action="<?php echo h($_SERVER['PHP_SELF']); ?>" method="get" name="frmSearch" id="frmSearch">
	...
	//********** END CONDITION **************
		...
		$query8 = "SELECT *, DATE_FORMAT(MR.date_require,'%d-%m-%Y') AS R FROM consumable_request AS MR WHERE MR.status_request = 'Y' AND (MR.status != 'Close' AND MR.status  != 'Cancel') AND MR.status_print != 'Y' ".$where_sql." GROUP BY MR.temp_mrin ORDER BY MR.date_posting ASC, MR.temp_mrin ASC"; $query8_args = [...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_require,'%d-%m-%Y') AS R FROM consumable_request AS MR WHERE MR.status_request = 'Y' AND (MR.status != 'Close' AND MR.status  != 'Cancel') AND MR.status_print != 'Y' ".$where_sql." GROUP BY MR.temp_mrin ORDER BY MR.date_posting ASC, MR.temp_mrin ASC "; $query_args = [...$where_args];
...
?>
   ...
   while ($row2 = mysqli_fetch_array($rs))
   {
    ...
    $data_u = mysqli_fetch_array($result_u);   //how many records are there?   
		  ...
		  ?>        
         <tr>
            <td width="211">&nbsp;<?php echo h($row2["temp_mrin"]); ?></td>
            <td width="103"><?php echo h($row2["factory"]); ?></td>
            <td width="94"><?php echo h($row2["id_work"]); ?></td>
            <td width="81"><?php echo h($row2["R"]); ?>&nbsp;</td>
            <td width="81"><?php echo h($row2["time_require"]); ?></td>
            <td width="156"><?php echo h($data_u["user_fullname"]); ?></td>
            <?php
...
elseif($curr_time >= $plus_20)
{
	...
	?>
           ...
           <td width="60"><a value="Details" href="detail_consumable_request.php?mrin_no=<?php echo h($row2["temp_mrin"]); ?>&&TB_iframe=true&&height=400&&width=800" class="thickbox" target="_self"><img src="../img/icon_view.jpg" width="16" height="16" alt="View">View</a></td>
           <td width="80">
<a href="detail_consumable_request_reprinting.php?mrin_no=<?php echo h($row2["temp_mrin"]); ?>&&TB_iframe=true&&height=400&&width=800" class="thickbox" target="_self"><img src="../img/print.jpg" width="16" height="16" alt="Print">Print</a></td>
  </tr>
