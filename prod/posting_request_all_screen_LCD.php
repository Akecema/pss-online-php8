...
/**
 * prod/posting_request_all_screen_LCD.php
 ...
 * Includes: config.php, paginator.class2.php, tc_calendar.php, content.php, top_modal_menu.php, left_production_menu.php, footer.php.
 *
...
<div id="content">
 <div id="content-header">
  <div id="breadcrumb"> <a href="index_production.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="tip-bottom">Board</a> <a href="#" class="current">Material Request Board</a> </div>
  <h1>Board</h1>
</div>
...
//---------------------------------------------------------------------------------------------------------------------
	...
	$query_tp = "SELECT *,SUM(rquantity) as TOT FROM post_detail_header WHERE mrin_no = '".$row2["temp_mrin"]."' AND prod_order = '".$row_scan["prod_order"]."' AND mvt_type = 311 AND status_posting = 'New' GROUP BY material_no";
	...
	while($row_tp = mysqli_fetch_assoc($result_tp))
{
 ...
 $result_upd3 = mysqli_query($dbc, $query_upd3); 
	 ...
	 $query_upd4 = "UPDATE `material_request` SET status = 'Close' WHERE temp_mrin = '".$row2["temp_mrin"]."' AND (bom_qty = '0.000' OR bom_qty = '')";
        ...
        if($result_upd3 || $result_upd4)
		 {
		  ...
		  $query_mm3 = "SELECT * FROM `material_request` WHERE temp_mrin = '".$row2["temp_mrin"]."' AND status = 'Close' AND bom_component = '".$row_tp["material_no"]."'"; 
