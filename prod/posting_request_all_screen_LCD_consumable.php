...
/**
 * prod/posting_request_all_screen_LCD_consumable.php
 ...
 * Includes: config.php, paginator.class2.php, tc_calendar.php, content2.php, top_modal_menu.php, left_production_menu.php, td>
           <td><?php echo $data_u[, ]..
 *
...
//----------------------------------------------------		
 ...
 $start_date_check = date('Y-m', strtotime("-1 month"));
 $end_date_check = date('Y-m', strtotime("+2 month"));
	?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div id="content">
 <div id="content-header">
  <div id="breadcrumb"> <a href="index_production.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="tip-bottom">Board</a> <a href="#" class="current">Consumable Request Board</a> </div>
  <h1>Board</h1>
</div>
...
$query = "SELECT *, DATE_FORMAT(MR.date_require,'%d-%m-%Y') AS R FROM consumable_request AS MR, consumable_detail AS SD WHERE MR.id_con = SD.id_con AND MR.status_request = 'Y' AND (MR.status != 'Close' AND MR.status  != 'Cancel') AND (MR.date_require >= '$start_date_check' AND MR.date_require <= '$end_date_check') GROUP BY MR.temp_mrin, MR.id_work ORDER BY MR.date_require, MR.time_require DESC ";
...
?>
      ...
      <div class="widget-box">
          <div class="widget-title"> <span class="icon"><i class="icon-th"></i></span>
            <h5>Consumable Request Board</h5>
          </div>
   ...
   while ($row2 = mysqli_fetch_array($rs))
   {
   	...
   	$query_scan = "SELECT * FROM scan_detail WHERE id_scan = ?"; $query_scan_args = [$row2[5]];
  ...
  //--------------------------------------------------------------------------------------------------------------------------------------------  
	...
	$query_tp = "SELECT *,SUM(rquantity) as TOT FROM post_consumable_detail_header WHERE mrin_no = ? AND mvt_type = 201 AND status_posting = 'New' GROUP BY material_no"; $query_tp_args = [$row2["temp_mrin"]];
	...
	while($row_tp = mysqli_fetch_assoc($result_tp))
{
 ...
 $query_upd2 = "UPDATE `post_consumable_detail_header` SET status_posting = 'Close', date_close = NOW() WHERE mrin_no = ? AND material_no = ? "; $query_upd2_args = [$row2["temp_mrin"], $row_tp["material_no"]];
 $result_upd2 = db_query_bind($dbc, $query_upd2, $query_upd2_args); 
	      
 $query_upd3 = "UPDATE `consumable_request` SET status = 'Close' WHERE temp_mrin = ? AND material_no = ? "; $query_upd3_args = [$row2["temp_mrin"], $row_tp["material_no"]];
 $result_upd3 = db_query_bind($dbc, $query_upd3, $query_upd3_args); 
 
	 $query_upd4 = "UPDATE `consumable_request` SET status = 'Close' WHERE temp_mrin = ? AND (con_qty = '0.000' OR con_qty = '')"; $query_upd4_args = [$row2["temp_mrin"]];
 $result_upd4 = db_query_bind($dbc, $query_upd4, $query_upd4_args); 
        ...
        if($result_upd3 || $result_upd4)
		 {
		   ...
		   $query_upd8 = "SELECT * FROM `consumable_request` WHERE temp_mrin = ? AND status = 'Close'"; $query_upd8_args = [$row2["temp_mrin"]];
		   $result_upd8 = db_query_bind($dbc, $query_upd8, $query_upd8_args);
	...
	$query_mm3 = "SELECT * FROM `cosumable_request` WHERE temp_mrin = '".db_esc($dbc, $row2["temp_mrin"])."' AND status = 'Close' AND material_no = '".db_esc($dbc, $row_tp["material_no"])."'"; 
    $result_mm3 = mysqli_query($dbc, $query_mm3) or die(db_fail($dbc));
	$row_mm3 = mysqli_fetch_array($result_mm3); 
			
			
			$query_mm3_insert =  "INSERT INTO consumable_request_close(id_req_con, mrin_doc, mrin_year, temp_mrin, id_hdr, id_dtl, id_scan, bom_id, bom_qty, bom_oum, status_request, status_print, user_create, date_create, user_update, date_update, date_posting, time_posting, status, bom_component, date_mrin, time_mrin, reason_close, reason_close2) VALUES('".db_esc($dbc, $row_mm3["id_req"])."','".db_esc($dbc, $row_mm3["mrin_doc"])."','".db_esc($dbc, $row_mm3["mrin_year"])."','".db_esc($dbc, $row_mm3["temp_mrin"])."','".db_esc($dbc, $row_mm3["id_hdr"])."','".db_esc($dbc, $row_mm3["id_dtl"])."','".db_esc($dbc, $row_mm3["id_scan"])."','".db_esc($dbc, $row_mm3["bom_id"])."','".db_esc($dbc, $row_mm3["con_qty"])."', '".db_esc($dbc, $row_mm3["bom_oum"])."','".db_esc($dbc, $row_mm3["status_request"])."','".db_esc($dbc, $row_mm3["status_print"])."','".db_esc($dbc, $row_mm3["user_create"])."','".db_esc($dbc, $row_mm3["date_create"])."','".db_esc($dbc, $row_mm3["user_update"])."','".db_esc($dbc, $row_mm3["date_update"])."','".db_esc($dbc, $row_mm3["date_posting"])."','".db_esc($dbc, $row_mm3["time_posting"])."','".db_esc($dbc, $row_mm3["status"])."','".db_esc($dbc, $row_mm3["bom_component"])."','".db_esc($dbc, $row_mm3["date_mrin"])."','".db_esc($dbc, $row_mm3["time_mrin"])."','6','')";
