...
/**
 * prod/print_tag_backflush_tran.php
 * Part of: Production module
 * Filename suggests: print tag backflush tran
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, factory_detail, pps_detail, pps_detail_transaction.
...
// Check, if username session is NOT set then this page will jump to login page$Cdate = date ("l, j F Y ");
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_production.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a><a href="#" class="#">Production</a> <a href="#" class="current">Print Tag</a></div>
  <h1>Print Tag</h1>
</div>
  ...
  <div class="container-fluid">
    ...
    <?php
        ...
        if(isset($_POST["Submit2"]))
        {
		  ...
		  if((($_POST["date1"]) == "0000-00-00") || (($_POST["date1"]) == ""))
     {
	     $dateF = FALSE;
		 $message_psdt = '<span class="badge badge-pill badge-danger"> Please select Planned Start Date !</span>';
	 }else{
		 $dateF = TRUE;
	  }
	  
	  if((($_POST["date2"]) == "0000-00-00") || (($_POST["date2"]) == ""))
     {
	     $dateT = FALSE;
		 $message_psdt2 = '<span class="badge badge-pill badge-danger"> Please select Planned End Date !</span>';
	 }else{
		 $dateT = TRUE;
	  }
	  ...
	  if($dateF && $dateT)
	  {
            ...
            echo "<script>";
echo "window.location='print_tag_backflush_tran2.php?date1=$dateF&&date2=$dateT&&factory=$factory&&work_center=$work_center&&plan_no=$plan_no&&shift_ops=$shift_ops'";
            echo "</script>";
      ...
      <form action="<?php echo h($_SERVER['PHP_SELF']); ?>" method="post" name="frmSearch" id="frmSearch">
            ...
            <tr>
              <th>Planned Start Date :<font color="#FF0000">*</font></th>
              <td><?php
			 ...
			 ?> <div><?php echo $message_psdt; ?></div></td>
              <th>Planned End Date :<font color="#FF0000">*</font></th>
              <td><?php
               ...
               if(isset($_POST['date2']))
                ...
                ?><div><?php echo $message_psdt2; ?></div></td>
            </tr>
 ...
 $start_date_check = date('Y-m', strtotime("0 month"));
 $end_date_check = date('Y-m', strtotime("+1 month"));
 
 // echo $start_date_check; echo "<br>";
  //echo $end_date_check;

								 
   $query8 = "SELECT COUNT(*) FROM pps_detail_transaction WHERE status_pps = ? AND status = 'Y' AND qty_actual != '' AND date_plan = '".$Curr_date."'"; $query8_args = [$rst_sta7["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction WHERE status_pps = ? AND status = 'Y' AND qty_actual != '' AND date_plan = '".$Curr_date."' order by plan_no ASC"; $query_args = [$rst_sta7["status_desc"]];
   ...
   while ($row = mysqli_fetch_array($rs))
   {
	...
	}elseif($sta_out == "3")
	{
      ...
      ?>
                ...
                <td width="99"><a value="Print Tag" href="display_tag_backflush_ts.php?uid=<?php echo h($row["id"]); ?>&&TB_iframe=true&height=600&width=1000" class="thickbox" target="_self"><img src="../img/tag.png" width="16" height="16" alt="Print Tag">Print Tag</a></td>
                </tr>
