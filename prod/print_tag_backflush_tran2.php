...
<?php
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail_transaction AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status_pps = '".$rst_sta7["status_desc"]."' AND MR.status = 'Y' AND MR.qty_actual != ''".$where_sql;
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status_pps = '".$rst_sta7["status_desc"]."' AND MR.status = 'Y' AND MR.qty_actual != ''".$where_sql." ORDER BY MR.plan_no ASC";
   ...
   while ($row = mysqli_fetch_array($rs))
   {
	...
	}elseif($sta_out == "3")
	{
      ...
      ?>
                ...
                <td width="99"><a value="Print Tag" href="display_tag_backflush_ts.php?uid=<?php echo $row["id"]; ?>&&TB_iframe=true&height=400&width=1000" class="thickbox" target="_self"><img src="../img/tag.png" width="16" height="16" alt="Print Tag">Print Tag</a></td>
                </tr>
