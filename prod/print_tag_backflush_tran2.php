...
/**
 * prod/print_tag_backflush_tran2.php
 * Part of: Production module
 * Filename suggests: print tag backflush tran2
 *
       ...
       <form action="<?php echo h($_SERVER['PHP_SELF']); ?>" method="get" name="frmSearch" id="frmSearch">
            ...
            <tr>
              ...
              <td><?php
				      ...
				      $ddF = substr($_GET["date1"],8,2);
					  $mmF = substr($_GET["date1"],5,2);
					  $yyF = substr($_GET["date1"],0,4);
	
	
                      $myCalendar = new tc_calendar("date1", true, false);
					  $myCalendar->setIcon("../calendar/images/iconCalendar.gif");
					  $myCalendar->setDate($ddF, $mmF, $yyF);
...
<?php
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail_transaction AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status_pps = ? AND MR.status = 'Y' AND MR.qty_actual != ''".$where_sql; $query8_args = [$rst_sta7["status_desc"], ...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status_pps = ? AND MR.status = 'Y' AND MR.qty_actual != ''".$where_sql." ORDER BY MR.plan_no ASC"; $query_args = [$rst_sta7["status_desc"], ...$where_args];
   ...
   while ($row = mysqli_fetch_array($rs))
   {
	...
	}elseif($sta_out == "3")
	{
      ...
      ?>
                ...
                <td width="99"><a value="Print Tag" href="display_tag_backflush_ts.php?uid=<?php echo h($row["id"]); ?>&&TB_iframe=true&height=400&width=1000" class="thickbox" target="_self"><img src="../img/tag.png" width="16" height="16" alt="Print Tag">Print Tag</a></td>
                </tr>
