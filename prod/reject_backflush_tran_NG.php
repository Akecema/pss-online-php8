...
/**
 * prod/reject_backflush_tran_NG.php
 * Part of: Production module
 * Filename suggests: reject backflush tran NG
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, factory_detail, run_count_no, pps_detail_transaction, mat_master_header, reject_detail_disposal, type_reject_detail, reason_ng_reject.
  ...
  if(isset($_POST["cancel"])) 
  {
	...
	$string2 = "";
	   ...
	   foreach($_POST["cancel"] as $j=>$i) {
		...
		//echo h($cancel[$i]); echo "-"; echo h($string2[$i]);
		//echo "</br>";
...
onClick="Check(document.myform.cancel)">  </div><?php //echo $no; ?></td>
                ...
                <td width="80"><?php echo h($row["R2"]); ?>  
                <input name="posting_date1[<?php echo h($row["id"]); ?>]" type="hidden" value="<?php echo h($row["date_posting"]); ?>"></td>  
