...
/**
 * prod/reject_backflush_tran_NG2.php
 * Part of: Production module
 * Filename suggests: reject backflush tran NG2
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, factory_detail, work_center_detail, run_count_no, pps_detail_transaction, mat_master_header, reject_detail_disposal, qqc_transaction.
...
$message = NULL; // create an empty new variable.
   ...
   //-------------------generate disposal doc no.[Backflush NG List]---------------
	
	
	$query_id = "SELECT count_max FROM run_count_no WHERE uid = '23'";
	...
	if ($result_id) 
{
  ...
  if(isset($_POST["cancel"])) 
  {
	...
	$string = "";
	  ...
	  $query_info = "SELECT * FROM pps_detail_transaction WHERE id = ?"; $query_info_args = [$cancel[$i]];
	...
	$query_insert2 = "INSERT INTO reject_detail_disposal(id_disposal, doc_dis, doc_disposal_no, bflush_qqc_no, plan_no, uid, material_no, material_desc, material_type, model_code, qty_plan, qty_actual, qty_balance, qty_NG, qty_qc, qty_qc_ok, qty_qc_NG, UOM_unit, comp_code, work_center, shift_day, date_plan, user_posting, date_posting, time_posting, status_disposal, ploc, ploc_prod_reject, ploc_qc_reject, type_reject, reason_reject, user_reject, date_reject, time_reject, qty_wastage, type_wastage, reason_wastage, user_wastage, date_wastage, time_wastage, user_disposal, date_disposal, remarks, approve_by, date_approve, remark_approve, status_part, user_update, date_update, approve_by2, date_approve2, remark_approve2, cost_center,id_factory, disposal_no_ref,user_cancel, date_cancel) VALUES('',?,?,?,?,?,?, ?,?,?,?,?,?,?,'','','',?,'2200',?,'".$sta."',?,?,?,?,?,?,?,'',?,?,?,?,?,'','','','','','',?,NOW(),?,'','','','PR','','','','','','','','','','')"; $query_insert2_args = [$ref, $ref, $data_info["bflush_no"], $data_info["plan_no"], $cancel[$i], $data_info["material_no"], $data_info["material_desc"], $data_info["material_type"], $data_info["model_code"], $data_info["qty_plan"], $data_info["qty_actual"], $data_info["qty_balance"], $data_info["qty_NG"], $data_info2["BUn"], $data_info["work_center"], $data_info["date_plan"], $data_info["user_posting"], $data_info["date_posting"], $data_info["time_posting"], $rst_sta["status_desc"], $ploc, $data_info["ploc"], $data_info["type_reject"], $data_info["reason_reject"], $data_info["user_reject"], $data_info["date_reject"], $data_info["time_reject"], $username, $string[$i]];
$result_insert2 = db_query_bind($dbc, $query_insert2, $query_insert2_args) or die(db_fail($dbc));
		...
		//update table pps_detail_transaction
			...
			//update table pps_detail_transaction
		$query_update2 = "UPDATE pps_detail_transaction SET status = 'Y' WHERE id = ?"; $query_update2_args = [$cancel[$i]];
        $result_update2 = db_query_bind($dbc, $query_update2, $query_update2_args) or die(db_fail($dbc));
   ...
   //update count_max----------------------------------------
       ...
       $query_max_a = "UPDATE run_count_no SET count_max = '".$number."', date_updated = NOW() WHERE uid = '23'";
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail_transaction AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status = 'N' AND MR.bflush_no_ref = ''".$where_sql; $query8_args = [...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status = 'N' AND MR.bflush_no_ref= '' ".$where_sql." ORDER BY MR.plan_no ASC"; $query_args = [...$where_args];
...
onClick="Check(document.myform.cancel)">  </div><?php //echo $no; ?></td>
                ...
                <td width="140"><textarea name="remark_reject[<?php echo h($row["id"]); ?>]" id="textarea" rows="2" cols="10" maxlength="250" ><?php if (isset($_POST['remark_reject'][($row["id"])])) { echo h($_POST['remark_reject'][($row["id"])] ?? ''); } ?></textarea>
