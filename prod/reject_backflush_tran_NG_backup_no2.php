...
/**
 * prod/reject_backflush_tran_NG_backup_no2.php
 * Part of: Production module
 * Filename suggests: reject backflush tran NG backup no2
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, factory_detail, reject_detail_disposal, pps_detail_transaction, mat_master_header, type_reject_detail, reason_ng_reject.
 ...
 if ($result_id_2) {
  ...
  if(isset($_POST["cancel"])) 
  {
	...
	$string2 = "";
	  ...
	  $query_info = "SELECT * FROM pps_detail_transaction WHERE id = '".db_esc($dbc, $cancel[$i])."'";
		...
		if($data_info["shift_pps1"] != "")
	{
	...
	}elseif($data_info["shift_pps2"] != "")
	 {
		...
		$query_insert2 = "INSERT INTO reject_detail_disposal (id_disposal, doc_dis, doc_disposal_no, bflush_qqc_no, plan_no, uid, material_no, material_desc, material_type, model_code, qty_plan, qty_actual, qty_balance, qty_NG, qty_qc, qty_qc_ok, qty_qc_NG, UOM_unit, comp_code, work_center, shift_day, date_plan, user_posting, date_posting, time_posting, status_disposal, ploc, ploc_prod_reject, ploc_qc_reject, type_reject, reason_reject, user_reject, date_reject, time_reject, qty_wastage, type_wastage, reason_wastage, user_wastage, date_wastage, time_wastage, user_disposal, date_disposal, remarks, approve_by, date_approve, remark_approve, status_part, user_update, date_update, approve_by2, date_approve2, remark_approve2, cost_center) VALUES('','".db_esc($dbc, $ref_no)."','".db_esc($dbc, $ref)."','".db_esc($dbc, $data_info["bflush_no"])."','".db_esc($dbc, $data_info["plan_no"])."','".db_esc($dbc, $cancel[$i])."','".db_esc($dbc, $data_info["material_no"])."', '".db_esc($dbc, $data_info["material_desc"])."','".db_esc($dbc, $data_info["material_type"])."','".db_esc($dbc, $data_info["model_code"])."','".db_esc($dbc, $data_info["qty_plan"])."','".db_esc($dbc, $data_info["qty_actual"])."','".db_esc($dbc, $data_info["qty_balance"])."','".db_esc($dbc, $data_info["qty_NG"])."','','','','".db_esc($dbc, $data_info2["BUn"])."','".db_esc($dbc, $data_info["comp_code"])."','".db_esc($dbc, $data_info["work_center"])."','".$sta."','".db_esc($dbc, $data_info["date_plan"])."','".db_esc($dbc, $data_info["user_posting"])."','".db_esc($dbc, $data_info["date_posting"])."','".db_esc($dbc, $data_info["time_posting"])."','".db_esc($dbc, $rst_sta["status_desc"])."','".db_esc($dbc, $ploc)."','".db_esc($dbc, $data_info["ploc"])."','','".db_esc($dbc, $data_info["type_reject"])."','".db_esc($dbc, $data_info["reason_reject"])."','".db_esc($dbc, $data_info["user_reject"])."','".db_esc($dbc, $data_info["date_reject"])."','".db_esc($dbc, $data_info["time_reject"])."','','','','','','','".db_esc($dbc, $username)."',NOW(),'".db_esc($dbc, $string[$i])."','','','','PR','','','','','','')";
