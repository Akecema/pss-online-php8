...
/**
 * prod/report_pps_close_download_selected.php
 * Part of: Production module
 * Filename suggests: report pps close download selected
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: sys_setup_maintain, request_status, work_center_detail, pps_detail_close, mat_master_header, pps_detail_transaction, qqc_detail_transaction, qqc_transaction.
 * Includes: config.php.
 *
...
$namaFile = "Closed Planned Order_".$date_tdy.".xls";
 //convert material no kpd id_hdr
			
		
    $query8 = "SELECT * FROM pps_detail_close AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status_pps = ? AND MR.status = 'Y'".$where_sql." GROUP BY MR.plan_no"; $query8_args = [$rst_sta13["status_desc"], ...$where_args];
...
$content .= "<font size='12px'><strong>CLOSED PLANNED ORDER REPORT</strong></font> ";
$content .= "<br>";
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_posting,'%H:%i:%s') as T2 FROM pps_detail_close AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status_pps = ? AND MR.status = 'Y' ".$where_sql." GROUP BY MR.plan_no order by MR.plan_no ASC"; $query_args = [$rst_sta13["status_desc"], ...$where_args];
...
//count how many data
 ...
 while ($row2 = mysqli_fetch_array($rs))
   {
	    //---------get material header---------
	    $query_mat_h = "SELECT * FROM mat_master_header AS HD, mat_master_detail AS AD WHERE HD.material_no = AD.material AND HD.material_no = ?"; $query_mat_h_args = [$row2["material_no"]];
   ...
   $query_display = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as RR, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as RR2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as RR3 FROM pps_detail_transaction AS MR WHERE MR.pps_id = ?"; $query_display_args = [$row2["id"]];
 ...
 $query_display3 = "SELECT *, DATE_FORMAT(N.date_plan,'%d-%m-%Y') as B, DATE_FORMAT(N.date_qc_posting,'%d-%m-%Y') as B2, DATE_FORMAT(N.date_create,'%d-%m-%Y') as B3 FROM qqc_detail_transaction AS N WHERE N.bflush_no = ?"; $query_display3_args = [$row_display["bflush_no"]];
