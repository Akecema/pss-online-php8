...
/**
 * prod/report_prod_consumableProc.php
 * Part of: Production module
 * Filename suggests: report prod consumableProc
 *
...
<div class="container-fluid">
 ...
 $query8 = "SELECT * FROM consumable_request AS MR WHERE MR.status_request = 'Y' AND MR.status != 'Cancel'" .$where_sql." GROUP BY MR.id_scan ORDER BY MR.id_req_con ASC"; $query8_args = [...$where_args];
...
$query = "SELECT * FROM consumable_request AS MR WHERE MR.status_request = 'Y' AND MR.status != 'Cancel'".$where_sql." GROUP BY MR.id_scan ORDER BY MR.id_req_con ASC"; $query_args = [...$where_args];
    ...
    while ($row2 = mysqli_fetch_array($rs))
   {
    ...
    <td>&nbsp;<?php if ($row2["status"] == "Close") {  ?><font color="#FF0000"><b><?php echo h($row2["temp_mrin"]); ?> </b></font> <?php } else { ?><font color="#000099"><b><?php echo h($row2["temp_mrin"]); ?></b></font> <?php }  ?></td>
     ...
     <td>&nbsp;</td>
         ...
         <?php
	$query_again = "SELECT * FROM consumable_request AS MR, consumable_detail AS SD WHERE MR.material_no = SD.material_no AND status_request = 'Y' and id_scan = ? ORDER BY id_req_con ASC"; $query_again_args = [$row2[5]];
