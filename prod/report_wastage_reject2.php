...
/**
 * prod/report_wastage_reject2.php
 * Part of: Production module
 * Filename suggests: report wastage reject2
 *
...
<div id="content-header">
  ...
  <div class="container-fluid">
              ...
              <tr>
                ...
                <select name="docF" id="docF">
                  ...
                  <?php
						...
						{
						?>
                  <option value="<?php echo h($rowd['doc_disposal_no'])?>" <?php if($rowd["doc_disposal_no"] == $_GET["docF"]) echo "selected"; ?> ><?php echo h($rowd['doc_disposal_no'])?></option>
                  <?php
						}
					  ?>
                </select>
               ...
               <select name="docT" id="docT">
                  ...
                  <?php
					...
					{
					?>
                  <option value="<?php echo h($rowd2['doc_disposal_no'])?>" <?php if($rowd2["doc_disposal_no"] == $_GET["docT"]) echo "selected"; ?>> <?php echo h($rowd2['doc_disposal_no'])?></option>
                  <?php
						}
					  ?>
                </select></td>
	...
	$query_convert = "SELECT * FROM `work_center_detail` WHERE id_factory = ? "; $query_convert_args = [$_GET["factory"]];
	...
	if($dateF == "0000-00-00") {
		 $wheresql_01 = ""; $wheresql_01_args = []; }
		  ...
		  $wheresql_01 = " AND (MR.date_posting >= ? )"; $wheresql_01_args = [$dateF]; }
		  
	// 2. DateT
	if($dateT == "0000-00-00") {
		 $wheresql_02 = ""; $wheresql_02_args = []; }
	else {
		  $wheresql_02 = " AND (MR.date_posting <= ? )"; $wheresql_02_args = [$dateT]; }
	...
	if ($factory == "NULL"){ 
		...
		$wheresql_03 = " AND MR.id_factory = ? "; $wheresql_03_args = [$factory]; } 
	...
	if ($docF == "NULL" ){
		...
		$wheresql_05 = " AND MR.doc_disposal_no >= ?"; $wheresql_05_args = [$docF]; }
	
	//6. document no
	if ($docT == "NULL" ){
		$wheresql_06 = ""; $wheresql_06_args = []; }
	else {
		$wheresql_06 = " AND MR.doc_disposal_no <= ?"; $wheresql_06_args = [$docT]; }
	...
	$query8 = "SELECT COUNT(*) FROM  reject_detail_disposal AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status_part = 'WS' AND MR.qty_wastage != '' AND MR.doc_disposal_no != '' ".$where_sql; $query8_args = [...$where_args];
	...
	$query = "SELECT *, DATE_FORMAT(MR.date_wastage,'%d-%m-%Y  %H:%i:%s') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2 FROM reject_detail_disposal AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status_part = 'WS' AND MR.qty_wastage != '' AND MR.doc_disposal_no != '' ".$where_sql." ORDER BY MR.plan_no ASC"; $query_args = [...$where_args];
...
?>
    ...
    <tr>
        ...
        <td width="85%"> <div class="small-nav"></div></td> 
          <td width="7%"><a href="report_wastage_reject_download.php?date1=<?php echo h($dateF); ?>&&date2=<?php echo h($dateT); ?>&&factory=<?php echo h($factory); ?>&&work_center=<?php echo h($work_center); ?>&&docF=<?php echo h($docF); ?>&&docT=<?php echo h($docT); ?>" ><img src="../img/dload_excel.jpg" width="48" height="48" title="Download" /></a></td>
