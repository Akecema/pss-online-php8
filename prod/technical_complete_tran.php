	       ...
	       $query3 = "SELECT * FROM factory_detail GROUP BY factory_desc2 ORDER BY id_fac ASC";
            ...
            <th>Plan Category :</th>
              ...
              <td><select name="status" id="status" class="span11">
                  ...
                  <option value="<?php echo h($rst_sta7["status_desc"]); ?>"><?php echo h($rst_sta7["status_desc"]); ?></option>
                  </select></td>
                     </tr>
            <tr>
              <th>&nbsp;</th>
...
// echo $start_date_check;
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail AS PD WHERE (PD.status_pps = ? OR PD.status_pps = ?) AND PD.status = 'Y' AND (PD.date_posting >= '$start_date_check' AND PD.date_posting <= '$end_date_check')"; $query8_args = [$rst_sta2["status_desc"], $rst_sta7["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(PD.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(PD.date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(PD.date_create,'%d-%m-%Y') as R3 FROM pps_detail AS PD WHERE (PD.status_pps = ? OR PD.status_pps = ?) AND PD.status = 'Y' AND (PD.date_posting >= '$start_date_check' AND PD.date_posting <= '$end_date_check') ORDER BY PD.plan_no ASC"; $query_args = [$rst_sta2["status_desc"], $rst_sta7["status_desc"]];
