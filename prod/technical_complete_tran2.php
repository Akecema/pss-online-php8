...
// $size2 = count($_POST["id"]) + 1;
 $cancel = $_POST["cancel"]; 
  ...
  <?php
		  ...
		  //6. Shift
		   ...
		   //7. plan category
                if ($plan_category == "NULL"){ 
                    ...
                    $wheresql_07 = " AND MR.plan_category = '".db_esc($dbc, $plan_category)."' "; } 	
		   ...
		   //8. status
                if ($status == "NULL"){ 
                    ...
                    $wheresql_08 = " AND MR.status_pps = '".db_esc($dbc, $status)."' "; } 			 			
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status = 'Y' AND (MR.status_pps != '".db_esc($dbc, $rst_sta13["status_desc"])."' AND MR.status_pps != '".db_esc($dbc, $rst_sta4["status_desc"])."' AND MR.status_pps != '".db_esc($dbc, $rst_sta16["status_desc"])."' AND MR.status_pps != '".db_esc($dbc, $rst_sta18["status_desc"])."')".$where_sql;
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM pps_detail AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND (MR.status_pps != '".db_esc($dbc, $rst_sta13["status_desc"])."' AND MR.status_pps != '".db_esc($dbc, $rst_sta4["status_desc"])."' AND MR.status_pps != '".db_esc($dbc, $rst_sta16["status_desc"])."' AND MR.status_pps != '".db_esc($dbc, $rst_sta18["status_desc"])."') AND MR.status = 'Y' ".$where_sql."order by MR.plan_no ASC";
