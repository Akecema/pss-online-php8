...
/**
 * prod/wip_request_listProc.php
 * Part of: Production module
 * Filename suggests: wip request listProc
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, scan_detail_wip, mat_master_detail, wip_request, mat_master_header.
...
<div class="container-fluid">
 ...
 <?php
	   ...
	   $query_scan = "SELECT * FROM scan_detail_wip WHERE id_scan = ?"; $query_scan_args = [$uid];
...
?></td>
  ...
  <tr>
    ...
    <td colspan="5">&nbsp;</td>
	   ...
	   $query_component = "SELECT * FROM mat_master_header AS h, mat_master_detail AS s WHERE h.id_hdr = s.id_hdr AND s.material = ? AND s.bom_status = 'Y' AND s.mat_type = 'Z200'"; $query_component_args = [$data_scan["material_no"]];
...
onClick="Check(document.myform.cancel)">  </div></td>
                 ...
                 <td width="30%"><input name="comp_quantity[<?php echo h($row["id_dtl"]); ?>]" type="text" value="<?php if(isset($_POST["comp_quantity"])) { echo h($_POST["comp_quantity"][($row["id_dtl"])] ?? ''); }else{  echo h($num_convert);   } ?>" /></td>
