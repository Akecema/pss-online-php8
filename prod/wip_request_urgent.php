...
/**
 * prod/wip_request_urgent.php
 * Part of: Production module
 * Filename suggests: wip request urgent
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, scan_detail_wip, mat_master_header, factory_detail.
...
$query_db = "INSERT INTO `scan_detail_wip` (id_scan, pps_ref, factory, work_center, prod_order, material_no, scan_oum, scan_plant, scan_sloc, scan_qty, user_create, date_create, user_update, date_update, status_urgent) VALUES ('".mysqli_insert_id($dbc)."', '', ?, ?, ?, ?, '', '', ?, '', ?, NOW(),'','','Y')"; $query_db_args = [$factory, $work_center, $prod_order, $material_no, $factory, $user_no];
$result = db_query_bind($dbc, $query_db, $query_db_args) or die(db_fail($dbc));
	...
	$query_update = "UPDATE `scan_detail_wip` SET scan_oum = ?, scan_plant = ? WHERE id_scan = ?"; $query_update_args = [$data_sql2["BUn"], $data_sql2["plant"], $data_sql["id_scan"]];
	$result_update =  db_query_bind($dbc, $query_update, $query_update_args);
			 
			 
			 
echo "<script>";
echo "window.location='wip_request_urgentProc.php?uid=$data_sql[id_scan]'";
echo "</script>";
...
<div class="widget-content nopadding">
         <form name="form1" method="post" action="wip_request_urgent.php" class="form-horizontal">
	       ...
	       $query2 = "SELECT * FROM mat_master_header ORDER BY material_no ASC";
			      ...
			      {
                  echo'<option value="',h($row2[1]),'">',stripslashes($row2[1]),' - ',stripslashes($row2[2]),'</option>';
                  }
				?>
                   </select>
              </div>
