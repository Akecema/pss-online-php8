...
/**
 * prod_super/approval_disposal_qc_new_tran_NG_wastage.php
 ...
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, reject_detail_disposal, wastage_transaction, type_wastage_detail, reason_wastage, work_center_detail, mat_master_header, pps_detail.
...
//------------update remarks reject detail disposal-------------------
 ...
 $result_upd3 = db_query_bind($dbc, $query_upd3, $query_upd3_args); 
  ...
  // //----------------------update table wastage_disposal
	 ...
	 $qry = db_query_bind($dbc, "SELECT *, DATE_FORMAT(A1.date_disposal,'%Y-%m-%d %H:%i:%s') AS M, DATE_FORMAT(A1.date_approve,'%Y-%m-%d %H:%i:%s') AS M2 FROM reject_detail_disposal AS A1, wastage_transaction AS A2 WHERE A2.id_wastage_tran = A1.uid AND A1.doc_disposal_no  = ?", [$doc_disposal]);
$data = "";
...
/*  $data .= $row_1['doc_disposal_no'].";".$row_1['id_disposal']."\r\n";*/
   ...
   $data .= $row_1['material_type'].";".$row_1['model_code'].";".$row_1['material_no'].";551;".";".$row_fac['id_factory'].";".$row_1['work_center'].";".$row_1['ploc_prod_reject'].";".$row_type['wastage_desc'].";".$row_reason['reason_wastage_desc'].";".$row_1['qty_wastage'].";".$row_1['UOM_unit'].";".$res['user_fullname'].";".$row_1['M'].";".$row_1['M2']."\r\n";
   
}
	 
		 
/* $filen= "GIPRD".date("YmdHis",time());
//$csv_filename = $filen."_".date("YmdHis",time());

$file = "../FromPortal2/".$filen.".csv";
//chmod($file, 0777);
          ...
          <form name="myform" action="approval_disposal_qc_new_tran_NG_wastage.php?doc_disposal=<?php echo h($doc_disposal); ?>" method="post" >
              ...
              <thead>
               <tr>
                 ...
                 <th bgcolor="#E9F58D"><span class="style3">Remarks Approval</span></th>
               </tr>
             </thead>  
             <?php
      $counter = 1;
...
onClick="Check(document.myform.cancel)"> <?php  echo h($row["id_disposal"]); ?></td>
                ...
                <td width="200"><textarea name="remark_approve[<?php echo h($row["id_disposal"]); ?>]2" id="remark_approve[<?php echo h($row["id_disposal"]); ?>]" rows="4" cols="10" maxlength="250"></textarea> <input name="id_disposal[<?php echo h($row["id_disposal"]); ?>]" type="hidden" value="<?php echo h($row["id_disposal"]); ?>"></td>
      </tr>
    </tbody>        
