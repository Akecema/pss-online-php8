...
//------------update remarks reject detail disposal-------------------
 ...
 $result_upd3 = mysqli_query($dbc, $query_upd3); 
  ...
  // //----------------------update table wastage_disposal
	  $query_wastage_update = "UPDATE wastage_transaction SET user_update = '".db_esc($dbc, $username)."', date_update = NOW(), remark_approve = '".db_esc($dbc, $row_all["remark_approve"])."', approve_by = '".db_esc($dbc, $username)."', date_approve = NOW(), status_disposal = '".db_esc($dbc, $rst_sta3["status_desc"])."' WHERE id_wastage_tran = '".db_esc($dbc, $row_all["uid"])."'";
	 ...
	 $qry = mysqli_query($dbc, "SELECT *, DATE_FORMAT(A1.date_disposal,'%Y-%m-%d %H:%i:%s') AS M, DATE_FORMAT(A1.date_approve,'%Y-%m-%d %H:%i:%s') AS M2 FROM reject_detail_disposal AS A1, wastage_transaction AS A2 WHERE A2.id_wastage_tran = A1.uid AND A1.doc_disposal_no  = '".db_esc($dbc, $doc_disposal)."'");
$data = "";
  ...
  <tr>
    ...
    <td width="38%"><table width="98%" border="0" cellpadding="2">
      ...
      <!-- Content -->
            ...
            <div class="form">
              <!-- End Box Head -->
          <form name="myform" action="approval_disposal_qc_new_tran_NG_wastage.php?doc_disposal=<?php echo $doc_disposal; ?>" method="post" >
...
onClick="Check(document.myform.cancel)"> <?php  echo $row["id_disposal"]; ?></td>
                ...
                <td width="200"><textarea name="remark_approve[<?php echo $row["id_disposal"]; ?>]2" id="remark_approve[<?php echo $row["id_disposal"]; ?>]" rows="4" cols="10" maxlength="250"></textarea> <input name="id_disposal[<?php echo $row["id_disposal"]; ?>]" type="hidden" value="<?php echo $row["id_disposal"]; ?>"></td>
      </tr>
    </tbody>        
...
<table width="1000" border="0" cellpadding="1" cellspacing="2">
         ...
         <input type="submit" onClick="return confirm('Are you sure you want to approve this disposal? : <?php echo $doc_disposal; ?>?');" name="Submit2" id="button" value="Approve" class="btn btn-success"/>
