...
<?php
 ...
 $no = sprintf('%03d', $no);
  ...
  <tr>
	...
	$query_display_reason = "SELECT * from `consumable_request_close` WHERE temp_mrin = '".db_esc($dbc, $temp_mrin)."' AND status = 'Close'";
    $result_display_reason = mysqli_query($dbc, $query_display_reason);
    $row_display_reason = mysqli_fetch_array($result_display_reason);
	
	$query_reason_tbl = "SELECT * from `reason_req_close` WHERE id_close = '".db_esc($dbc, $row_display_reason["reason_close"])."'";
  ...
  <tr>
    ...
    <th width="3%">:</th>
    <td width="84%"><?php echo $row_reason_tbl["reason_desc"].' - ' . $row_display_reason["reason_close2"]; ?></td>
  </tr>
  </thead>
  </table> 
<?php
