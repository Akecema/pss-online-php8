...
/**
 * prod_super/detail_consumable_request_reprinting.php
...
<?php
	...
	$query_update_print = "UPDATE consumable_request SET status_print = 'Y' WHERE temp_mrin = '".db_esc($dbc, $temp_mrin)."'";
...
<?php
 ...
 $no = sprintf('%03d', $no);
  ...
  <tr>
	...
	$query_display_reason = "SELECT * from `consumable_request_close` WHERE temp_mrin = ? AND status = 'Close'"; $query_display_reason_args = [$temp_mrin];
	...
	$query_reason_tbl = "SELECT * from `reason_req_close` WHERE id_close = ?"; $query_reason_tbl_args = [$row_display_reason["reason_close"]];
  ...
  <thead>
  <tr>
    <th width="13%"><div align="right">Reason</div></th>
    <th width="3%">:</th>
    <td width="84%"><?php echo h($row_reason_tbl["reason_desc"]).' - ' . h($row_display_reason["reason_close2"]); ?></td>
  </tr>
  </thead>
  </table> 
<?php
