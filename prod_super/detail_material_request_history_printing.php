...
<tr>
 ...
 $no = sprintf('%03d', $no);
   ...
   $query_scan = "SELECT * FROM scan_detail WHERE id_scan = '".$row["id_scan"]."'";
   $result_scan = mysqli_query($dbc, $query_scan);
   $row_scan = mysqli_fetch_array($result_scan);
   		 
   $query1_p = "SELECT * FROM scan_detail WHERE id_scan = '".$row["id_scan"]."' GROUP BY id_scan";
	...
	} elseif($row4_p["mat_type"] == "Z210")	 
	{
	...
	} elseif($row4_p["mat_type"] == "Z310")
	 {
	  ...
	  $sta = "Invalid";
		...
		$query_tp = "SELECT *,SUM(rquantity) as TOT FROM post_detail_header WHERE mrin_no = '".$row["temp_mrin"]."' AND prod_order = '".$row_scan["prod_order"]."' AND mvt_type = 311 AND material_no = '".$row4_p["bill_component"]."'";
