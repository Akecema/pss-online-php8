...
<tr>
 ...
 $no = sprintf('%03d', $no);
  ...
  $query4_p = "SELECT * from mat_master_header as LD, mat_master_detail as SD WHERE SD.id_hdr = LD.id_hdr and SD.id_dtl = '".db_esc($dbc, $row["id_dtl_wip"])."'";
...
?>
	...
	$query_display_reason = "SELECT * from `wip_request_cancel` WHERE temp_mrin_wip = '".db_esc($dbc, $temp_mrin)."' AND status = 'Cancel'";
...
<?php
	...
	$query_display_reason2 = "SELECT * from `wip_request_close` WHERE temp_mrin_wip = '".db_esc($dbc, $temp_mrin)."' AND status = 'Close'";
