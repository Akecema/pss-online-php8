...
/**
 * prod_super/detail_wip_request.php
 * Part of: Production module (supervisor/admin tier)
 * Filename suggests: detail wip request
 *
 * Behavior: requires an active login session ($_SESSION['username']); reads parameters from the query string ($_GET).
 * Database tables referenced: user_detail, sys_setup_maintain, wip_request, factory_detail, scan_detail_wip, mat_master_header, wip_request_cancel, reason_req_cancel, wip_request_close, reason_req_close.
...
<tr>
 ...
 $no = sprintf('%03d', $no);
  ...
  $query4_p = "SELECT * from mat_master_header as LD, mat_master_detail as SD WHERE SD.id_hdr = LD.id_hdr and SD.id_dtl = ?"; $query4_p_args = [$row["id_dtl_wip"]];
  $result4_p = db_query_bind($dbc, $query4_p, $query4_p_args);
  $row4_p = mysqli_fetch_array($result4_p); 
		  	 
	if(($row4_p["mat_type"] == "Z210") && ($data_2["factory"] == "1"))
	{
	  $sta = "S120";
	} elseif(($row4_p["mat_type"] == "Z210") && ($data_2["factory"] == "2"))
	{
	  $sta = "S220";
	} elseif(($row4_p["mat_type"] == "Z210") && ($data_2["factory"] == "3"))
	 {
	  $sta = "S220";
...
?>
	...
	$query_display_reason = "SELECT * from `wip_request_cancel` WHERE temp_mrin_wip = ? AND status = 'Cancel'"; $query_display_reason_args = [$temp_mrin];
    ...
    <th width="3%" >:</th>
    <th width="84%" ><?php echo h($row_reason_tbl["reason_desc_cancel"]).' - ' . h($row_display_reason["reason_cancel2"]); ?><br></th>
  </tr></thead>
</table>
<?php
  }
	...
	$query_display_reason2 = "SELECT * from `wip_request_close` WHERE temp_mrin_wip = ? AND status = 'Close'"; $query_display_reason2_args = [$temp_mrin];
   ...
   <tr>
    <th width="13%">Reason</th>
    <th width="3%">:</th>
    <th width="84%"><?php echo h($row_reason_tbl2["reason_desc"]).' - ' . h($row_display_reason2["reason_close2"]); ?><br></th>
  </tr></thead>
</table>
<?php
  }
