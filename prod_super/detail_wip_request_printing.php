...
<tr>
 ...
 $no = sprintf('%03d', $no);
   ...
   $query_scan = "SELECT * FROM scan_detail_wip WHERE id_scan = '".$row["id_scan_wip"]."'";
   $result_scan = mysqli_query($dbc, $query_scan);
   $row_scan = mysqli_fetch_array($result_scan);

		 
   $query1_p = "SELECT * FROM scan_detail_wip WHERE id_scan = '".$row["id_scan_wip"]."' GROUP BY id_scan";
   $result1_p = mysqli_query($dbc, $query1_p);
   $row1_p = mysqli_fetch_array($result1_p);
	
		 
  $query4_p = "SELECT * from mat_master_header as LD, mat_master_detail as SD WHERE SD.id_hdr = LD.id_hdr and SD.id_dtl = '".$row["id_dtl_wip"]."'";
