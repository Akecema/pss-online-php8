...
/**
 * prod_super/disposal_backflush_tran_NG2_approved.php
 * Part of: Production module (supervisor/admin tier)
 * Filename suggests: disposal backflush tran NG2 approved
 *
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_production_super.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a><a href="#" class="#">Production</a> <a href="#"><b>Approval Disposal</b></a></div>
  <h1>Approval Disposal</h1>
</div>
  ...
  <div class="container-fluid">
         ...
         <div class="widget-box">
          <div class="widget-title">
             <ul class="nav nav-tabs" role="tablist" id="templatemo-tabs">
              <li><a role="tab" href="disposal_backflush_tran_NG.php">New Disposal</a></li>
              <li class="active"><a role="tab" href="disposal_backflush_tran_NG_approved.php">Approved Disposal</a></li>
             </ul>
          </div>
          </div>
			 ...
			 ?></td>
              ...
              <td><select name="doc_disposal_no" id="doc_disposal_no" class="span8">
               <option value="NULL" placeholder="Select Disposal Doc. No.">-Disposal Doc. No.-</option>
                  <?php
	       $query9 = "SELECT * FROM reject_detail_disposal WHERE (status_part = 'PR' OR status_part = 'WS') AND doc_disposal_no != '' AND (status_disposal = ? OR status_disposal = ? OR status_disposal = ?) GROUP BY doc_disposal_no"; $query9_args = [$rst_sta3["status_desc"], $rst_sta17["status_desc"], $rst_sta4["status_desc"]];
		 ...
		 // 1. DateT
                if($dateT == "0000-00-00") {
                     $wheresql_01 = ""; $wheresql_01_args = []; }
                      ...
                      $wheresql_01 = " AND (date_posting <= ?)"; $wheresql_01_args = [$dateT]; }
		  //2. DateF 
                if ($dateF  == "0000-00-00" ){
                     $wheresql_02 = ""; $wheresql_02_args = [];}
                else {
                     $wheresql_02 = " AND (date_posting >= ?)"; $wheresql_02_args = [$dateF];}  
					 
		 //3. Doc. Disposal No. 
                if ($doc_disposal_no == "NULL"){ 
                    $wheresql_03 = ""; $wheresql_03_args = []; }
                else {
                    $wheresql_03 = " AND doc_disposal_no = ?"; $wheresql_03_args = [$doc_disposal_no]; } 
   ...
   $query8 = "SELECT * FROM reject_detail_disposal WHERE (status_part = 'PR' OR status_part = 'WS') AND doc_disposal_no != '' AND (status_disposal = ? OR status_disposal = ? OR status_disposal = ?) ".$where_sql." GROUP BY doc_disposal_no"; $query8_args = [$rst_sta3["status_desc"], $rst_sta17["status_desc"], $rst_sta4["status_desc"], ...$where_args];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2 FROM reject_detail_disposal WHERE (status_part = 'PR' OR status_part = 'WS') AND doc_disposal_no != '' AND (status_disposal = ? OR status_disposal = ? OR status_disposal = ?)".$where_sql." GROUP BY doc_disposal_no ORDER BY plan_no ASC $pages->limit"; $query_args = [$rst_sta3["status_desc"], $rst_sta17["status_desc"], $rst_sta4["status_desc"], ...$where_args];
      ...
      ?>
              <tbody>
			   ...
			   { 
				   ...
				   {
				   ?> 
                <a value="Approval" href="approval_disposal_qc_new_tran_NG.php?doc_disposal=<?php echo h($row["doc_disposal_no"]); ?>&&TB_iframe=true&height=400&width=1100" class="thickbox" target="_self"><img src="../img/tick.png" width="16" height="16" alt="Approval">Approval</a>
                <?php 
				   }elseif($row["status_part"] == "WS")
				   {
					?>   
				<a value="Approval" href="approval_disposal_qc_new_tran_NG_wastage.php?doc_disposal=<?php echo h($row["doc_disposal_no"]); ?>&&TB_iframe=true&height=400&width=1100" class="thickbox" target="_self"><img src="../img/tick.png" width="16" height="16" alt="Approval">Approval</a>
