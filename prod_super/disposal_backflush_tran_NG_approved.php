...
/**
 * prod_super/disposal_backflush_tran_NG_approved.php
 * Part of: Production module (supervisor/admin tier)
 * Filename suggests: disposal backflush tran NG approved
 *
...
<?php
        if(isset($_POST["Submit2"]))
        {
            ...
            echo "<script>";
echo "window.location='disposal_backflush_tran_NG2_approved.php?date1=$dateF&&date2=$dateT&&doc_disposal_no=$doc_disposal_no'";
            echo "</script>";
   ...
   $query8 = "SELECT * FROM reject_detail_disposal WHERE (status_part = 'PR' OR status_part = 'WS') AND doc_disposal_no != '' AND (status_disposal = ? OR status_disposal = ? OR status_disposal = ?) GROUP BY doc_disposal_no ORDER BY plan_no ASC"; $query8_args = [$rst_sta3["status_desc"], $rst_sta17["status_desc"], $rst_sta4["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2 FROM reject_detail_disposal WHERE (status_part = 'PR' OR status_part = 'WS') AND doc_disposal_no != '' AND (status_disposal = '".db_esc($dbc, $rst_sta3["status_desc"])."' OR status_disposal = '".db_esc($dbc, $rst_sta17["status_desc"])."' OR status_disposal = '".db_esc($dbc, $rst_sta4["status_desc"])."') GROUP BY doc_disposal_no ORDER BY plan_no ASC $pages->limit";
...
<tr>
    <td width="1%">&nbsp;</td> 
      <td>&nbsp;</td>
      <td width="7%"><a href="disposal_backflush_tran_approved_download.php" ><img src="../img/dload_excel.jpg" width="48" height="48" title="Download" /></a></td>
   ...
   while ($row = mysqli_fetch_array($rs))
   {
      ...
      ?>
              <tbody>
                ...
                <td width="93">
               <?php  
			   if(($row["status_disposal"] == "Approved") || ($row["status_disposal"] == "Approved QC"))
		   { 
