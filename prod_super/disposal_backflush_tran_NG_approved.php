...
echo "window.location='disposal_backflush_tran_NG2_approved.php?date1=$dateF&&date2=$dateT&&doc_disposal_no=$doc_disposal_no'";
   ...
   $query8 = "SELECT * FROM reject_detail_disposal WHERE (status_part = 'PR' OR status_part = 'WS') AND doc_disposal_no != '' AND (status_disposal = '".db_esc($dbc, $rst_sta3["status_desc"])."' OR status_disposal = '".db_esc($dbc, $rst_sta17["status_desc"])."' OR status_disposal = '".db_esc($dbc, $rst_sta4["status_desc"])."') GROUP BY doc_disposal_no ORDER BY plan_no ASC";
