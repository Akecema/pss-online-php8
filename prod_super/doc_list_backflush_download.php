...
/**
 * prod_super/doc_list_backflush_download.php
...
$namaFile = "Backflush Document List_".$date_tdy2.".xls";
 //convert material no kpd id_hdr
    ...
    $query8 = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction WHERE status_pps = ? AND (date_plan >= '$start_date_check' AND date_plan <= '$end_date_check') order by plan_no ASC"; $query8_args = [$rst_sta7["status_desc"]];
