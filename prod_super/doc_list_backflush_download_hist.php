...
/**
 * prod_super/doc_list_backflush_download_hist.php
 * Part of: Production module (supervisor/admin tier)
 * Filename suggests: doc list backflush download hist
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: sys_setup_maintain, request_status, pps_detail_transaction_v1, mat_master_header, qqc_detail_transaction, qqc_transaction.
 * Includes: config.php.
 *
...
$namaFile = "Backflush Document List_".$date_tdy2.".xls";
 //convert material no kpd id_hdr
    ...
    $query8 = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction_v1 WHERE status_pps = ? AND (date_plan >= '$start_date_check' AND date_plan <= '$end_date_check') order by plan_no ASC"; $query8_args = [$rst_sta7["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction_v1 WHERE status_pps = ? AND (date_plan >= '$start_date_check' AND date_plan <= '$end_date_check') order by plan_no ASC"; $query_args = [$rst_sta7["status_desc"]];
...
//count how many data
  ...
  while ($row2 = mysqli_fetch_array($rs))
   ...
   $query_display = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as RR, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as RR2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as RR3 FROM pps_detail_transaction_v1 AS MR WHERE MR.pps_id = ?"; $query_display_args = [$row2["id"]];
