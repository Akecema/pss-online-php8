...
/**
 * prod_super/index_production_super.php
 * Part of: Production module (supervisor/admin tier)
 * Filename suggests: index production super
 *
 * Behavior: requires an active login session ($_SESSION['username']).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, login_detail, material_request, consumable_request, wip_request, reject_detail_disposal.
...
exit();
	...
	$url = "index_production_super.php"; 
...
$query_disposal_req = "SELECT * FROM reject_detail_disposal WHERE status_disposal = ? AND (status_part = 'PR' OR status_part = 'WS') AND doc_disposal_no != '' ORDER BY date_plan DESC"; $query_disposal_req_args = [$rst_sta["status_desc"]];
