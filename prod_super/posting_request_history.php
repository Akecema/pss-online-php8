...
/**
 * prod_super/posting_request_history.php
 * Part of: Production module (supervisor/admin tier)
 * Filename suggests: posting request history
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, factory_detail, material_request_close, scan_detail, material_request, mat_master_header.
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_production_super.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="tip-bottom">Material Request</a> <a href="#" class="current">Material Request History</a> </div>
...
<?php
       ...
       if(isset($_POST["Submit2"]))
        {
            ...
            echo "<script>";
			echo "window.location='posting_request_history2.php?temp_mrin=$temp_mrin&&prod_order=$prod_order&&date1=$dateF&&date2=$dateT&&work_center=$work_center&&factory=$factory'";
	 ...
	 $query8 = "SELECT *, DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') AS R2 FROM material_request_close AS MR, scan_detail AS SD WHERE MR.id_scan = SD.id_scan AND MR.status_request = 'Y' AND MR.status = 'Close' GROUP BY MR.temp_mrin";
...
$query = "SELECT *, DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') AS R2 FROM material_request_close AS MR, scan_detail AS SD WHERE MR.id_scan = SD.id_scan AND MR.status_request = 'Y' AND MR.status = 'Close' GROUP BY MR.temp_mrin ORDER BY MR.date_posting ASC, MR.temp_mrin ASC";
