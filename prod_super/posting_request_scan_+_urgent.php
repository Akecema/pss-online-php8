...
/**
 * prod_super/posting_request_scan_+_urgent.php
 * Part of: Production module (supervisor/admin tier)
 * Filename suggests: posting request scan + urgent
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, factory_detail, material_request, scan_detail, material_request_close, material_request_cancel.
...
echo "window.location='posting_request_scan_+_urgent2.php?temp_mrin=$temp_mrin&&prod_order=$prod_order&&date1=$dateF&&date2=$dateT&&work_center=$work_center&&factory=$factory'";
    ...
    $query8 = "SELECT *, DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') AS R2 FROM material_request AS MR, scan_detail AS SD WHERE MR.id_scan = SD.id_scan AND MR.status_request = 'Y' GROUP BY MR.temp_mrin  ORDER BY MR.id_req ASC";
...
$query = "SELECT *, DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') AS R2 FROM material_request AS MR, scan_detail AS SD WHERE MR.id_scan = SD.id_scan AND MR.status_request = 'Y' GROUP BY MR.temp_mrin ORDER BY MR.date_posting ASC, MR.temp_mrin ASC";
