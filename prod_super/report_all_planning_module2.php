...
/**
 * prod_super/report_all_planning_module2.php
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_production_super.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a><a href="#" class="#">Production</a> <a href="#" class="current">Report Planned Order</a> </div>
...
<?php
		 ...
		 //3. Factory 
		  ...
		  //7. Material No.
                if ($material_no == "NULL"){ 
                    ...
                    $wheresql_07 = " AND MR.material_no = ?"; $wheresql_07_args = [$material_no]; }  			 			
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail AS MR, work_center_detail AS SR WHERE MR.work_center = SR.id_work AND (MR.status_pps != ? AND MR.status_pps != ?)".$where_sql; $query8_args = [$rst_sta4["status_desc"], $rst_sta16["status_desc"], ...$where_args];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_posting,'%H:%i:%s') as T2 FROM pps_detail AS MR, work_center_detail AS SR WHERE MR.work_center = SR.id_work AND (MR.status_pps != ? AND MR.status_pps != ?)".$where_sql. " GROUP BY MR.plan_no ORDER BY MR.plan_no ASC"; $query_args = [$rst_sta4["status_desc"], $rst_sta16["status_desc"], ...$where_args];
...
<tr>
  ...
  $row4_p = mysqli_fetch_array($result4_p);
        ...
        //---------get material header---------
	    $query_mat_h = "SELECT * FROM table_material WHERE material_no = ?"; $query_mat_h_args = [$row['material_no']];
		$result_mat_h = db_query_bind($dbc, $query_mat_h, $query_mat_h_args);
		$data_mat_h = mysqli_fetch_array($result_mat_h);	 
		
		//---------get sloc ---------
	    $query_mat_h2 = "SELECT * FROM mat_master_detail WHERE material = ?"; $query_mat_h2_args = [$row['material_no']];
		$result_mat_h2 = db_query_bind($dbc, $query_mat_h2, $query_mat_h2_args);
...
<?php		
   ...
   $query_display = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction AS MR WHERE MR.pps_id = ? ORDER BY plan_no ASC"; $query_display_args = [$row["id"]];
   ...
   while ($row_display = mysqli_fetch_array($result_display))
   {
       ...
       //---------get material header---------
		...
		$query_display3 = "SELECT *, DATE_FORMAT(N.date_plan,'%d-%m-%Y') as B, DATE_FORMAT(N.date_qc_posting,'%d-%m-%Y') as B2, DATE_FORMAT(N.date_create,'%d-%m-%Y') as B3 FROM qqc_detail_transaction AS N WHERE N.bflush_no = ? ORDER BY plan_no ASC"; $query_display3_args = [$row_display["bflush_no"]];
		$result_display3 = db_query_bind($dbc, $query_display3, $query_display3_args);   //run the query.  
		
  		$query_display2 = "SELECT *, DATE_FORMAT(M.date_plan,'%d-%m-%Y') as J, DATE_FORMAT(M.date_qc_posting,'%d-%m-%Y') as J2, DATE_FORMAT(M.date_create,'%d-%m-%Y') as J3 FROM qqc_transaction AS M WHERE M.bflush_no = ? ORDER BY plan_no ASC"; $query_display2_args = [$row_display["bflush_no"]];
