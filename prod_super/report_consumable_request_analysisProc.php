...
/**
 * prod_super/report_consumable_request_analysisProc.php
 ...
 while ($row2 = mysqli_fetch_array($rs))
   {
	...
	$query5 = "SELECT *,DATE_FORMAT(date_create,'%d-%m-%Y') AS T FROM post_consumable_detail_header WHERE mrin_no = ? AND material_no = ? AND mvt_type = 201"; $query5_args = [$row2["temp_mrin"], $row2["material_no"]];
    $result5 = db_query_bind($dbc, $query5, $query5_args);
	$row5 = mysqli_fetch_array($result5);
	
	$query7 = "SELECT *  FROM consumable_request_close AS MC, reason_req_close AS MRC WHERE MC.reason_close = MRC.id_close AND MC.temp_mrin = ? AND MC.material_no = ?"; $query7_args = [$row2["temp_mrin"], $row2["material_no"]];
