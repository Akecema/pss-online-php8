 ...
 while ($row2 = mysqli_fetch_array($rs))
   {
	...
	$query6 = "SELECT *,DATE_FORMAT(PD.date_create,'%d-%m-%Y') AS T FROM post_detail_header AS PD, material_request AS MR WHERE PD.mrin_no = MR.temp_mrin AND PD.material_no = MR.bom_component AND PD.mrin_no = '".db_esc($dbc, $row2["temp_mrin"])."' AND PD.material_no = '".db_esc($dbc, $row2["bom_component"])."' AND PD.mvt_type = 311";
    $result6 = mysqli_query($dbc, $query6);
	$row6 = mysqli_fetch_array($result6);
	
	$query7 = "SELECT * FROM material_request_close AS MC, reason_req_close AS MRC WHERE MC.reason_close = MRC.id_close AND MC.id_req = '".db_esc($dbc, $row2["id_req"])."' AND MC.temp_mrin = '".db_esc($dbc, $row2["temp_mrin"])."' AND MC.bom_component = '".db_esc($dbc, $row2["bom_component"])."'";
...
//------------------------------------------------------------------------------------------------------------------	
    ...
    $query_tp = "SELECT *,SUM(rquantity) as TOT FROM post_detail_header WHERE mrin_no = '".db_esc($dbc, $row2["temp_mrin"])."' AND mvt_type = 311 AND material_no = '".db_esc($dbc, $row4_p["bill_component"])."'";
