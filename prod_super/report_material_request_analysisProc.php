...
/**
 * prod_super/report_material_request_analysisProc.php
 ...
 while ($row2 = mysqli_fetch_array($rs))
   {
	...
	$query6 = "SELECT *,DATE_FORMAT(PD.date_create,'%d-%m-%Y') AS T FROM post_detail_header AS PD, material_request AS MR WHERE PD.mrin_no = MR.temp_mrin AND PD.material_no = MR.bom_component AND PD.mrin_no = ? AND PD.material_no = ? AND PD.mvt_type = 311"; $query6_args = [$row2["temp_mrin"], $row2["bom_component"]];
    $result6 = db_query_bind($dbc, $query6, $query6_args);
	$row6 = mysqli_fetch_array($result6);
	
	$query7 = "SELECT * FROM material_request_close AS MC, reason_req_close AS MRC WHERE MC.reason_close = MRC.id_close AND MC.id_req = ? AND MC.temp_mrin = ? AND MC.bom_component = ?"; $query7_args = [$row2["id_req"], $row2["temp_mrin"], $row2["bom_component"]];
...
//------------------------------------------------------------------------------------------------------------------	
    ...
    $query_tp = "SELECT *,SUM(rquantity) as TOT FROM post_detail_header WHERE mrin_no = ? AND mvt_type = 311 AND material_no = ?"; $query_tp_args = [$row2["temp_mrin"], $row4_p["bill_component"]];
