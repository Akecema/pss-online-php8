...
<?php
   ...
   $query8 = "SELECT COUNT(*) FROM  reject_detail_disposal AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status_part = 'PR' AND MR.qty_NG != '' AND MR.doc_disposal_no != '' ".$where_sql; $query8_args = [...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_reject,'%d-%m-%Y  %H:%i:%s') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2 FROM reject_detail_disposal AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status_part = 'PR' AND MR.qty_NG != '' AND MR.doc_disposal_no != '' ".$where_sql." ORDER BY MR.plan_no ASC"; $query_args = [...$where_args];
