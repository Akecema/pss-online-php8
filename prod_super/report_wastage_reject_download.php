...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_wastage,'%d-%m-%Y %H:%i:%s') as R3 FROM reject_detail_disposal AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status_part = 'WS' AND MR.qty_wastage != '' AND MR.doc_disposal_no != ''".$where_sql." ORDER BY MR.plan_no ASC";
