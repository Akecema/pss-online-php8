...
/**
 * qqc/cancel_qc_rework_output_QC_ok.php
 * Part of: QQC module (Quality)
 * Filename suggests: cancel qc rework output QC ok
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, qqc_transaction, qqc_detail_transaction, table_material, run_count_no, ftp_goodtran_detail, qqc_transaction_cancel2, Cancel.
...
$message = NULL; // create an empty new variable.
  ...
  $query_info = "SELECT * FROM qqc_transaction WHERE id_qqc = ?"; $query_info_args = [$cancel[$i]]; 
  ...
  $query_upd_detail2 = "UPDATE qqc_detail_transaction SET status_QC = ? WHERE id_qqc = ? AND qqc_doc_no = ?"; $query_upd_detail2_args = [$rst_sta8["status_desc"], $data_info["id_tran"], $data_info["qqc_doc_no"]];
  $result_upd_detail2 = db_query_bind($dbc, $query_upd_detail2, $query_upd_detail2_args) or die(db_fail($dbc));
  
  		$query_ftp = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as J, DATE_FORMAT(date_cancel,'%Y-%m-%d') as J2, DATE_FORMAT(date_cancel,'%H:%i:%s') as J3 FROM qqc_transaction WHERE id_qqc = ? AND qqc_doc_no = ?"; $query_ftp_args = [$cancel[$i], $data_info["qqc_doc_no"]];
        ...
        $result_q2 = db_query_bind($dbc, $query_q2, $query_q2_args);
	...
	if ($result_id) 
{
  ...
  $query_upd_detail = "UPDATE qqc_transaction SET status_QC = ?, qqc_no_ref = ?, user_cancel = ?, date_cancel = NOW() WHERE id_qqc = ?"; $query_upd_detail_args = [$rst_sta4["status_desc"], $ref, $username, $cancel[$i]];
	...
	if ($result_id_2) 
{
 ...
 //echo $ref_2;
	  ...
	  $query_upd_detail = "UPDATE qqc_transaction SET status_QC = ?, qqc_no_ref = ?, user_cancel = ?, date_cancel = NOW() WHERE id_qqc = ?"; $query_upd_detail_args = [$rst_sta4["status_desc"], $ref_2, $username, $cancel[$i]];
  $result_upd_detail = db_query_bind($dbc, $query_upd_detail, $query_upd_detail_args) or die(db_fail($dbc));
...
//----------update table ftp_goodtran_detail------------
    ...
    $query_ftp_info = "INSERT INTO ftp_goodtran_detail(id, file_name, qqc_no, bflush_no, id_tran, plan_no, material_no, material_desc, qty_ftp, uom, status_ftp, posting_date, posting_time, user_create, date_create) VALUES('',?,?,".$data_ftp["bflush_no"].",?,?,?,?,?,?,'Y',?,?,?,NOW())"; $query_ftp_info_args = [$filen, $data_ftp["qqc_no_ref"], $data_ftp["id_tran"], $data_ftp["plan_no"], $data_ftp["material_no"], $data_ftp["material_desc"], $data_ftp["qty_qc_NG"], $ans3['BUn'], $data_ftp["date_cancel"], $data_ftp["J3"], $username]; 
     $rst_ftp_info = db_query_bind($dbc, $query_ftp_info, $query_ftp_info_args);
...
$query_display = "SELECT *, DATE_FORMAT(date_qc_posting,'%d-%m-%Y') as R2 FROM qqc_transaction WHERE id_tran = ? AND status_QC != ?"; $query_display_args = [$uid, $rst_sta4["status_desc"]];
$result_display = db_query_bind($dbc, $query_display, $query_display_args);   //run the query.
   ...
   while($row2 = mysqli_fetch_array($result_display))
   {
	...
	//query material 
	 $query_mat =  "SELECT * FROM table_material WHERE material_no = ?"; $query_mat_args = [$row2["material_no"]];		
