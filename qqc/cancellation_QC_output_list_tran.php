...
$sta_res11 = mysql_query($sta11);
$rst_sta11 = mysql_fetch_array($sta_res11);	
...
echo "window.location='cancellation_QC_output_list_tran2.php?date1=$dateF&&date2=$dateT&&factory=$factory&&work_center=$work_center&&plan_no=$plan_no&&shift_ops=$shift_ops&&model_code=$model_code'";
   ...
   $query8 = "SELECT COUNT(*) FROM qqc_detail_transaction AS MR, pps_detail_transaction AS PD WHERE MR.bflush_no = PD.bflush_no AND (MR.status_QC = '".$rst_sta8["status_desc"]."' OR MR.status_QC = '".$rst_sta11["status_desc"]."') AND PD.status_pps != '".$rst_sta14["status_desc"]."' AND MR.status_QC != '".$rst_sta4["status_desc"]."' ";
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_qc_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM qqc_detail_transaction AS MR, pps_detail_transaction AS PD WHERE MR.bflush_no = PD.bflush_no AND (MR.status_QC = '".$rst_sta8["status_desc"]."' OR MR.status_QC = '".$rst_sta11["status_desc"]."') AND MR.status_QC != '".$rst_sta4["status_desc"]."' AND PD.status_pps != '".$rst_sta14["status_desc"]."' ORDER BY MR.plan_no ASC";
   ...
   $qty_total_NG2 = 0.000;
	...
	//}	
	  ...
	  /* $query_qqc = "SELECT * FROM qqc_transaction WHERE bflush_no = '".$row["bflush_no"]."' AND plan_no = '".$row["plan_no"]."'";
	   ...
	   while($data_qqc = mysql_fetch_array($result_qqc))
	   {
