...
/**
 * qqc/cancellation_QC_output_list_tran.php
 * Part of: QQC module (Quality)
 * Filename suggests: cancellation QC output list tran
 *
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_qqc.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a><a href="#" class="#">QA/QC</a> <a href="#" class="current">Cancellation Rework/Prod. Output</a></div>
  <h1>Cancellation Rework/Production Output</h1>
</div>
...
<?php
        if(isset($_POST["Submit2"]))
        {
            ...
            echo "<script>";
echo "window.location='cancellation_QC_output_list_tran2.php?date1=$dateF&&date2=$dateT&&factory=$factory&&work_center=$work_center&&plan_no=$plan_no&&shift_ops=$shift_ops&&model_code=$model_code'";
            echo "</script>";
   ...
   $query8 = "SELECT COUNT(*) FROM qqc_detail_transaction AS MR, pps_detail_transaction AS PD WHERE MR.bflush_no = PD.bflush_no AND (MR.status_QC = ? OR MR.status_QC = ?) AND PD.status_pps != ? AND MR.status_QC != ? "; $query8_args = [$rst_sta8["status_desc"], $rst_sta11["status_desc"], $rst_sta14["status_desc"], $rst_sta4["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_qc_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM qqc_detail_transaction AS MR, pps_detail_transaction AS PD WHERE MR.bflush_no = PD.bflush_no AND (MR.status_QC = ? OR MR.status_QC = ?) AND MR.status_QC != ? AND PD.status_pps != ? ORDER BY MR.plan_no ASC"; $query_args = [$rst_sta8["status_desc"], $rst_sta11["status_desc"], $rst_sta4["status_desc"], $rst_sta14["status_desc"]];
	...
	//  echo h($row["qty_balance"]);
	  ...
	  */
      ...
      ?>
                ...
                <a value="Cancellation" href="cancel_qc_rework_output.php?uid=<?php echo h($row["id_tran"]); ?>&&TB_iframe=true&&height=400&&width=800" class="thickbox" target="_self"><img src="../img/delete.png" width="16" height="16" alt="Cancellation">Cancellation</a>
                ...
                <a value="Cancellation" href="cancel_qc_rework_output_QC_ok.php?uid=<?php echo h($row["id_tran"]); ?>&&TB_iframe=true&&height=400&&width=800" class="thickbox" target="_self"><img src="../img/delete.png" width="16" height="16" alt="Cancellation">Cancellation</a>                
                 <?php  }else{  ?>  <a value="Cancellation" href="cancel_qc_product_output.php?uid=<?php echo h($row["id_tran"]); ?>&&TB_iframe=true&&height=400&&width=800" class="thickbox" target="_self"><img src="../img/delete.png" width="16" height="16" alt="Cancellation">Cancellation Product Output</a> <?php } ?>
                </td>
                </tr>
