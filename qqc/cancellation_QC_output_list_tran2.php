...
<?php
   ...
   $query8 = "SELECT COUNT(*) FROM qqc_detail_transaction AS MR, work_center_detail AS SR, pps_detail_transaction AS PD WHERE SR.id_work = MR.work_center AND MR.bflush_no = PD.bflush_no AND (MR.status_QC = '".$rst_sta8["status_desc"]."' OR MR.status_QC = '".$rst_sta11["status_desc"]."')  AND PD.status_pps != '".$rst_sta14["status_desc"]."' AND MR.status_QC != '".$rst_sta4["status_desc"]."' AND MR.qty_balance != '0'".$where_sql;
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_qc_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM qqc_detail_transaction AS MR, work_center_detail AS SR, pps_detail_transaction AS PD WHERE SR.id_work = MR.work_center AND MR.bflush_no = PD.bflush_no AND (MR.status_QC = '".$rst_sta8["status_desc"]."' OR MR.status_QC = '".$rst_sta11["status_desc"]."')  AND PD.status_pps != '".$rst_sta14["status_desc"]."' AND MR.status_QC != '".$rst_sta4["status_desc"]."' AND MR.qty_balance != '0'".$where_sql." ORDER BY MR.plan_no ASC";
