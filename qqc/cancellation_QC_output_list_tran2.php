...
/**
 * qqc/cancellation_QC_output_list_tran2.php
 * Part of: QQC module (Quality)
 * Filename suggests: cancellation QC output list tran2
 *
...
<?php
   ...
   $query8 = "SELECT COUNT(*) FROM qqc_detail_transaction AS MR, work_center_detail AS SR, pps_detail_transaction AS PD WHERE SR.id_work = MR.work_center AND MR.bflush_no = PD.bflush_no AND (MR.status_QC = ? OR MR.status_QC = ?)  AND PD.status_pps != ? AND MR.status_QC != ? AND MR.qty_balance != '0'".$where_sql; $query8_args = [$rst_sta8["status_desc"], $rst_sta11["status_desc"], $rst_sta14["status_desc"], $rst_sta4["status_desc"], ...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_qc_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM qqc_detail_transaction AS MR, work_center_detail AS SR, pps_detail_transaction AS PD WHERE SR.id_work = MR.work_center AND MR.bflush_no = PD.bflush_no AND (MR.status_QC = ? OR MR.status_QC = ?)  AND PD.status_pps != ? AND MR.status_QC != ? AND MR.qty_balance != '0'".$where_sql." ORDER BY MR.plan_no ASC"; $query_args = [$rst_sta8["status_desc"], $rst_sta11["status_desc"], $rst_sta14["status_desc"], $rst_sta4["status_desc"], ...$where_args];
