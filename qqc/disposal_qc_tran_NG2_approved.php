...
/**
 * qqc/disposal_qc_tran_NG2_approved.php
 * Part of: QQC module (Quality)
 * Filename suggests: disposal qc tran NG2 approved
 *
    ...
    <!--  <div class="span12">-->
      <div class="widget-box">
          ...
          <ul class="nav nav-tabs" role="tablist" id="templatemo-tabs">
           ...
           <li class="active"><a role="tab" href="disposal_qc_tran_NG_approved.php">Approved Disposal</a></li>
             </ul>
          </div>
          </div>
          ...
          <form action="<?php echo h($_SERVER['PHP_SELF']); ?>" method="get" name="frmSearch" id="frmSearch">
            ...
            <tr>
              ...
              <td><select name="doc_disposal_no" id="doc_disposal_no">
                  ...
                  <?php
	             $query9 = "SELECT * FROM reject_detail_disposal WHERE (status_part = 'QC' OR status_part = 'WQ') AND doc_disposal_no != '' AND status_disposal != ? AND status_disposal != ? GROUP BY doc_disposal_no"; $query9_args = [$rst_sta["status_desc"], $rst_sta16["status_desc"]];
...
<?php
   ...
   $query8 = "SELECT * FROM reject_detail_disposal AS MR WHERE (MR.status_part = 'QC' OR MR.status_part = 'WQ') AND MR.doc_disposal_no != '' AND MR.status_disposal = ?".$where_sql." GROUP BY doc_disposal_no"; $query8_args = [$rst_sta3["status_desc"], ...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2 FROM reject_detail_disposal AS MR WHERE (MR.status_part = 'QC' OR MR.status_part = 'WQ') AND MR.doc_disposal_no != '' AND MR.status_disposal = ?".$where_sql." GROUP BY MR.doc_disposal_no ORDER BY MR.plan_no ASC $pages->limit"; $query_args = [$rst_sta3["status_desc"], ...$where_args];
...
<tr>
    <td width="1%">&nbsp;</td> 
      <td>&nbsp;</td>
      <td width="7%"><a href="disposal_qc_tran_approved_download_selected.php?date1=<?php echo h($dateF); ?>&&date2=<?php echo h($dateT); ?>&&doc_disposal_no=<?php echo h($doc_disposal_no); ?>" ><img src="../img/dload_excel.jpg" width="48" height="48" title="Download" /></a></td>
...
<?php echo '<div align="center">There are currently  '.h($num_rows).' record(s).</div>';  ?>      
      ...
      ?>
			   ...
			   { 
			       if($row["status_part"] == "QC")
				   {
					...
					?>
				 <a value="Details" href="view_disposal_qc_approve_tran_NG.php?doc_disposal=<?php echo h($row["doc_disposal_no"]); ?>&&TB_iframe=true&height=400&width=1100" class="thickbox" target="_self"><img src="../img/icon_view.jpg" width="16" height="16" alt="View">View</a>	<?php
				 
				   }elseif($row["status_part"] == "WQ")
					{   
					 ?> <a value="Details" href="view_disposal_qc_approve_tran_NG_wastage.php?doc_disposal=<?php echo h($row["doc_disposal_no"]); ?>&&TB_iframe=true&height=400&width=1100" class="thickbox" target="_self"><img src="../img/icon_view.jpg" width="16" height="16" alt="View">View</a> 
					<?php }
