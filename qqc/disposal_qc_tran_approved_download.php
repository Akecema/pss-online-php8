...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_reject,'%d-%m-%Y') as R3, DATE_FORMAT(date_wastage,'%d-%m-%Y') as R4, DATE_FORMAT(date_disposal,'%d-%m-%Y') as R5, DATE_FORMAT(date_approve,'%d-%m-%Y') as R6, DATE_FORMAT(date_approve2,'%d-%m-%Y') as R7 FROM reject_detail_disposal WHERE (status_part = 'QC' OR status_part = 'WQ') AND doc_disposal_no != '' AND status_disposal = '".db_esc($dbc, $rst_sta3["status_desc"])."' ORDER BY doc_disposal_no ASC ";
