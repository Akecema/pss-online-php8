...
//------------update remarks reject detail disposal-------------------
 ...
 $query_upd3 = "UPDATE reject_detail_disposal SET remarks = ?, user_update = ?, date_update = NOW() WHERE id_disposal = ?"; $query_upd3_args = [$string[$i], $username, $cancel[$i]];
 $result_upd3 = db_query_bind($dbc, $query_upd3, $query_upd3_args); 
   ...
   //---------------------get data table reject_detail_disposal-------
	  ...
	  $query_wastage_update = "UPDATE wastage_transaction SET user_update = ?, date_update = NOW(), remarks = ? WHERE id_wastage_tran = ?"; $query_wastage_update_args = [$username, $row_all["remarks"], $row_all["uid"]];
