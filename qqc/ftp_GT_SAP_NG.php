...
$data = "";
   ...
   $query_qc = "SELECT * FROM qqc_detail_transaction WHERE id_qqc = '".db_esc($dbc, $uid)."'";
 ...
 //----------create text file sent ftp to SAP[comp code][5][running no]--------------
   $query_ftp = "SELECT *, DATE_FORMAT(date_qc_posting,'%Y-%m-%d') AS P, DATE_FORMAT(date_create,'%Y-%m-%d') AS P2, DATE_FORMAT(date_create,'%H:%i:%s') AS P3 FROM qqc_transaction WHERE id_tran = '".db_esc($dbc, $data_qc["id_tran"])."' AND qqc_no = '".db_esc($dbc, $buid)."'";
   $result_ftp = mysqli_query($dbc, $query_ftp);
   $data_ftp = mysqli_fetch_array($result_ftp);
	
	$query_type = "SELECT * FROM type_reject_detail WHERE id_type = '".db_esc($dbc, $data_ftp["type_qc_reject"])."' ORDER BY id_type ASC";
    $result_type = mysqli_query($dbc, $query_type);
    $row_type = mysqli_fetch_array($result_type); 
	
	$query_reason = "SELECT * FROM reason_ng_reject WHERE id_reject = '".db_esc($dbc, $data_ftp["reason_qc_reject"])."' ORDER BY id_reject ASC";
...
file_put_contents($file,$data);
  ...
  //----------update table ftp_goodtran_detail------------
    ...
    $query_ftp_info = "INSERT INTO ftp_goodtran_detail(id,file_name,qqc_no,bflush_no,id_tran,plan_no,material_no,material_desc,qty_ftp,uom,status_ftp,posting_date,posting_time,user_create,date_create) VALUES('','".db_esc($dbc, $filen)."','".db_esc($dbc, $data_ftp["qqc_no"])."',".$data_ftp["bflush_no"].",'".db_esc($dbc, $data_ftp["id_tran"])."','".db_esc($dbc, $data_ftp["plan_no"])."','".db_esc($dbc, $data_ftp["material_no"])."','".db_esc($dbc, $data_ftp["material_desc"])."','".db_esc($dbc, $data_ftp["qty_qc_NG"])."','".db_esc($dbc, $ans3["BUn"])."','Y','".db_esc($dbc, $data_ftp["P"])."','".db_esc($dbc, $data_ftp["time_qc_posting"])."','".db_esc($dbc, $username)."',NOW())"; 
