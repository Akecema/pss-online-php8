...
/**
 * qqc/list_ftp_pending4_sap.php
 * Part of: QQC module (Quality)
 * Filename suggests: list ftp pending4 sap
 *
 * Behavior: requires an active login session ($_SESSION['username']).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, ftp_goodtran_detail, table_material, ftp_qc_received_detail.
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_qqc.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a><a href="#" class="#">QC FTP Monitor</a> <a href="#" class="current">QC FTP Monitor</a></div>
  <h1>QC FTP Monitor</h1>
</div>
         ...
         <div class="widget-box">
          <div class="widget-title">
             <ul class="nav nav-tabs" role="tablist" id="templatemo-tabs">
              <li><a role="tab" href="list_ftp_pending2_sap.php">Rework OK</a></li>
              <li><a  role="tab" href="list_ftp_pending2_NG_sap.php">Rework NG</a></li>
              <li><a role="tab" href="list_ftp_pending3_sap.php">Disposal</a></li>
              <li class="active"><a role="tab" href="list_ftp_pending4_sap.php">Production Output</a></li>
              <!--<li><a role="tab" href="list_ftp_pending5_sap.php">Good Issue</a></li>-->
             </ul>
          </div>
          </div>
...
$path = "../../PSS_Online/FromPortal2/GT/"; 

// Open the folder
 $dir_handle = @opendir($root . $path) or die("Unable to open $path");

$dir = "$root/FromPortal2/GT/";

$folder = '../../PSS_Online/FromPortal2/GT/';
$filetype = '*.*';    
...
$dir = "$root/PSS_Online/FromPortal2/GT/";
...
if(is_dir($dir)){
  ...
  <table class="table">
   ...
   <?php
	...
	$query = "SELECT *,DATE_FORMAT(posting_date,'%d-%m-%Y') AS R2 FROM ftp_goodtran_detail WHERE status_ftp = 'Y' AND file_name = ?"; $query_args = [$filename2];
	...
	//---check filename from table  ftp_qc_received_detail--------
		
	$query_qc = "SELECT *,DATE_FORMAT(posting_date,'%d-%m-%Y') AS S2 FROM  ftp_qc_received_detail WHERE status_ftp = 'Y' AND file_name = ?"; $query_qc_args = [$filename2];
	$rs_qc = db_query_bind($dbc, $query_qc, $query_qc_args);   //run the query.
	$row_rs_qc = mysqli_fetch_array($rs_qc);   //how many material are there?
	
	//---check material type in table material--------
		
	$query_mat_type_qc = "SELECT * FROM table_material WHERE material_no = ?"; $query_mat_type_qc_args = [$row_rs_qc["material_no"]];
	$rs_mat_type_qc = db_query_bind($dbc, $query_mat_type_qc, $query_mat_type_qc_args);   //run the query.
	$row_mat_type_qc = mysqli_fetch_array($rs_mat_type_qc);   //how many material are there?	
