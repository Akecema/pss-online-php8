...
/**
 * qqc/production_output_list_tran.php
 * Part of: QQC module (Quality)
 * Filename suggests: production output list tran
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, factory_detail, pps_detail, model_detail, pps_detail_transaction, qqc_detail_transaction.
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_qqc.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a><a href="#" class="#">QA/QC</a> <a href="#" class="current">Production Output List</a></div>
  <h1>Production Output List</h1>
</div>
      ...
      <form action="<?php echo h($_SERVER['PHP_SELF']); ?>" method="post" name="frmSearch" id="frmSearch">
	       ...
	       $query3 = "SELECT * FROM factory_detail GROUP BY factory_desc2 ORDER BY id_fac ASC";
              ...
              <td><select name="plan_no" id="plan_no" class="span11">
                  ...
                  <?php
	             $query9 = "SELECT * FROM pps_detail AS PP, mat_master_header AS MM WHERE MM.material_no = PP.material_no AND PP.status_pps = ? AND MM.material_type = 'Z310' ORDER BY PP.plan_no ASC"; $query9_args = [$rst_sta7["status_desc"]];
...
<?php
        if(isset($_POST["Submit2"]))
        {
            ...
            echo "<script>";
echo "window.location='production_output_list_tran2.php?date1=$dateF&&date2=$dateT&&factory=$factory&&work_center=$work_center&&plan_no=$plan_no&&shift_ops=$shift_ops&&model_code=$model_code'";
            echo "</script>";
 ...
 // echo $end_date_check;
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail_transaction WHERE status_pps = ? AND qty_actual != '' AND material_type != 'Z110' AND (date_plan >= '$start_date_check' AND date_plan <= '$end_date_check')"; $query8_args = [$rst_sta7["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction WHERE status_pps = ? AND qty_actual != '' AND material_type != 'Z110' AND (date_plan >= '$start_date_check' AND date_plan <= '$end_date_check') ORDER BY date_posting DESC"; $query_args = [$rst_sta7["status_desc"]];
   ...
   $sta_out2 = substr($row["bflush_no"],2,3);
	...
	}elseif($sta_out2 == "221")
	{
      ...
      ?>
                ...
                <!--<td width="80"><?php echo h($row["R3"]); ?></td>-->
			  ...
			  //-------info qqc detail --------- 
	   $query_qqc = "SELECT * FROM qqc_detail_transaction WHERE id_tran = ? AND bflush_no = ? AND plan_no = ? "; $query_qqc_args = [$row["id"], $row["bflush_no"], $row["plan_no"]];
	   ...
	   $data_qqc = mysqli_fetch_array($result_qqc);
	     ...
	     if($data_qqc == 0) { //If it ran ok
		   ...
		   ?> 
                ...
                <a value="Quality Control" href="qc_production_output_list.php?uid=<?php echo h($row["id"]); ?>&&TB_iframe=true&height=400&width=650" class="thickbox" target="_self"><img src="../img/page_white_magnify.png" width="16" height="16" alt="Quality Control"><?php echo h($data_qqc["qqc_doc_no"]);   ?> QC</a>
