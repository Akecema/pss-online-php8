...
/**
 * qqc/production_output_list_tran2.php
 * Part of: QQC module (Quality)
 * Filename suggests: production output list tran2
 *
 * Behavior: requires an active login session ($_SESSION['username']); reads parameters from the query string ($_GET).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, factory_detail, work_center_detail, pps_detail, model_detail, pps_detail_transaction, qqc_detail_transaction.
...
<?php
		...
		// 2. DateT
                if($dateT == "0000-00-00") {
                     $wheresql_02 = ""; $wheresql_02_args = []; }
                      ...
                      $wheresql_02 = " AND (MR.date_plan <= ?)"; $wheresql_02_args = [$dateT]; }	
		  ...
		  //6. Shift
		   ...
		   //7. Model Code
                if ($model_code == "NULL"){ 
                    ...
                    $wheresql_07 = " AND MR.model_code = ?"; $wheresql_07_args = [$model_code]; }   	
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail_transaction AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status_pps = ? AND qty_actual != '' AND material_type != 'Z110' ".$where_sql; $query8_args = [$rst_sta7["status_desc"], ...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status_pps = ? AND qty_actual != '' AND material_type != 'Z110' ".$where_sql." ORDER BY MR.plan_no ASC"; $query_args = [$rst_sta7["status_desc"], ...$where_args];
   ...
   $sta_out2 = substr($row["bflush_no"],2,3);
	...
	}elseif($sta_out2 == "221")
	{
      ...
      ?>
			  ...
			  //-------info qqc detail --------- 
	   $query_qqc = "SELECT * FROM qqc_detail_transaction WHERE id_tran = ? AND bflush_no = ?"; $query_qqc_args = [$row["id"], $row["bflush_no"]];
	   $result_qqc = db_query_bind($dbc, $query_qqc, $query_qqc_args) or die(db_fail($dbc));
