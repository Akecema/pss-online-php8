...
/**
 * qqc/production_output_list_tran_close.php
 * Part of: QQC module (Quality)
 * Filename suggests: production output list tran close
 *
...
echo "window.location='production_output_list_tran2.php?date1=$dateF&&date2=$dateT&&factory=$factory&&work_center=$work_center&&plan_no=$plan_no&&shift_ops=$shift_ops&&model_code=$model_code'";
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail_transaction WHERE status_pps = ? AND qty_actual != ''"; $query8_args = [$rst_sta7["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_create,'%d-%m-%Y') as R3 FROM pps_detail_transaction WHERE status_pps = ? AND qty_actual != '' order by plan_no ASC"; $query_args = [$rst_sta7["status_desc"]];
