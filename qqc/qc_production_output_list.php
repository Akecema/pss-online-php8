...
<div class="widget-box">
 ...
 $status_new = "";
  ...
  //--------- pps detail ------------
	   ...
	   $query_pps = "SELECT * FROM pps_detail_transaction WHERE id = ?"; $query_pps_args = [$uid];
   ...
   //shift	
		if($data_pps["shift_pps1"] != "")
	{
	...
	}elseif($data_pps["shift_pps2"] != "")
	 {
	   ...
	   $query_qqc = "SELECT * FROM qqc_detail_transaction WHERE id_tran = ? AND bflush_no = ?"; $query_qqc_args = [$uid, $data_pps["bflush_no"]];
...
$result_data2 = db_query_bind($dbc, $query_data2, $query_data2_args) or die(db_fail($dbc));
   ...
   $query_rcv_ftp = "SELECT *, DATE_FORMAT(date_qc_posting,'%Y-%m-%d') AS R, DATE_FORMAT(date_create,'%Y-%m-%d') AS R2, DATE_FORMAT(date_create,'%H:%i:%s') AS R3, DATE_FORMAT(date_plan,'%Y-%m-%d') AS J1 FROM qqc_detail_transaction WHERE id_tran = ? AND qqc_doc_no = ?"; $query_rcv_ftp_args = [$uid, $ref];
...
file_put_contents($file_rcv,$data_rcv);
   ...
   //----------update table ftp_qc_received_detail------------
    ...
    $query_rcv_ftp_info = "INSERT INTO ftp_qc_received_detail(id, file_name, qqc_doc_no, bflush_no, id_tran, plan_no, material_no, material_desc, qty_ftp, uom, status_ftp, posting_date, posting_time, user_create, date_create) VALUES('',?,?,".$data_rcv_ftp["bflush_no"].",?,?,?,?,?,'PCS','Y',?,?,?,NOW())"; $query_rcv_ftp_info_args = [$filen_rcv, $data_rcv_ftp["qqc_doc_no"], $data_rcv_ftp["id_tran"], $data_rcv_ftp["plan_no"], $data_rcv_ftp["material_no"], $data_rcv_ftp["material_desc"], $data_rcv_ftp["qty_qc"], $data_rcv_ftp["R"], $data_rcv_ftp["time_qc_posting"], $username]; 
     $rst_rcv_ftp_info = db_query_bind($dbc, $query_rcv_ftp_info, $query_rcv_ftp_info_args);
	 
	   // ---update status 

		$query_rcv_ftp2 = "UPDATE qqc_detail_transaction SET status_ftp_fgtran = 'Y' WHERE bflush_no = ?"; $query_rcv_ftp2_args = [$data_rcv_ftp["bflush_no"]];
   ...
   else
   {
    ...
    $number_2 = sprintf('%07d', $number_2);  
	  ...
	  $ref_2 = ($dht_OK_2.($number_2));
		...
		$query_data3 = "INSERT INTO qqc_transaction (id_qqc, id_tran, qqc_doc_no, qqc_no, bflush_no, plan_no, material_no, material_desc, material_type, qty_plan, qty_actual, qty_balance, qty_NG, qty_qc, qty_qc_ok, qty_qc_NG, status_QC, comp_code, work_center, shift_day, date_plan, user_create, date_create, user_update, date_update, user_qc_posting, date_qc_posting, time_qc_posting, ploc_qc, ploc, delivery_loc, type_qc_reject, reason_qc_reject, user_qc_reject, date_qc_reject, time_qc_reject, status_ftp_fgtran, status, qqc_no_ref, user_cancel, date_cancel) VALUES('',?,?,?,?,?,?, ?,?,?,?,?,'',?,?,'',?,?, ?, ?,?,?,NOW(),'','',?,NOW(),NOW(),?,?,'','','','','','','N','Y','','','')"; $query_data3_args = [$data_qqc["id_tran"], $data_qqc["qqc_doc_no"], $ref_2, $data_qqc["bflush_no"], $data_qqc["plan_no"], $data_qqc["material_no"], $data_qqc["material_desc"], $data_qqc["material_type"], $data_qqc["qty_plan"], $data_qqc["qty_actual"], $data_qqc["qty_balance"], $data_qqc["qty_qc"], $data_qqc["qty_qc"], $rst_sta8["status_desc"], $data_qqc["comp_code"], $data_qqc["work_center"], $data_qqc["shift_day"], $data_qqc["date_plan"], $username, $username, $ploc_qc, $data_qqc["ploc"]];
$result_data3 = db_query_bind($dbc, $query_data3, $query_data3_args) or die(db_fail($dbc));  

        
        //----------create text file sent ftp to SAP[comp code][5][running no]--------------
   $query_ftp = "SELECT *, DATE_FORMAT(date_qc_posting,'%Y-%m-%d') AS P, DATE_FORMAT(date_create,'%Y-%m-%d') AS P2, DATE_FORMAT(date_create,'%H:%i:%s') AS P3 FROM qqc_transaction WHERE id_tran = ? AND bflush_no = ?"; $query_ftp_args = [$data_qqc["id_tran"], $data_qqc["bflush_no"]];
   $result_ftp = db_query_bind($dbc, $query_ftp, $query_ftp_args);
   $data_ftp = mysqli_fetch_array($result_ftp);
   
   $query_q2 = "SELECT * FROM mat_master_header WHERE material_no = ?"; $query_q2_args = [$data_ftp["material_no"]];
...
file_put_contents($file,$data);
   ...
   //----------update table ftp_goodtran_detail------------
    ...
    $query_ftp_info = "INSERT INTO ftp_goodtran_detail(id, file_name, qqc_no, bflush_no, id_tran, plan_no, material_no, material_desc, qty_ftp, uom, status_ftp, posting_date, posting_time, user_create, date_create) VALUES('',?,?,".$data_ftp["bflush_no"].",?,?,?,?,?,'PCS','Y',?,?,?,NOW())"; $query_ftp_info_args = [$filen, $data_ftp["qqc_no"], $data_ftp["id_tran"], $data_ftp["plan_no"], $data_ftp["material_no"], $data_ftp["material_desc"], $data_ftp["qty_qc"], $data_ftp["P"], $data_ftp["time_qc_posting"], $username]; 
...
<p>&nbsp;</p> 
<form name="form1" action="qc_production_output_list.php?uid=<?php echo h($uid); ?>" method="post" class="form-horizontal">
<table width="98%" class="table table-bordered">
  ...
  <tr>
    <th width="25%">OK</th>
    <td width="75%"><input name="qty_qc" id="qty_qc" type="number" min="0" max="<?php echo h($total_qty_pending); ?>" value="<?php echo h($total_qty_pending); ?>"></td>
  </tr>
  <tr>
    <th>Pending</th>
    <td><input type="text" name="qty_balance" id="qty_balance" readonly value="<?php echo h($total_qty_pending); ?>" >
   <!-- <input type="text" name="qty_balance" id="qty_balance" readonly value="<?php echo h($qty_final); ?>" >--></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td> <input type="hidden" name="uid" value="<?php echo h($uid); ?>">
