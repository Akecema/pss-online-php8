...
$result_insert2 = db_query_bind($dbc, $query_insert2, $query_insert2_args) or die(db_fail($dbc));
   ...
   $query8 = "SELECT COUNT(*) FROM qqc_transaction WHERE status = 'N' AND status_QC != ?"; $query8_args = [$rst_sta4["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_qc_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_create,'%d-%m-%Y') as R3 FROM qqc_transaction WHERE status = 'N' AND status_QC != ? ORDER BY plan_no ASC"; $query_args = [$rst_sta4["status_desc"]];
