...
/**
 * qqc/reject_qc_tran_NG2.php
 * Part of: QQC module (Quality)
 * Filename suggests: reject qc tran NG2
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, factory_detail, work_center_detail, reject_detail_disposal, qqc_transaction, mat_master_header, pps_detail_transaction, type_reject_detail.
 ...
 if ($result_id_2) {
  ...
  if(isset($_POST["cancel"])) 
  {
	...
	$string = "";
	  ...
	  $query_info2 = "SELECT * FROM mat_master_header WHERE material_no = ?"; $query_info2_args = [$data_info["material_no"]];
	  $result_info2 = db_query_bind($dbc, $query_info2, $query_info2_args);
	  $data_info2 = mysqli_fetch_array($result_info2);
				
	 //get data table pps_detail_transaction
	  $query_info3 = "SELECT * FROM pps_detail_transaction WHERE material_no = ?"; $query_info3_args = [$data_info["material_no"]];
...
$result_insert2 = db_query_bind($dbc, $query_insert2, $query_insert2_args) or die(db_fail($dbc));
		...
		$query_update1 = "UPDATE qqc_transaction SET status = 'Y' WHERE id_qqc = ?"; $query_update1_args = [$cancel[$i]];
        $result_update1 = db_query_bind($dbc, $query_update1, $query_update1_args) or die(db_fail($dbc));
        ...
        $result_update2 = mysqli_query($dbc, $query_update2) or die(db_fail($dbc));*/
	       ...
	       echo "<script>";
		   echo "alert('Disposal Document No : " . h($data_info['qqc_no']) . "');";
		 ...
		 // 1. DateF
                if($dateF == "0000-00-00") {
                     $wheresql_01 = ""; $wheresql_01_args = []; }
                      ...
                      $wheresql_01 = " AND (MR.date_plan >= ?)"; $wheresql_01_args = [$dateF]; }
							 
		 //2. Factory 
                if ($factory == "NULL"){ 
                    $wheresql_02 = ""; $wheresql_02_args = []; }
                else {
                    $wheresql_02 = " AND SR.id_factory = ?"; $wheresql_02_args = [$row_convert["id_factory"]]; } 
					
          //3. Work Center
                if ($work_center == "NULL" ){
                    $wheresql_03 = ""; $wheresql_03_args = []; }
                else {
					$wheresql_03 = " AND MR.work_center = ?"; $wheresql_03_args = [$work_center]; }
   ...
   $query8 = "SELECT COUNT(*) FROM qqc_transaction AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status = 'N' AND MR.status_QC != ? ".$where_sql; $query8_args = [$rst_sta4["status_desc"], ...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_qc_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM qqc_transaction AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status = 'N' AND MR.status_QC != ? ".$where_sql." ORDER BY MR.plan_no ASC"; $query_args = [$rst_sta4["status_desc"], ...$where_args];
...
<tr>
   ...
   while ($row = mysqli_fetch_array($rs))
   {
	...
	$query_type = "SELECT * FROM type_reject_detail WHERE id_type = ? ORDER BY id_type ASC"; $query_type_args = [$row['type_qc_reject']];
    $result_type = db_query_bind($dbc, $query_type, $query_type_args);
    $row_type = mysqli_fetch_array($result_type); 
	
	$query_reason = "SELECT * FROM reason_ng_reject WHERE id_reject = ? ORDER BY id_reject ASC"; $query_reason_args = [$row['reason_qc_reject']];
    $result_reason = db_query_bind($dbc, $query_reason, $query_reason_args);
    $row_reason = mysqli_fetch_array($result_reason);
	
	$query_scan = "SELECT * FROM mat_master_header WHERE material_no = ?"; $query_scan_args = [$row['material_no']];
    ...
    $row_scan = mysqli_fetch_array($result_scan);
      ...
      ?>
                ...
                <tr class="gradeX">
                <td width="30"><div align="center"><input type="checkbox" name="cancel[]" value="<?php echo h($row["id_qqc"]); ?>" <?=was_checked($row["id_qqc"],$a) ?> /><input type="hidden" name="Check_ctr" value="yes" 
onClick="Check(document.myform.cancel)">  </div><?php echo $no; ?></td>
                ...
                <td width="60"><?php echo h($row["ploc_qc"]); ?></td>
                ...
                <td width="140"><textarea name="remark_reject[<?php echo h($row["id_qqc"]); ?>]" id="textarea" rows="2" cols="10" maxlength="250" ><?php if (isset($_POST['remark_reject'][($row["id_qqc"])])) { echo h($_POST['remark_reject'][($row["id_qqc"])] ?? ''); } ?></textarea>
               <input name="id_qqc[<?php echo $k; ?>]" type="hidden" value="<?php echo h($row["id_qqc"]); ?>">
          </td>
                </tr>
