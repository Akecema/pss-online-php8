...
$result_insert2 = mysqli_query($dbc, $query_insert2) or die(db_fail($dbc));
   ...
   $query8 = "SELECT COUNT(*) FROM qqc_transaction AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status = 'N' AND MR.status_QC != '".db_esc($dbc, $rst_sta4["status_desc"])."' ".$where_sql;
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_qc_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM qqc_transaction AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status = 'N' AND MR.status_QC != '".db_esc($dbc, $rst_sta4["status_desc"])."' ".$where_sql." ORDER BY MR.plan_no ASC";
...
onClick="Check(document.myform.cancel)">  </div><?php echo $no; ?></td>
                ...
                <td width="140"><textarea name="remark_reject[<?php echo h($row["id_qqc"]); ?>]" id="textarea" rows="2" cols="10" maxlength="250" ><?php if (isset($_POST['remark_reject'][($row["id_qqc"])])) { echo $_POST['remark_reject'][($row["id_qqc"])]; } ?></textarea>
