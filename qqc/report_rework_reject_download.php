...
/**
 * qqc/report_rework_reject_download.php
...
//********* CONDITION *************
			...
			$query_convert2 = "SELECT * FROM `work_center_detail` as SR WHERE SR.id_factory = ?"; $query_convert2_args = [$_GET["factory"]];
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_reject,'%d-%m-%Y %H:%i:%s') as R3 FROM reject_detail_disposal AS MR, work_center_detail AS SR WHERE SR.id_work = MR.work_center AND MR.status_part = 'QC' AND MR.qty_qc_NG != '' ".$where_sql." ORDER BY MR.plan_no ASC"; $query_args = [...$where_args];
