...
/**
 * qqc/report_wastage_reject2.php
 * Part of: QQC module (Quality)
 * Filename suggests: report wastage reject2
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, reject_detail_disposal, type_wastage_detail, reason_wastage, mat_master_header.
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_qqc.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a><a href="#" class="#">QA/QC</a> <a href="#" class="current">Report Wastage Reject</a></div>
  ...
  <div class="container-fluid">
           ...
           <form action="<?php echo h($_SERVER['PHP_SELF']); ?>" method="get" name="frmSearch" id="frmSearch">
            ...
            <tr>
              ...
              <td><select name="disposal_doc" id="disposal_doc">
                ...
                {
				?>
                <option value="<?php echo h($rowd1["doc_disposal_no"]); ?>"<?php if(($rowd1["doc_disposal_no"]) == ($_GET["disposal_doc"])) echo "selected"; ?>> <?php echo h($rowd1["doc_disposal_no"]); ?></option>
                <?php
                }
                ?>
              </select></td>
             <th>Disposal Doc. No. To : </th>
              <td><select name="disposal_doc2" id="disposal_doc2">
                ...
                {
				?>
                <option value="<?php echo h($rowd2["doc_disposal_no"]); ?>"<?php if(($rowd2["doc_disposal_no"]) == ($_GET["disposal_doc2"])) echo "selected"; ?>> <?php echo h($rowd2["doc_disposal_no"]); ?></option>
                <?php
                }
                ?>
              </select></td>
	...
	$query = "SELECT *, DATE_FORMAT(MR.date_wastage,'%d-%m-%Y  %H:%i:%s') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2 FROM reject_detail_disposal AS MR WHERE MR.status_part = 'WQ' AND MR.qty_wastage != '' AND MR.doc_disposal_no != '' ".$where_sql." ORDER BY MR.plan_no ASC"; $query_args = [...$where_args];
