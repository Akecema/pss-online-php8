...
/**
 * qqc/rework_history_output_list_tran.php
 * Part of: QQC module (Quality)
 * Filename suggests: rework history output list tran
 *
...
<?php
        if(isset($_POST["Submit2"]))
        {
            ...
            echo "<script>";
echo "window.location='rework_history_output_list_tran2.php?date1=$dateF&&date2=$dateT&&factory=$factory&&work_center=$work_center&&plan_no=$plan_no&&shift_ops=$shift_ops'";
            echo "</script>";
   ...
   $query8 = "SELECT COUNT(*) FROM qqc_detail_transaction WHERE status_QC = ? AND (date_plan >= '$start_date_check' AND date_plan <= '$end_date_check')"; $query8_args = [$rst_sta11["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_qc_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_create,'%d-%m-%Y') as R3 FROM qqc_detail_transaction WHERE status_QC = ? AND (date_plan >= '$start_date_check' AND date_plan <= '$end_date_check') ORDER BY plan_no ASC"; $query_args = [$rst_sta11["status_desc"]];
