...
echo "window.location='rework_output_list_tran2.php?date1=$dateF&&date2=$dateT&&factory=$factory&&work_center=$work_center&&plan_no=$plan_no&&shift_ops=$shift_ops&&model_code=$model_code'";
 ...
 // echo $end_date_check;
   ...
   $query8 = "SELECT COUNT(*) FROM qqc_detail_transaction AS MR, pps_detail_transaction AS PD WHERE MR.bflush_no = PD.bflush_no AND MR.status_QC = ? AND PD.status_pps != ? AND MR.status_QC != ? AND MR.qty_balance != '0' AND (MR.date_plan >= '$start_date_check' AND MR.date_plan <= '$end_date_check')"; $query8_args = [$rst_sta8["status_desc"], $rst_sta14["status_desc"], $rst_sta4["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_qc_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM qqc_detail_transaction AS MR, pps_detail_transaction AS PD WHERE MR.bflush_no = PD.bflush_no AND MR.status_QC = ? AND MR.status_QC != ? AND PD.status_pps != ? AND MR.qty_balance != '0' AND (MR.date_plan >= '$start_date_check' AND MR.date_plan <= '$end_date_check') ORDER BY MR.plan_no ASC"; $query_args = [$rst_sta8["status_desc"], $rst_sta4["status_desc"], $rst_sta14["status_desc"]];
	  ...
	  */
      ...
      ?>
               ...
               <!-- <td>
                ...
                <a value="Cancellation" href="cancel_qc_rework_output.php?uid=<?php echo h($row["id_qqc"]); ?>&&TB_iframe=true&height=400&width=1000" class="thickbox" target="_self"><img src="../img/delete.png" width="16" height="16" alt="Cancellation">Cancellation</a>
                 <?php }elseif($total_qty_pending < 0) {    ?>
                <a value="Cancellation" href="cancel_qc_rework_output.php?uid=<?php echo h($row["id_qqc"]); ?>&&TB_iframe=true&height=400&width=1000" class="thickbox" target="_self"><img src="../img/delete.png" width="16" height="16" alt="Cancellation">Cancellation</a>
                 <?php  }else{  ?>  <a value="Cancellation" href="cancel_qc_product_output.php?uid=<?php echo h($row["id_qqc"]); ?>&&TB_iframe=true&height=400&width=1000" class="thickbox" target="_self"><img src="../img/delete.png" width="16" height="16" alt="Cancellation">Cancellation Product Output</a> <?php } ?>
                </td>-->
                </tr>
