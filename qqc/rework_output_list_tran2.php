...
/**
 * qqc/rework_output_list_tran2.php
 * Part of: QQC module (Quality)
 * Filename suggests: rework output list tran2
 *
 * Behavior: requires an active login session ($_SESSION['username']); reads parameters from the query string ($_GET).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, factory_detail, work_center_detail, pps_detail, model_detail, qqc_detail_transaction, qqc_transaction.
...
<?php
		 ...
		 //3. Factory 
		  ...
		  //6. Shift
                if ($shift_ops == "NULL"){ 
                    ...
                    $wheresql_06 = " AND (MR.shift_day = ?)"; $wheresql_06_args = [$shift_ops]; }  
					
		  //7. Model Code
                if ($model_code == "NULL"){ 
                    $wheresql_07 = ""; $wheresql_07_args = []; }
                else {
                    $wheresql_07 = " AND PD.model_code = ?"; $wheresql_07_args = [$model_code]; }   		 			
   ...
   $query8 = "SELECT COUNT(*) FROM qqc_detail_transaction AS MR, work_center_detail AS SR, pps_detail_transaction AS PD WHERE SR.id_work = MR.work_center AND MR.bflush_no = PD.bflush_no AND MR.status_QC = ? AND PD.status_pps != ? AND MR.status_QC != ? AND MR.qty_balance != '0'".$where_sql; $query8_args = [$rst_sta8["status_desc"], $rst_sta14["status_desc"], $rst_sta4["status_desc"], ...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_qc_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM qqc_detail_transaction AS MR, work_center_detail AS SR, pps_detail_transaction AS PD WHERE SR.id_work = MR.work_center AND MR.bflush_no = PD.bflush_no AND MR.status_QC = ? AND PD.status_pps != ? AND MR.status_QC != ? AND MR.qty_balance != '0'".$where_sql." ORDER BY MR.plan_no ASC"; $query_args = [$rst_sta8["status_desc"], $rst_sta14["status_desc"], $rst_sta4["status_desc"], ...$where_args];
...
<tr>
   ...
   $qty_total_NG2 = 0.000;
	   ...
	   $query_qqc = "SELECT * FROM qqc_detail_transaction WHERE bflush_no = ? AND plan_no = ?"; $query_qqc_args = [$row["bflush_no"], $row["plan_no"]];
	   ...
	   $query_qqc2 = "SELECT * FROM qqc_transaction WHERE bflush_no = ? AND plan_no = ? AND status_QC != ?"; $query_qqc2_args = [$data_qqc["bflush_no"], $data_qqc["plan_no"], $rst_sta4["status_desc"]];
	...
	$total_qty_pending = ($total_qty - (($qty_total_ok) + ($qty_total_NG)));
      ...
      ?>
                ...
                <td width="50"><?php echo h($qty_total_ok); ?></td>
                <td width="60"><?php echo h($qty_total_NG); ?></td>
                <td width="99"><?php echo h($row["status_QC"]); ?></td>
                <td width="99">
                <a value="Rework" href="qc_rework_output_list.php?uid=<?php echo h($row["id_qqc"]); ?>" ><img src="../img/page_white_magnify.png" width="16" height="16" alt="Rework">Rework</a>
                </td>
              <!--  <td>
                <?php if($total_qty_pending > 0) {    ?>
                <a value="Cancellation" href="cancel_qc_rework_output.php?uid=<?php echo h($row["id_qqc"]); ?>&&TB_iframe=true&height=400&width=1000" class="thickbox" target="_self"><img src="../img/delete.png" width="16" height="16" alt="Cancellation">Cancellation</a> <?php  }else{  ?>  <a value="Cancellation" href="cancel_qc_product_output.php?uid=<?php echo h($row["id_qqc"]); ?>&&TB_iframe=true&height=400&width=1000" class="thickbox" target="_self"><img src="../img/delete.png" width="16" height="16" alt="Cancellation">Cancellation Product Output</a> <?php } ?>
                </td>-->
                </tr>
