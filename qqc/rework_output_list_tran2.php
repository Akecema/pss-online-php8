...
<?php
		 ...
		 //3. Factory 
		  ...
		  //6. Shift
                if ($shift_ops == "NULL"){ 
                    ...
                    $wheresql_06 = " AND (MR.shift_day = '$shift_ops')"; }  
					
		  //7. Model Code
                if ($model_code == "NULL"){ 
                    $wheresql_07 = ""; }
                else {
                    $wheresql_07 = " AND PD.model_code = '$model_code'"; }   		 			
   ...
   $query8 = "SELECT COUNT(*) FROM qqc_detail_transaction AS MR, work_center_detail AS SR, pps_detail_transaction AS PD WHERE SR.id_work = MR.work_center AND MR.bflush_no = PD.bflush_no AND MR.status_QC = '".$rst_sta8["status_desc"]."' AND PD.status_pps != '".$rst_sta14["status_desc"]."' AND MR.status_QC != '".$rst_sta4["status_desc"]."' AND MR.qty_balance != '0'".$where_sql;
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_qc_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM qqc_detail_transaction AS MR, work_center_detail AS SR, pps_detail_transaction AS PD WHERE SR.id_work = MR.work_center AND MR.bflush_no = PD.bflush_no AND MR.status_QC = '".$rst_sta8["status_desc"]."' AND PD.status_pps != '".$rst_sta14["status_desc"]."' AND MR.status_QC != '".$rst_sta4["status_desc"]."' AND MR.qty_balance != '0'".$where_sql." ORDER BY MR.plan_no ASC";
	...
	$total_qty_pending = ($total_qty - (($qty_total_ok) + ($qty_total_NG)));
      ...
      ?>
              ...
              <!--  <td>
                ...
                <a value="Cancellation" href="cancel_qc_rework_output.php?uid=<?php echo $row["id_qqc"]; ?>&&TB_iframe=true&height=400&width=1000" class="thickbox" target="_self"><img src="../img/delete.png" width="16" height="16" alt="Cancellation">Cancellation</a> <?php  }else{  ?>  <a value="Cancellation" href="cancel_qc_product_output.php?uid=<?php echo $row["id_qqc"]; ?>&&TB_iframe=true&height=400&width=1000" class="thickbox" target="_self"><img src="../img/delete.png" width="16" height="16" alt="Cancellation">Cancellation Product Output</a> <?php } ?>
                </td>-->
                </tr>
