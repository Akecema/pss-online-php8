	...
	if ($result_id) 
{
  ...
  if(isset($_POST["cancel"])) 
  {
	...
	$string = "";
	  ...
	  $query_generate = "SELECT *, DATE_FORMAT(date_wastage,'%Y-%m-%d') AS P, DATE_FORMAT(date_disposal,'%Y-%m-%d') AS P2, DATE_FORMAT(date_disposal,'%H:%i:%s') AS P3 FROM reject_detail_disposal WHERE doc_disposal_no = ?"; $query_generate_args = [$ref];
...
file_put_contents($file,$data);
   ...
   $query8 = "SELECT COUNT(*) FROM reject_detail_disposal WHERE status_disposal = ? AND status_part = 'WQ' AND doc_disposal_no = ''"; $query8_args = [$rst_sta["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2 FROM reject_detail_disposal WHERE status_disposal = ? AND status_part = 'WQ' AND doc_disposal_no = '' order by plan_no ASC"; $query_args = [$rst_sta["status_desc"]];
...
onClick="Check(document.myform.cancel)">  </div><?php echo $no; ?></td>
          ...
          </td><td><a value="Remove Item" href="cancel_wastage_tran_proc.php?uid=<?php echo h($row["id_disposal"]); ?>&&TB_iframe=true&height=400&width=1000" class="thickbox" target="_self"><img src="../img/delete.png" width="16" height="16" alt="Remove Item">Remove Item</a>
               </td>
                </tr>
