	...
	if ($result_id) 
{
  ...
  if(isset($_POST["cancel"])) 
  {
	...
	$string = "";
	  ...
	  $query_generate = "SELECT *, DATE_FORMAT(date_wastage,'%Y-%m-%d') AS P, DATE_FORMAT(date_disposal,'%Y-%m-%d') AS P2, DATE_FORMAT(date_disposal,'%H:%i:%s') AS P3 FROM reject_detail_disposal WHERE doc_disposal_no = '".$ref."'";
...
file_put_contents($file,$data);
   ...
   $query8 = "SELECT COUNT(*) FROM reject_detail_disposal WHERE status_disposal = '".$rst_sta["status_desc"]."' AND status_part = 'WQ' AND doc_disposal_no = ''";
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2 FROM reject_detail_disposal WHERE status_disposal = '".$rst_sta["status_desc"]."' AND status_part = 'WQ' AND doc_disposal_no = '' order by plan_no ASC";
