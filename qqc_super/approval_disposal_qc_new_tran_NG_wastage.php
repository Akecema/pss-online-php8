...
/**
 * qqc_super/approval_disposal_qc_new_tran_NG_wastage.php
 * Part of: QQC module (supervisor/admin tier)
 * Filename suggests: approval disposal qc new tran NG wastage
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, reject_detail_disposal, wastage_transaction, type_wastage_detail, reason_wastage, work_center_detail, ftp_approval_qc, mat_master_header.
...
//------------update remarks reject detail disposal-------------------
 ...
 $query_upd3 = "UPDATE reject_detail_disposal SET remark_approve = ?, approve_by = ?, date_approve = NOW(), status_disposal = ? WHERE id_disposal = ?"; $query_upd3_args = [$string[$i], $username, $rst_sta3["status_desc"], $cancel[$i]];
 $result_upd3 = db_query_bind($dbc, $query_upd3, $query_upd3_args); 
  ...
  // //----------------------update table wastage_disposal
	  $query_wastage_update = "UPDATE wastage_transaction SET user_update = ?, date_update = NOW(), remark_approve = ?, approve_by = ?, date_approve = NOW(), status_disposal = ? WHERE id_wastage_tran = ?"; $query_wastage_update_args = [$username, $row_all["remark_approve"], $username, $rst_sta3["status_desc"], $row_all["uid"]];
	 ...
	 $qry = db_query_bind($dbc, "SELECT *, DATE_FORMAT(A1.date_disposal,'%Y-%m-%d') AS M, DATE_FORMAT(A1.date_posting,'%Y-%m-%d') AS M2 FROM reject_detail_disposal AS A1, wastage_transaction AS A2 WHERE A2.id_wastage_tran = A1.uid AND A1.doc_disposal_no  = ?", [$doc_disposal]);
$data = "";
while($row_1 = mysqli_fetch_array($qry)) {
	
/*  $data .= $row_1['doc_disposal_no'].";".$row_1['id_disposal']."\r\n";*/

    $query_type = "SELECT * FROM type_wastage_detail WHERE id_wastage = ? ORDER BY id_wastage ASC"; $query_type_args = [$row_1['type_wastage']];
    $result_type = db_query_bind($dbc, $query_type, $query_type_args);
    $row_type = mysqli_fetch_array($result_type); 
	
	$query_reason = "SELECT * FROM reason_wastage WHERE id_reason_wastage = ? ORDER BY id_reason_wastage ASC"; $query_reason_args = [$row_1['reason_wastage']];
    $result_reason = db_query_bind($dbc, $query_reason, $query_reason_args);
    $row_reason = mysqli_fetch_array($result_reason);
	
	$query_fac = "SELECT * FROM work_center_detail WHERE id_work = ? ORDER BY id_work ASC"; $query_fac_args = [$row_1['work_center']];
    $result_fac = db_query_bind($dbc, $query_fac, $query_fac_args);
    $row_fac = mysqli_fetch_array($result_fac);

  
   $data .= $row_1['material_type'].";".$row_1['model_code'].";".$row_1['material_no'].";551;".";".$row_1['comp_code'].";".$row_1['ploc_prod_reject'].";".$row_type['wastage_desc'].";".$row_reason['reason_wastage_desc'].";".$row_1['qty_wastage'].";".$row_1['UOM_unit'].";".$res['user_fullname'].";".$row_1['M'].";".$row_1['M2'].";".$row_1['cost_center'].";".$row_1['doc_disposal_no']."\r\n";
...
$file = "../FromPortal2/GI/".$filen.".csv";
//chmod($file, 0777);
file_put_contents($file,$data);

       

 $qry_all = "SELECT *, DATE_FORMAT(A1.date_disposal,'%Y-%m-%d') AS M, DATE_FORMAT(A1.date_posting,'%Y-%m-%d') AS M2 FROM reject_detail_disposal AS A1, wastage_transaction AS A2 WHERE A2.id_wastage_tran = A1.uid AND A1.doc_disposal_no  = ?"; $qry_all_args = [$doc_disposal];
  $result_all = db_query_bind($dbc, $qry_all, $qry_all_args);
  while($row_all = mysqli_fetch_array($result_all))
   {
  //----------update table ftp_approval_qc------------
   
    $query_ftp_info = "INSERT INTO ftp_approval_qc(id,file_name,doc_disposal_no,id_disposal,material_no, material_desc,qty_ftp,uom,status_ftp,posting_date,posting_time,user_create,date_create,status_part) VALUES('',?,?,?,?,?,?,?,'Y',?,?,?,NOW(),?)"; $query_ftp_info_args = [$filen, $row_all['doc_disposal_no'], $row_all["id_disposal"], $row_all["material_no"], $row_all["material_desc"], $row_all["qty_wastage"], $row_all["UOM_unit"], $row_all["date_posting"], $row_all["time_wastage"], $username, $row_all["status_part"]]; 
...
<p align="center" class="style4">&nbsp;</p>
      ...
      <!-- Content -->
              ...
              <!-- End Box Head -->
          <form name="myform" action="approval_disposal_qc_new_tran_NG_wastage.php?doc_disposal=<?php echo h($doc_disposal); ?>" method="post" >
            <!-- Form -->
            <div class="form">
