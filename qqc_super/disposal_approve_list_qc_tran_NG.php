...
/**
 * qqc_super/disposal_approve_list_qc_tran_NG.php
 * Part of: QQC module (supervisor/admin tier)
 * Filename suggests: disposal approve list qc tran NG
 *
...
<?php
        if(isset($_POST["Submit2"]))
        {
            ...
            echo "<script>";
echo "window.location='disposal_approve_list_qc_tran_NG2.php?date1=$dateF&&date2=$dateT&&doc_disposal_no=$doc_disposal_no'";
            echo "</script>";
   ...
   $query8 = "SELECT COUNT(*) FROM reject_detail_disposal WHERE status_disposal = ? AND status_part = 'QC' AND doc_disposal_no != ''"; $query8_args = [$rst_sta3["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2 FROM reject_detail_disposal WHERE status_disposal = ? AND status_part = 'QC' AND doc_disposal_no != '' GROUP BY doc_disposal_no ORDER BY plan_no ASC"; $query_args = [$rst_sta3["status_desc"]];
