...
/**
 * qqc_super/disposal_approved_qc_tran_NG.php
 * Part of: QQC module (supervisor/admin tier)
 * Filename suggests: disposal approved qc tran NG
 *
...
<?php
        if(isset($_POST["Submit2"]))
        {
            ...
            echo "<script>";
echo "window.location='disposal_approved_qc_tran_NG2.php?date1=$dateF&&date2=$dateT&&doc_disposal_no=$doc_disposal_no'";
            echo "</script>";
   ...
   $query8 = "SELECT * FROM reject_detail_disposal WHERE (status_part = 'QC' OR status_part = 'WQ') AND doc_disposal_no != '' AND status_disposal != ? AND status_disposal != ? GROUP BY doc_disposal_no"; $query8_args = [$rst_sta["status_desc"], $rst_sta16["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2 FROM reject_detail_disposal WHERE (status_part = 'QC' OR status_part = 'WQ') AND doc_disposal_no != '' AND status_disposal != '".db_esc($dbc, $rst_sta["status_desc"])."' AND status_disposal != '".db_esc($dbc, $rst_sta16["status_desc"])."'  GROUP BY doc_disposal_no ORDER BY plan_no ASC $pages->limit";
