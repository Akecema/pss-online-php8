...
<?php
   ...
   $query8 = "SELECT * FROM reject_detail_disposal AS MR WHERE (MR.status_part = 'QC' OR MR.status_part = 'WQ') AND MR.doc_disposal_no != '' AND MR.status_disposal != ? AND MR.status_disposal != ? ".$where_sql." GROUP BY MR.doc_disposal_no"; $query8_args = [$rst_sta["status_desc"], $rst_sta16["status_desc"], ...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2 FROM reject_detail_disposal AS MR WHERE (MR.status_part = 'QC' OR MR.status_part = 'WQ') AND MR.doc_disposal_no != '' AND MR.status_disposal != ? AND MR.status_disposal != ? ".$where_sql." GROUP BY MR.doc_disposal_no ORDER BY MR.plan_no ASC $pages->limit"; $query_args = [$rst_sta["status_desc"], $rst_sta16["status_desc"], ...$where_args];
