...
/**
 * qqc_super/disposal_production_tran_NG_approved.php
 * Part of: QQC module (supervisor/admin tier)
 * Filename suggests: disposal production tran NG approved
 *
            ...
            <tr>
			 ...
			 ?></td>
              ...
              <td><select name="doc_disposal_no" id="doc_disposal_no" class="span8">
                  ...
                  <?php
	             $query9 = "SELECT * FROM reject_detail_disposal WHERE (status_part = 'PR' OR status_part = 'WS') AND (status_disposal = ?) GROUP BY doc_disposal_no"; $query9_args = [$rst_sta17["status_desc"]];
...
<?php
        if(isset($_POST["Submit2"]))
        {
            ...
            echo "<script>";
echo "window.location='disposal_production_tran_NG2_approved.php?date1=$dateF&&date2=$dateT&&doc_disposal_no=$doc_disposal_no'";
            echo "</script>";
   ...
   $query8 = "SELECT * FROM reject_detail_disposal WHERE (status_part = 'PR' OR status_part = 'WS') AND doc_disposal_no != '' AND status_disposal = ? GROUP BY doc_disposal_no ORDER BY plan_no ASC"; $query8_args = [$rst_sta17["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2 FROM reject_detail_disposal WHERE (status_part = 'PR' OR status_part = 'WS') AND doc_disposal_no != '' AND status_disposal = '".db_esc($dbc, $rst_sta17["status_desc"])."' GROUP BY doc_disposal_no ORDER BY plan_no ASC $pages->limit";
   ...
   while ($row = mysqli_fetch_array($rs))
   {
      ?>
               <tbody>
                ...
                <td width="93">
              <?php
			    if($row["status_disposal"] == "Approved QC"){
