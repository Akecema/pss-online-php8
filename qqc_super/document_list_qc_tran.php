...
/**
 * qqc_super/document_list_qc_tran.php
 * Part of: QQC module (supervisor/admin tier)
 * Filename suggests: document list qc tran
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, factory_detail, pps_detail, qqc_detail_transaction, qqc_transaction.
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_qqc_super.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a><a href="#" class="#">QA/QC</a> <a href="#" class="current">Document List</a></div>
  <h1>Document List</h1>
</div>
      ...
      <form action="<?php echo h($_SERVER['PHP_SELF']); ?>" method="post" name="frmSearch" id="frmSearch">
	       ...
	       $query3 = "SELECT * FROM factory_detail GROUP BY factory_desc2 ORDER BY id_fac ASC";
              ...
              <td><select name="plan_no" id="plan_no" class="span11">
                  ...
                  <?php
	             $query9 = "SELECT * FROM pps_detail WHERE status_pps = ? ORDER BY plan_no ASC"; $query9_args = [$rst_sta13["status_desc"]];
...
<?php
        if(isset($_POST["Submit2"]))
        {
            ...
            echo "<script>";
echo "window.location='document_list_qc_tran2.php?date1=$dateF&&date2=$dateT&&factory=$factory&&work_center=$work_center&&plan_no=$plan_no&&shift_ops=$shift_ops'";
            echo "</script>";
   ...
   $query8 = "SELECT COUNT(*) FROM qqc_detail_transaction WHERE status_QC = ?"; $query8_args = [$rst_sta11["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_qc_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_create,'%d-%m-%Y') as R3 FROM qqc_detail_transaction WHERE status_QC = ? ORDER BY plan_no ASC"; $query_args = [$rst_sta11["status_desc"]];
   ...
   $total_qty = 0.000;	
	...
	if($sta_out2 == "211")
	{
	 ...
	 // $status_output = "";
	   ...
	   $query_qqc = "SELECT * FROM qqc_transaction WHERE qqc_doc_no = ?"; $query_qqc_args = [$row["qqc_doc_no"]];
