...
/**
 * qqc_super/qc_rework_display_detail.php
 * Part of: QQC module (supervisor/admin tier)
 * Filename suggests: qc rework display detail
 *
 * Behavior: requires an active login session ($_SESSION['username']); reads parameters from the query string ($_GET).
 * Database tables referenced: user_detail, sys_setup_maintain, request_status, qqc_transaction.
...
<div class="widget-box">
 ...
 $uid = $_GET["uid"];
  ...
  //--------- qqc transaction detail ------------
	   ...
	   $query_pps = "SELECT * FROM qqc_transaction WHERE id_tran = ?"; $query_pps_args = [$uid];
...
<p>&nbsp;</p> 
  <table class="table table-bordered data-table">
   ...
   $query_display = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_qc_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_create,'%d-%m-%Y') as R3 FROM qqc_transaction AS MR WHERE MR.status_QC = ? AND MR.id_tran = ?"; $query_display_args = [$rst_sta8["status_desc"], $uid];
