      ...
      <form action="<?php echo h($_SERVER['PHP_SELF']); ?>" method="post" name="frmSearch" id="frmSearch">
            ...
            <tr>
              ...
              <td><select name="material_no" id="material_no">
			      ...
			      {
				   ?>
                  <option value="<?php echo h($row10["material_no"]); ?>"> <?php echo h($row10["material_no"]); ?></option>
                  <?php
                  }
				?>
              </select></td>
...
echo "window.location='report_all_planning_module2.php?date1=$dateF&&date2=$dateT&&factory=$factory&&work_center=$work_center&&plan_no=$plan_no&&shift_ops=$shift_ops&&material_no=$material_no'";
   ...
   $query8 = "SELECT COUNT(*) FROM pps_detail WHERE (status_pps != ? AND status_pps != ?)"; $query8_args = [$rst_sta4["status_desc"], $rst_sta16["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_posting,'%H:%i:%s') as T2 FROM pps_detail WHERE (status_pps != ? AND status_pps != ?) ORDER BY plan_no ASC"; $query_args = [$rst_sta4["status_desc"], $rst_sta16["status_desc"]];
...
<tr>
  ...
  $row4_p = mysqli_fetch_array($result4_p);
	...
	?>	
                ...
                <td width="60"><?php echo h($data_mat_h["material_type"]); ?></td>
               ...
               <!-- <td width="120"><?php //echo h($data_mat_h["material_desc"]); ?></td>-->
                ...
                <td width="60"><?php echo h($data_mat_h["sloc"]); ?></td>
