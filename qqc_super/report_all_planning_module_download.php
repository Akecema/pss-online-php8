...
/**
 * qqc_super/report_all_planning_module_download.php
 * Part of: QQC module (supervisor/admin tier)
 * Filename suggests: report all planning module download
 *
 * Behavior: requires an active login session ($_SESSION['username']); processes submitted form data ($_POST).
 * Database tables referenced: sys_setup_maintain, request_status, pps_detail, mat_master_header, pps_detail_transaction, qqc_detail_transaction, qqc_transaction.
 * Includes: config.php.
 *
...
$namaFile = "Report Planned Order_".$date_tdy.".xls";
 //convert material no kpd id_hdr
  ...
  $query8 = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2 FROM pps_detail WHERE (status_pps != ? AND status_pps != ? AND status_pps != ?) ORDER BY plan_no ASC"; $query8_args = [$rst_sta2["status_desc"], $rst_sta4["status_desc"], $rst_sta16["status_desc"]];
...
$query = "SELECT *, DATE_FORMAT(date_plan,'%d-%m-%Y') as R, DATE_FORMAT(date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(date_posting,'%H:%i:%s') as T2 FROM pps_detail WHERE (status_pps != ? AND status_pps != ? AND status_pps != ?) ORDER BY plan_no ASC"; $query_args = [$rst_sta2["status_desc"], $rst_sta4["status_desc"], $rst_sta16["status_desc"]];
