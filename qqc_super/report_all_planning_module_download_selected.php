...
/**
 * qqc_super/report_all_planning_module_download_selected.php
...
$namaFile = "Report Planned Order_".$date_tdy.".xls";
 //convert material no kpd id_hdr
    ...
    $query8 = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2 FROM pps_detail AS MR, work_center_detail AS SR WHERE MR.work_center = SR.id_work AND (MR.status_pps != ? AND MR.status_pps != ?)".$where_sql. " ORDER BY MR.plan_no ASC"; $query8_args = [$rst_sta4["status_desc"], $rst_sta16["status_desc"], ...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_plan,'%d-%m-%Y') as R, DATE_FORMAT(MR.date_posting,'%d-%m-%Y') as R2, DATE_FORMAT(MR.date_posting,'%H:%i:%s') as T2 FROM  pps_detail AS MR, work_center_detail AS SR WHERE MR.work_center = SR.id_work AND (MR.status_pps != ?  AND MR.status_pps != ?)".$where_sql. " ORDER BY MR.plan_no ASC"; $query_args = [$rst_sta4["status_desc"], $rst_sta16["status_desc"], ...$where_args];
...
//count how many data
 ...
 while ($row2 = mysqli_fetch_array($rs))
   {
	    //---------get material header---------
	    $query_mat_h = "SELECT * FROM table_material WHERE material_no = ?"; $query_mat_h_args = [$row2['material_no']];
		$result_mat_h = db_query_bind($dbc, $query_mat_h, $query_mat_h_args);
		$data_mat_h = mysqli_fetch_array($result_mat_h);	 
		
		//---------get sloc ---------
	    $query_mat_h2 = "SELECT * FROM mat_master_detail WHERE material = ?"; $query_mat_h2_args = [$row2['material_no']];
...
//quantity output
	...
	$sta_out_2 = substr($row_rst_display2["qqc_no"],2,3);
	
	if($sta_out_2 == "421")
	{
		$status_output2 = "QC OK";
	}elseif($sta_out_2 == "331")
	{
	    $status_output2 = "QC NG";
	}else{
	    // $status_output2 = "";
