	   ...
	   while ($row2 = mysqli_fetch_array($result_tp))
{
  ...
  $query4_p = "SELECT * from mat_master_header as LD, mat_master_detail as SD WHERE SD.id_hdr = LD.id_hdr and SD.id_dtl = '".db_esc($dbc, $row2["id_dtl_wip"])."'";
  	$result4_p = mysqli_query($dbc, $query4_p);
 	$row4_p = mysqli_fetch_array($result4_p); 
	

 $query_upd2 = "UPDATE post_detail_header_wip SET status_posting = 'Close', date_close = NOW() WHERE mrin_no = '".db_esc($dbc, $temp_mrin)."'";
 $result_upd2 = mysqli_query($dbc, $query_upd2); 
	      
 $query_upd3 = "UPDATE wip_request SET status = 'Close', user_update = '".db_esc($dbc, $user_no)."', date_update = NOW() WHERE temp_mrin_wip = '".db_esc($dbc, $temp_mrin)."'";
 $result_upd3 = mysqli_query($dbc, $query_upd3); 
	   ...
	   //---------------------------------------------------------------------------------------------------------------	//-insert material request with status "Cancel" into table material request cancel
		  //---------------------------------------------------------------------------------------------------------------
		  	...
		  	$query_mm3 = "SELECT * FROM wip_request WHERE temp_mrin_wip = '".db_esc($dbc, $temp_mrin)."' AND status = 'Close' AND id_dtl_wip = '".db_esc($dbc, $row2["id_dtl_wip"])."'"; 
	...
	} elseif(($row4_p["mat_type"] == "Z200") && ($data_2["factory"] == "2"))
	{
	  ...
	  }elseif($row4_p["mat_type"] == "Z200")
	 {
	  ...
	  $sta = "Invalid";
		...
		$query_tp = "SELECT *,SUM(rquantity) as TOT FROM post_detail_header_wip WHERE mrin_no = '".db_esc($dbc, $row["temp_mrin_wip"])."' AND prod_order = '".db_esc($dbc, $row_scan["prod_order"])."' AND mvt_type = 311 AND material_no = '".db_esc($dbc, $row4_p["bill_component"])."' AND status_posting != 'Cancel'";
	$result_tp  = mysqli_query($dbc, $query_tp); 
