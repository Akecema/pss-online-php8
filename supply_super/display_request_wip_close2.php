...
/**
 * supply_super/display_request_wip_close2.php
 * Part of: Supply Chain module (supervisor/admin tier)
 * Filename suggests: display request wip close2
 *
 * Behavior: requires an active login session ($_SESSION['username']); reads parameters from the query string ($_GET).
 * Database tables referenced: user_detail, sys_setup_maintain, factory_detail, work_center_detail, wip_request, scan_detail_wip, mat_master_header.
...
<div id="content-header">
  <div id="breadcrumb"> <a href="index_supply_super.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="tip-bottom">WIP Request</a><a href="#" class="current">WIP Request Maintenance</a> </div>
  <h1>WIP Request</h1>
</div>
...
<div class="container-fluid">
  ...
  <!--  <div class="span6">-->
    ...
    $query8 = "SELECT *, DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R FROM wip_request AS MR, scan_detail_wip AS SD WHERE MR.id_scan_wip = SD.id_scan AND MR.status_request = 'Y' AND (MR.status != 'Close' AND MR.status != 'Cancel')".$where_sql." GROUP BY MR.temp_mrin_wip"; $query8_args = [...$where_args];
...
$query = "SELECT *, DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R FROM wip_request AS MR, scan_detail_wip AS SD WHERE MR.id_scan_wip = SD.id_scan AND MR.status_request = 'Y' AND (MR.status != 'Close' AND MR.status != 'Cancel')".$where_sql." GROUP BY MR.temp_mrin_wip ORDER BY MR.date_posting ASC, MR.temp_mrin_wip ASC "; $query_args = [...$where_args];
...
?>
  ...
  $row4_p = mysqli_fetch_array($result4_p); 
	...
	if(($row4_p["mat_type"] == "Z200") && ($row2["factory"] == "1"))
	{
	  $sta = "S120";
	} elseif(($row4_p["mat_type"] == "Z200") && ($row2["factory"] == "2"))
	{
	  $sta = "S220";
	} elseif(($row4_p["mat_type"] == "Z200") && ($row2["factory"] == "3"))
	 {
	  $sta = "S320";
  ...
  <tr>
    ...
    <td width="50"><a href="detail_wip_request_printing.php?mrin_no=<?php echo h($row2["temp_mrin_wip"]); ?>&&prod_order=<?php echo h($row2["prod_order"]); ?>&&TB_iframe=true&height=400&width=1000" class="thickbox" target="_self"><img src="../img/print.jpg" width="16" height="16" alt="Print">Print</a> </td>
  </tr>  
  <?php 
