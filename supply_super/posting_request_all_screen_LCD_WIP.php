...
/**
 * supply_super/posting_request_all_screen_LCD_WIP.php
 * Part of: Supply Chain module (supervisor/admin tier)
 * Filename suggests: posting request all screen LCD WIP
 *
 * Behavior: requires an active login session ($_SESSION['username']).
 * Database tables referenced: user_detail, sys_setup_maintain, wip_request, scan_detail_wip, factory_detail, mat_master_header, post_detail_header_wip, wip_request_close.
 * Includes: config.php, paginator.class2.php, tc_calendar.php, content.php, top_modal_menu.php, left_supply_super_menu.php, footer.php.
 *
...
$url = "posting_request_all_screen_LCD_WIP.php";
...
<div id="content">
 <div id="content-header">
  <div id="breadcrumb"> <a href="index_supply_super.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="tip-bottom">Display Board</a> <a href="#" class="current">WIP Request Board</a> </div>
  <h1>Display Board</h1>
</div>
 ...
 $query = "SELECT *, DATE_FORMAT(MR.date_mrin,'%d-%m-%Y') AS R FROM wip_request AS MR, scan_detail_wip AS SD WHERE MR.id_scan_wip = SD.id_scan AND MR.status_request = 'Y' AND (MR.status != 'Close' AND MR.status  != 'Cancel') GROUP BY MR.temp_mrin_wip ORDER BY MR.date_mrin DESC,MR.time_mrin DESC";
...
//---------------------------------------------------------------------------------------------------------------------
	...
	$query_tp = "SELECT *,SUM(rquantity) as TOT FROM post_detail_header_wip WHERE mrin_no = ? AND prod_order = ? AND mvt_type = 311 AND status_posting = 'New' GROUP BY material_no"; $query_tp_args = [$row2["temp_mrin_wip"], $row_scan["prod_order"]];
	...
	while($row_tp = mysqli_fetch_assoc($result_tp))
{
 ...
 $query_upd2 = "UPDATE `post_detail_header_wip` SET status_posting = 'Close', date_close = NOW() WHERE mrin_no = ? AND material_no = ? "; $query_upd2_args = [$row2["temp_mrin"], $row_tp["material_no"]];
 $result_upd2 = db_query_bind($dbc, $query_upd2, $query_upd2_args); 
	      
 $query_upd3 = "UPDATE `wip_request` SET status = 'Close' WHERE temp_mrin_wip = ? AND bom_component = ? "; $query_upd3_args = [$row2["temp_mrin"], $row_tp["material_no"]];
 $result_upd3 = db_query_bind($dbc, $query_upd3, $query_upd3_args); 
 
	 $query_upd4 = "UPDATE `wip_request` SET status = 'Close' WHERE temp_mrin_wip = '".db_esc($dbc, $row2["temp_mrin"])."' AND (bom_qty = '0.000' OR bom_qty = '')";
        ...
        if($result_upd3 || $result_upd4)
		 {
		   ...
		   $query_upd4 = "SELECT * FROM `wip_request` WHERE temp_mrin_wip = ? AND status = 'Close'"; $query_upd4_args = [$row2["temp_mrin"]];
...
$result_mm3_insert = db_query_bind($dbc, $query_mm3_insert, $query_mm3_insert_args) or die(db_fail($dbc));
	   ...
	   }elseif(($row["bom_qty_wip"] > $tp_quantity))
       {
		  ...
		  ?>  
           <tr class="gradeX">
            <td>&nbsp;<?php echo h($row2["temp_mrin_wip"]); ?>&nbsp;&nbsp;<?php if($row2["status_urgent"] == "Y") 
	{
	?>
