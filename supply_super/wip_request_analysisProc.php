...
/**
 * supply_super/wip_request_analysisProc.php
 * Part of: Supply Chain module (supervisor/admin tier)
 * Filename suggests: wip request analysisProc
 *
 * Behavior: requires an active login session ($_SESSION['username']); reads parameters from the query string ($_GET).
 * Database tables referenced: user_detail, sys_setup_maintain, factory_detail, work_center_detail, mat_master_detail, mat_master_header, wip_request, scan_detail_wip, post_detail_header_wip.
...
<div id="content">
 <div id="content-header" class="noprint">
  <div id="breadcrumb"> <a href="index_supply_super.php" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="tip-bottom">WIP Request</a> <a href="#" class="current">WIP Request Report</a> </div>
  <h1>WIP Request</h1>
</div>
...
<tr>
    ...
    <td width="85%"> <div class="small-nav"></div></td> 
      <td width="7%"><a href="report_wip_request_analysisProc.php?factory=<?php echo h($factory); ?>&&work_center=<?php echo h($work_center); ?>&&status=<?php echo h($status); ?>&&date1=<?php echo h($dateF); ?>&&date2=<?php echo h($dateT); ?>&&material_no=<?php echo h($material_no); ?>"><img src="../img/dload_excel.jpg" width="48" height="48" title="Download" /></a></td>
   ...
   while ($row2 = mysqli_fetch_array($rs))
   {
	...
	$query6 = "SELECT * FROM post_detail_header_wip AS PD, wip_request AS MR WHERE PD.mrin_no = MR.temp_mrin_wip AND PD.material_no = MR.bom_component AND PD.mrin_no = ? AND PD.material_no = ? AND PD.mvt_type = 311"; $query6_args = [$row2["temp_mrin_wip"], $row2["bom_component"]];
...
//------------------------------------------------------------------------------------------------------------------	
    ...
    $query_tp = "SELECT *,SUM(rquantity) as TOT FROM post_detail_header_wip WHERE mrin_no = ? AND prod_order = ? AND mvt_type = 311 AND material_no = ?"; $query_tp_args = [$row2["temp_mrin_wip"], $row_scan["prod_order"], $row4_p["bill_component"]];
