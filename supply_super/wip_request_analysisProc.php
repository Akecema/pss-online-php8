...
<tr>
   ...
   while ($row2 = mysqli_fetch_array($rs))
   {
	...
	$query6 = "SELECT * FROM post_detail_header_wip AS PD, wip_request AS MR WHERE PD.mrin_no = MR.temp_mrin_wip AND PD.material_no = MR.bom_component AND PD.mrin_no = '".db_esc($dbc, $row2["temp_mrin_wip"])."' AND PD.material_no = '".db_esc($dbc, $row2["bom_component"])."' AND PD.mvt_type = 311";
...
//------------------------------------------------------------------------------------------------------------------	
    ...
    $query_tp = "SELECT *,SUM(rquantity) as TOT FROM post_detail_header_wip WHERE mrin_no = '".db_esc($dbc, $row2["temp_mrin_wip"])."' AND prod_order = '".db_esc($dbc, $row_scan["prod_order"])."' AND mvt_type = 311 AND material_no = '".db_esc($dbc, $row4_p["bill_component"])."'";
